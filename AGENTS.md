...
- 複数の child `HWND` を持つ pane／dialog の `WM_SIZE` と splitter relayout は、最終 geometry を先に計算し、変更のない child を原則として動かさず、変更対象を中間描画なしでまとめて配置してから、移動前に露出した領域を含む bounded な範囲を一度だけ同期再描画する。`WM_SETREDRAW` を解除した control の最終 visibility を再公開する場合は、同じ bounds でも配置対象にしてよい。子ごとの配置と再描画を交互に行わない。
- batched placement は変更前の geometry と local visibility を保持し、deferred／fallback 配置の最終状態を全件検証する。途中失敗時は旧状態への rollback を全件試行して検証する。rollback 検証も失敗した場合は成功扱いせず、candidate の論理状態／hit-test state を commit せずに旧 projection の再構築または再配置経路へ戻す。
- pane parent は必要に応じて `WS_CLIPCHILDREN` または同等の clipping を使い、現在の child 領域を親の背景消去から除外する。標準 control は背景消去を含む最終描画、全面を描く owner-draw surface は不要な erase を避ける。複数 child の旧領域が交差する場合は pane subtree 全体、単一境界や少数 control だけなら affected old/new bounds の union を dirty region とし、workspace 全体の同期再描画で代用しない。DockHost の Structure 変更は old/new right-zone と tool-tab bounds も明示的に含める。
- geometry-only resize で control `HWND`、pane-owned tab/list contents、selection、focus、および新しい有効範囲内に残る scroll position を再生成または reset しない。resize／reflow で旧 scroll position が新しい有効範囲外になる場合だけ clamp する。DockHost chrome の tab projection は Structure 変更時に redraw を抑止して再同期してよい。実装上の `SWP_NOREDRAW`、deferred placement、`RedrawWindow` flags と検証方法は `docs/architecture.md` の Win32 multi-pane resize painting contract を正本とする。
- DockHost の splitter、および同じ DockHost 親に留まる right-pane の追加／削除、tab 構造変更は同じ二層の resize transaction として扱う。外側 transaction は affected sibling pane の配置中に内側 pane の同期 repaint を保留し、tab、pane、splitter が最終配置へ到達してから bounded な範囲を一度だけ同期再描画する。同期 DockHost mutation の配置または内側 layout が失敗した場合は、mutation 前の DockLayout／right-tab model を復元し、物理 geometry の rollback を全件試行・検証して旧 projection を再適用する。rollback 検証失敗も command failure とし、candidate model／projection を commit しない。Floating／expanded AutoHide／親変更は各親の別 transaction とし、この same-parent contract へ混在させない。layout 通知を理由に surviving pane の pane-owned tab／list 内容を refresh、reset、再構築しない。
- resize transaction 中の `WM_SETFONT`、column width、item height、scrollbar 等の Common Controls metric 変更は即時 redraw を無効化するか、local visibility を保持する `WM_SETREDRAW` guard で囲む。guard は metric mutation を包み、visibility-dependent placement の検証前に解除し、その invalidation を最終 `RedrawWindow` の `RDW_FRAME`／subtree repaint へ回収する。
- Common Controls の tab と sibling page は最終 repaint 前に z-order を確定する。tab を `HWND_BOTTOM`、可視 page を既存の sibling／keyboard order を保って前面へ配置し、全操作と最終 z-order を検証する。失敗時は元の sibling order への復元を全件試行・検証し、復元検証失敗も resize transaction failure とする。
