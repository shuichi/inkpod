...
- アプリケーションのバージョンは 0.3.2、Rust／C++ 間の公開 ABI は v34 です。
...
```text
build/windows-x64-release/package/Inkpod-0.3.2-windows-x64.zip
build/windows-arm-release/package/Inkpod-0.3.2-windows-arm.zip
```
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.3.2 -DryRun
```
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.3.2 -Publish
```
