...
- ネイティブ形式は `.inkpod` v33（replay epoch 28）、バッチ設定は `.inkbatch` v5 です。フォーマットフリーズ前のため、各形式は現在のバージョンだけを受理します。
...
- `.inkpod` v33 形式に加え、PNG、TIFF、TGA、BMP の読み込みと書き出しに対応しています。
...
- 現在のネイティブ保存形式は `.inkpod` v33（replay epoch 28）です。フォーマットフリーズ宣言までは現在のバージョンだけを受理し、v32 以前を含む非 v33 形式の migration reader は提供しません。v33 の圧縮方式は code 0 のみです。
