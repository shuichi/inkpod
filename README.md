...
- アプリケーションのバージョンは 0.3.0、Rust／C++ 間の公開 ABI は v25 です。
...
```text
build/windows-x64-release/package/Inkpod-0.3.0-windows-x64.zip
build/windows-arm-release/package/Inkpod-0.3.0-windows-arm.zip
```
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.3.0 -DryRun
```
...
- 現在のネイティブ保存形式は `.inkpod` v32（replay epoch 27）です。フォーマットフリーズ宣言までは現在のバージョンだけを受理し、v31 以前を含む非 v32 形式の migration reader は提供しません。v32 の圧縮方式は code 0 のみです。
