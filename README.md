...
- アプリケーションのバージョンは 0.2.6、Rust／C++ 間の公開 ABI は v17 です。
- ネイティブ形式は `.inkpod` v27（replay epoch 24）、バッチ設定は `.inkbatch` v2 です。フォーマットフリーズ前のため、ネイティブ形式は現在のバージョンだけを受理します。
...
- レイヤー／プレーンの複数対象編集、変換、統合、主線保護を利用できます。
...
- `.inkpod` v27 形式に加え、PNG、TIFF、TGA、BMP の読み込みと書き出しに対応しています。
...
**レイヤー**は、セル画を重ね合わせる大きな単位です。レイヤー単位で順序、表示、編集可否、不透明度などを管理し、2 値彩色、階調彩色、汎用ラスター、選択、調整など、種類ごとの役割を持ちます。レイヤーの種類によって、内部に置けるプレーンの構成が決まります。
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.6 -DryRun
```
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.6 -Publish
```
...
- 現在のネイティブ保存形式は `.inkpod` v27（replay epoch 24）です。フォーマットフリーズ宣言までは現在のバージョンだけを受理し、v26 以前を含む非 v27 形式の migration reader は提供しません。v27 の圧縮方式は code 0 のみです。
