...
主要な制作ワークフローは UI から Core まで実装・検証されています。互換性一覧で追跡している要件のうち、現在 `In progress` なのは Windows とワークスペースのアクセシビリティに関する 2 項目で、その他は `Verified` です。正確な実装範囲、代表検証、既知の差分については、[実装状況](docs/implementation-status.md)と[互換性一覧](docs/compatibility.md)を参照してください。

- [https://shuichi.github.io/inkpod/](https://shuichi.github.io/inkpod/)

## 現在の実装状況

- アプリケーションのバージョンは 0.2.4、Rust／C++ 間の公開 ABI は v13 です。
- ネイティブ形式は `.inkpod` v26（replay epoch 23）、バッチ設定は `.inkbatch` v2 です。フォーマットフリーズ前のため、ネイティブ形式は現在のバージョンだけを受理します。
...
- UI は日本語／英語に対応しています。`編集 > 設定 > 言語` で明示選択でき、`システム設定` では Windows の第1優先 UI 言語が日本語なら日本語、それ以外なら英語で起動します。
- 最新の自動検証結果と未検証のプラットフォーム項目は、[実装状況の「Latest representative verification」](docs/implementation-status.md#latest-representative-verification)に集約しています。
...
- レイヤー／プレーンの複数対象編集、変換、統合、主線保護と、再編集可能なテキスト／指示注釈を利用できます。
- カットとセルシーケンス、前後セル移動の Stop／Wrap、ライトテーブル、参照画像、モーションチェックを備えています。
- 角度付き撮影フレーム、複数の消失点、放射線ガイドへのスナップ、通常画像とは独立した指示画像の書き出しに対応しています。
- フィルター、色調整、グラデーション、調整レイヤー、出力色チェック、バッチ処理を利用できます。
- 元に戻す／やり直す、非累積プレビューの適用／取消、自動保存と復元、履歴の可視化、明示的な履歴圧縮コピーに対応しています。
- `.inkpod` v26 形式に加え、PNG、TIFF、TGA、BMP の読み込みと書き出しに対応しています。
...
```text
build/windows-x64-release/package/Inkpod-0.2.4-windows-x64.zip
build/windows-arm-release/package/Inkpod-0.2.4-windows-arm.zip
```
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.4 -DryRun
```
...
```powershell
.\scripts\publish-windows-release.ps1 -Version 0.2.4 -Publish
```
...
- 現在のネイティブ保存形式は `.inkpod` v26（replay epoch 23）です。フォーマットフリーズ宣言までは現在のバージョンだけを受理し、v25 以前を含む非 v26 形式の migration reader は提供しません。v26 の圧縮方式は code 0 のみです。
- バッチ処理の出力形式は、現在 `.inkpod` のみです。
