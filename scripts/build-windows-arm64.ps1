...
function Get-HighestCachedBuildNumber {
    ...
    $highest = 0
    $arm64BuildDirectories = @("windows-arm-debug", "windows-arm-release")
    $cacheFiles = Get-ChildItem -LiteralPath $buildRoot -Filter "CMakeCache.txt" `
        -Recurse -File -ErrorAction SilentlyContinue
    foreach ($cacheFile in $cacheFiles) {
        if ($cacheFile.Directory.Name -notin $arm64BuildDirectories) {
            continue
        }
...
function Resolve-VsDevCmd {
    if ($env:VSCMD_ARG_TGT_ARCH -eq "arm64" -and
    ...
    if ($vswhereExitCode -ne 0 -or
        ...
        throw "No Visual Studio installation with the ARM64 MSVC toolchain was found."
    }
...
function Get-MsvcHostArchitecture {
    $processArchitecture = [string] $env:PROCESSOR_ARCHITECTURE
    switch ($processArchitecture.ToUpperInvariant()) {
        "AMD64" { return "x64" }
        "ARM64" { return "arm64" }
        default {
            throw "Unsupported MSVC host process architecture: '$processArchitecture'. Expected AMD64 or ARM64."
        }
    }
}
...
$msvcHostArchitecture = Get-MsvcHostArchitecture
...
try {
    ...
    foreach ($selectedConfiguration in $selectedConfigurations) {
        $preset = "windows-arm-$($selectedConfiguration.ToLowerInvariant())"
        Invoke-BuildTool `
    ...
    Write-Host "Completed Inkpod ARM64 build $nextBuildNumber."
} finally {
