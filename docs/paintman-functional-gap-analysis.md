...
- **不足層／カバレッジ:** 旧`VANISHING-POINT-001`のstable-ID object、canonical CRUD／preview、snapshot、ABI v17、Windows routeの証跡は履歴として保持する。現行v32／epoch27／ABI25では旧codeをtombstoneとして拒否する。
...
| VanishingPoint の内容 | — | Not required | 旧stable-ID／preview／snapshot／snap／Windows routeのverificationは履歴。現行v32／epoch27／ABI25ではdata／API／UIから削除し、手動guideまたは外部作図で代替 |
