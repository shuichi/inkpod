...
- `resource.h` defines 330 unique `IDM_*` values.
- `app.rc` references 329 unique production command IDs through 336 actionable
...
`apps/windows/ui/command_state_catalog.inc` independently assigns the same 329
...
- Layer and Plane commands are nested under Cell; shortcut settings are under
  Edit > Settings.
- All 329 production commands have a command-unique, prefix-free sequence of one
