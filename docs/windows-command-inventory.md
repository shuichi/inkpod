...
- `resource.h` defines 335 unique `IDM_*` values.
- `app.rc` references 332 unique production command IDs through 339 actionable
...
`apps/windows/ui/command_state_catalog.inc` independently assigns the same 332
...
- Layer and Plane commands are nested under Cell; shortcut settings are under
  Edit > Settings.
- All 335 production commands have a command-unique, prefix-free sequence of one
