...
- `resource.h` defines 388 unique `IDM_*` values.
- `app.rc` references 387 unique production command IDs through 394 actionable
...
`apps/windows/ui/command_state_catalog.inc` independently assigns the same 387
...
- All 387 production commands have a command-unique, prefix-free sequence of one
...
| 41900-41999 | Window, pane-target, and Batch shell | 62 |
