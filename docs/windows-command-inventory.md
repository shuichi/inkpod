...
- `resource.h` defines 331 unique `IDM_*` values.
- `app.rc` references 330 unique production command IDs through 337 actionable
...
`apps/windows/ui/command_state_catalog.inc` independently assigns the same 330
...
- Layer and Plane commands are nested under Cell; shortcut settings are under
  Edit > Settings.
- All 330 production commands have a command-unique, prefix-free sequence of one
