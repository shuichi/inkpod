...
| `inkpod-format` | Bounded procedure-authoritative `.inkpod` v15 DTO/container and `.inkbatch` v2 models, streaming encode/decode/validation, atomic file I/O, and PNG/TIFF/TGA/BMP codecs                              |
...
and there is no supported incomplete-journal state. The v15 writer serializes
...
replay contract is epoch 12 and native format version 15 for canonical fixed-
...
`AssetRecord`, payload, or `TileRaster` ownership. Production v15 persists the
...
The current `.inkpod` v15 container requires `META`, `GENS`, `ASST`, `PROC`, and
...
Partial selection revert reconstructs the saved document through this same v15
