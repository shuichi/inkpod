...
- ABI v12 で既知の構造体末尾まで読み書きできるサイズが必要である。
...
現行ライブラリは ABI v12 だけを受理し、`InkpodCoreConfig::abi_version` が完全一致しなければ
...
現行の v12 ヘッダーへ更新して再ビルドする。

ABI v12 は `_v3` の値／ID 専用プリミティブ制御 API と永続化 API を保持し、
sequence source identity、Rust-owned二セルpair preview、その bounded candidate照会、
読込済みBatch graphのoperation照会とimmutable run-copy作成を保持し、annotation edit／stroke／snapshot API を追加した。
...
既存のラスタ文書オープン／インポート、クリップボード、ライトテーブル入力は、現行 v12 の操作別 API の
...
## Text／instruction annotation（現行 ABI v12）
...
- `inkpod_core_get_replay_contract` は、呼び出し側が所有する `InkpodReplayContract` へ値を書き込む。
  リプレイエポック 21、現行のプロシージャ／コンテナバージョン 24、正規数値バージョン 1、閉じた
...
製品の保存／オープン API は同じ v24 のリプレイ／カタログ契約を使い、現行でないネイティブ形式の
バージョンをすべて拒否する。
...
とする別の v24 ファイルを書き出すが、作業中のパス、リビジョン、未保存状態、保存点、ID、履歴は変更しない。
...
相対ファイル名、Cell ID、document UUID が一致するときだけ current v24 / Cut replay epoch 21 の
...
## ABI v12 の `_v3` 付き値／ID 制御 API

この節の `V3` / `_v3` は API 群とレコード名の一部である。すべての呼び出しは、全体として
ABI v12 に一致するヘッダーとライブラリの組み合わせで使用する。
...
## Batch行と二セルpair preview（ABI v12）
...
v5; the current library nevertheless requires ABI v12 because the selection
...
retention. The additive export and view commands retain ABI v12.
...
operate on the Core owner thread and reuse the existing ABI v12 records. The
...
segment flag bits remain invalid. These are additive ABI v12 exports and do not
...
## Light Table 前後 N セル一括登録（現行 ABI v12）
...
## EditorDefaults / EditorState（現行 ABI v12）
...
次の八つの Core 所有スレッド用 API と固定幅レコードは ABI v2 以降に追加され、現行 ABI v12 に保持されている。
...
ABI v12 の `InkpodSelectionInput` と `InkpodEditorSelectionOptions` は、range interpretation、
...
その入力が使用する各入れ子レコードの `struct_size` を、現行 ABI v12 ヘッダーにある完全な
...
## 正規 Genesis とアセット取り込み（現行 ABI v12）
...
| 現行 v24 の通常保存                                           | 不変                 | 置換成功時に文書／EditorState とも保存済み | 不変                    |
| 自動保存                                                      | 不変                 | 不変                              | 不変                              |
| 新規作成／インポート                                          | 新しい文書情報が正本 | 戻り情報が正本                    | 新しい Genesis／履歴              |
| v24 のオープン／復旧                                          | 実行時リビジョンを付け直す | 戻り情報が正本               | ファイルの全ジャーナル／履歴を復元 |
...
通常保存では、v24 の必須セクション `META` / `GENS` / `ASST` / `PROC` / `EDIT`、保持対象の不透明な任意
...
変えない。通常の v24 オープンでは、Genesis、アセット、プロシージャジャーナル、カーソル／分岐、すべての
