...
- ABI v13 で既知の構造体末尾まで読み書きできるサイズが必要である。
...
現行ライブラリは ABI v13 だけを受理し、`InkpodCoreConfig::abi_version` が完全一致しなければ
...
ABI v13 は `_v3` の値／ID 専用プリミティブ制御 API と永続化 API を保持し、
...
## Text／instruction annotation（現行 ABI v13）
...
現行 ABI v13 は、Core 所有スレッド専用の永続化操作を三つ提供する。`inkpod_core_get_persistence_info` は、
...
対話 UI は ABI v13 の分割 API を使う。`inkpod_core_history_visualization_builder_begin` は ready task と
...
## 新規 Cell creation plan（現行 ABI v13）
...
## ABI v13 の `_v3` 付き値／ID 制御 API

この節の `V3` / `_v3` は API 群とレコード名の一部である。すべての呼び出しは、全体として
ABI v13 に一致するヘッダーとライブラリの組み合わせで使用する。
...
## Batch行と二セルpair preview（ABI v13）
...
v5; the current library nevertheless requires ABI v13 because the selection
...
retention. The additive export and view commands retain ABI v13.
...
operate on the Core owner thread and reuse the existing ABI v13 records. The
...
segment flag bits remain invalid. These are additive ABI v13 exports and do not
...
通常の前後セル切替は ABI v13 の additive な二段階 API を使う。
...
## Light Table 前後 N セル一括登録（現行 ABI v13）
...
## EditorDefaults / EditorState（現行 ABI v13）
...
次の八つの Core 所有スレッド用 API と固定幅レコードは ABI v2 以降に追加され、現行 ABI v13 に保持されている。
...
ABI v13 の `InkpodSelectionInput` と `InkpodEditorSelectionOptions` は、range interpretation、
...
その入力が使用する各入れ子レコードの `struct_size` を、現行 ABI v13 ヘッダーにある完全な
...
## 正規 Genesis とアセット取り込み（現行 ABI v13）
