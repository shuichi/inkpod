...
- ABI v11 で既知の構造体末尾まで読み書きできるサイズが必要である。
...
現行ライブラリは ABI v11 だけを受理し、`InkpodCoreConfig::abi_version` が完全一致しなければ
...
現行の v11 ヘッダーへ更新して再ビルドする。

ABI v11 は `_v3` の値／ID 専用プリミティブ制御 API と永続化 API を保持し、
...
- `inkpod_core_get_replay_contract` は、呼び出し側が所有する `InkpodReplayContract` へ値を書き込む。
  リプレイエポック 19、現行のプロシージャ／コンテナバージョン 22、正規数値バージョン 1、閉じた
...
## 新規 Cell creation plan（現行 ABI v11）
...
## ABI v11 の `_v3` 付き値／ID 制御 API

この節の `V3` / `_v3` は API 群とレコード名の一部である。すべての呼び出しは、全体として
ABI v11 に一致するヘッダーとライブラリの組み合わせで使用する。
...
## Batch行と二セルpair preview（ABI v11）
...
v5; the current library nevertheless requires ABI v11 because the selection
...
retention. The additive export and view commands retain ABI v11.
...
operate on the Core owner thread and reuse the existing ABI v11 records. The
...
segment flag bits remain invalid. These are additive ABI v11 exports and do not
...
## Light Table 前後 N セル一括登録（現行 ABI v11）
...
## EditorDefaults / EditorState（現行 ABI v11）
...
次の八つの Core 所有スレッド用 API と固定幅レコードは ABI v2 以降に追加され、現行 ABI v11 に保持されている。
...
ABI v11 の `InkpodSelectionInput` と `InkpodEditorSelectionOptions` は、range interpretation、
...
その入力が使用する各入れ子レコードの `struct_size` を、現行 ABI v11 ヘッダーにある完全な
...
## 正規 Genesis とアセット取り込み（現行 ABI v11）
