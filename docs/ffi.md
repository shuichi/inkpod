...
- ABI v34 で既知の構造体末尾まで読み書きできるサイズが必要である。
...
現行ライブラリは ABI v34 だけを受理し、`InkpodCoreConfig::abi_version` が完全一致しなければ
`INKPOD_STATUS_INCOMPATIBLE_ABI` を返す。関数名や型名の `_v3` は、値／ID API 群が導入された世代を
示す接尾辞であり、ABI v3 の呼び出し側との実行時互換性を意味しない。ABI v1-v33 の呼び出し側は、
現行の v34 ヘッダーへ更新して再ビルドする。
...
の clean repair-needed snapshot なら clean／non-recovered とする。ABI v34 が保持する recovery pair proof は `NONE`、両 member の完全 stamp を
...
ABI v34 が保持する `InkpodIoCacheInfo::sequence_render_allocations` と `sequence_render_bytes` は、同じ manager に
...
全InkScript recordは`struct_size`と`version == INKPOD_INKSCRIPT_RECORD_VERSION`を持つ。ABI v34のRust unit contract、
...
## InkScript execution／report（現行 ABI v34）

ABI v34は、M11／M12から維持している単一のRust planner／runnerへ、authority-free `PathIntent`、検証済み
...
現行 ABI v34 は、Core 所有スレッド専用の永続化操作を三つ提供する。`inkpod_core_get_persistence_info` は、
...
対話 UI は ABI v34 の分割 API を使う。`inkpod_core_history_visualization_builder_begin` は ready task と
...
## 新規 Cell creation plan（現行 ABI v34）
...
## ABI v34 の `_v3` 付き値／ID 制御 API

この節の `V3` / `_v3` は API 群とレコード名の一部である。すべての呼び出しは、全体として
ABI v34 に一致するヘッダーとライブラリの組み合わせで使用する。
...
## Batch v5 graph／staged result／二セルpair preview（ABI v34）

ABI v34 の public authoring catalog は `COLOR_REPLACE`、
...
v5; the current library nevertheless requires the exact ABI v34 header,
...
operate on the Core owner thread and reuse the existing ABI v34 records. The
...
Rust-owned snapshot lifetime. These additive exports are retained in ABI v34 and do not
...
ABI v34 の sequence import job は、共通 companion resolver で検証した最大 64 セルの complete editable target を
...
通常の前後セル切替は ABI v34 の additive な二段階 API を使う。
...
## Light Table 前後 N セル一括登録（現行 ABI v34）
...
## EditorDefaults / EditorState（現行 ABI v34）
...
ABI v34 では current selection と saved-selection mask は layer／plane tree の外にある文書状態である。
...
次の八つの Core 所有スレッド用 API と固定幅レコードは ABI v2 以降に追加され、現行 ABI v34 に保持されている。
...
ABI v34 の `InkpodSelectionInput` と `InkpodEditorSelectionOptions` は、range interpretation、
...
その入力が使用する各入れ子レコードの `struct_size` を、現行 ABI v34 ヘッダーにある完全な
...
## 正規 Genesis とアセット取り込み（現行 ABI v34）
