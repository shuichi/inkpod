...
`InkpodLineCorrectionInput`は、編集可能なMainLine/Color/Rasterのstable ID、
期待document revision、任意の操作領域、ブラシ設定、背景判定と補正モードを固定する。
`inkpod_core_line_correct`は一つのcanonical transactionとして実行する。
`inkpod_core_line_preview_begin`は一時previewだけを公開し、既存のfilter-preview
apply/cancelで一Undo単位の確定または破棄を行う。

全recordとsample spanは、呼び出し中に有効・整列済み・非重複であること。
Core owner threadで呼び出し、READY taskは復帰まで保持する。Rustは呼出側pointerを保持しない。
taskのquery/cancel/releaseは既存契約を使う。成功時だけoutputを書き、no-opはdocument
revision/historyを進めない。invalid/cancel/stale/割当・処理上限の失敗は部分編集を公開しない。

COLOR背景はnative RGBA16値で指定する。8bit planeでは各channelを257の倍数とし、
暗黙の量子化をしない。scalarはcoverage=0が背景になる。既定値はcanonical記録前に
MainLineでは白＋透明、それ以外では透明へ解決する。sampleの座標単位と径の単位は独立で、
`coordinate_space`はsample、`screen_size`とgesture開始時の`view_zoom_q16`は径に適用する。
CoreではOS DPIを適用しない。操作の境界・サイズ単位はSPECのPAINT-003/SEL-001/SEL-004と
公開headerを参照する。
...
通常の文書 command／render 経路で Core は C++ コールバックを呼ばない。InkScript の既存 callback adapter は
Core engine thread 上の `PlanTask`／`RunTask` advance 中だけ呼ばれ、Core lockを保持せず、同じ Coreへ
reenterしてはならない。Windows private engine の file I/O は ABI v35 の共有 Rust adapter を使い、
identity／lock／atomic install を C++ callback で実装しない。UI スレッドは Core やレンダラーの完了を同期的に待たず、レンダラーは古い未描画
...
Rust 側の公開 record は `#[repr(C)]` と固定幅整数で宣言し、内部 domain model と分離する。
高頻度入力は bounded batch/span/snapshot 単位で渡し、sample や pixel ごとの FFI 往復にしない。
確保した側が対応する release API で解放し、panic／C++ exception を ABI 境界で捕捉する。
...
- ABI v35 で既知の構造体末尾まで読み書きできるサイズが必要である。
...
現行ライブラリは ABI v35 だけを受理し、`InkpodCoreConfig::abi_version` が完全一致しなければ
`INKPOD_STATUS_INCOMPATIBLE_ABI` を返す。関数名や型名の `_v3` は、値／ID API 群が導入された世代を
示す接尾辞であり、ABI v3 の呼び出し側との実行時互換性を意味しない。ABI v1-v34 の呼び出し側は、
現行の v35 ヘッダーへ更新して再ビルドする。

実セル切替では、最後に保存・exact recovery で表現した runtime baseline より journal／Redo branch／
editor generation が新しければ、clean savepoint でも fresh source recovery を要求する。
`INKPOD_IO_REVERT_CURRENT` は `INKPOD_IO_OPEN_NATIVE` と `INKPOD_IO_FORCE_RELOAD` の組合せでだけ有効。
発行対象の native path／UUID を検査し、runtime sequence と全 live view の ID／論理 state を保持する。
検証済み target cache、inactive sequence target の COW resident transfer、render preparation の
所有権・上限・release は下記の各 API 契約に従う。Cut records と指示画像 export flags は存在しない。
`InkpodResourceUsage` は 136 bytes、`InkpodIoCacheInfo` は 120 bytes の現行 layout を保持する。
新規の `InkpodIoSequenceResidentInfo` は 72 bytes、`InkpodSequenceRenderPreparationInfo` は 24 bytes、`InkpodSnapshotSequenceSourceView` は 64 bytes である。旧サイズの構造体を渡してはならない。native v34、document replay epoch 29 と ABI v35 は完全一致が必要である。
...
Batch contact-sheet preview APIも保持する。
`InkpodHistoryItem` は表示用UTF-8名ではなく固定幅の `InkpodHistoryEntryKind` を返す。
...
vectorおよびText／Annotationのenum値、record、edit、snapshot、clipboard、diagnostic exportは
公開境界に存在しない。旧enum値、旧primitive ID、旧record size、非現行ABI configは拒否する。
...
の clean repair-needed snapshot なら clean／non-recovered とする。ABI v35 が保持する recovery pair proof は `NONE`、両 member の完全 stamp を
...
Genesis/assets/composite query を使うため、pair authority 自体は native v34、replay epoch 29、既存 record layout を
...
ABI v35 が保持する `InkpodIoCacheInfo::sequence_render_allocations` と `sequence_render_bytes` は、同じ manager に
...
## InkScript source／compiler／fragment（現行 ABI v35）

ABI v15 で追加されたsource parse、diagnostic copy、static compile、journal fragment exportは、exact-current
InkScript file v3／procedure catalog v8／replay epoch 29としてABI v35に保持される。`.inkscript` file filterと
製品Windows command／UIは未接続であり、既存のprivate Windows authority経路とは区別する。
M5は共有Rust I/O／画像preview／active・新規tab結果をABI v35から接続する。
callback-only adapterは引き続きstaged出力のpreflightを拒否する。shared adapterのstaged結果は
`STAGED`として報告し、fileの`INSTALLED`、書込みを伴わない`DRY_RUN`とは区別する。
...
全InkScript recordは`struct_size`と`version == INKPOD_INKSCRIPT_RECORD_VERSION`を持つ。ABI v35のRust unit contract、
...
## InkScript execution／report（現行 ABI v35）

ABI v35は、M11／M12から維持している単一のRust planner／runnerへ、authority-free `PathIntent`、検証済み
...
run itemはstaged Coreでdecode／cache-free replay／canonical execution／current-version encodeを完了し、temporary fileをwrite／
...
### 共有 Rust I/O と staged publication（M5）

`inkpod_core_inkscript_io_create`は、intent IDごとの承認済みabsolute UTF-8 pathをbounded spanからcopyし、
共有`InkpodIoManager`のservice ownerを保持する。NULL managerなら同じRust managerの既定instanceを所有する。
pathをsourceの文字列やcwdから自動解決しない。`io_capture_session`は指定したlive Coreの不変snapshotをcaptureし、
session ID／generation／source generation、label、任意backing pathを固定する。
`INKPOD_INKSCRIPT_SESSION_USE_CORE_BACKING`ではbacking spanを空にし、Coreが所有するnative／raster pairのpathを使う。
backingがあればlabel／display_numberも同pathから導出し、callerの値はpathless sessionのfallbackだけに使う。
pairの他方も同じsnapshotのaliasとして扱い、canonical raster入力は未保存内容を保持する。
batch入力はdiskを読み、planned／repair-needed pairの欠落memberも出力先として予約する。
`io_capture_sequence`はsession IDまたは承認済みabsolute file pathの順序をcopyし、実際の読込みをPlanTaskのadvanceへ遅延する。
path／memberの件数、stride、合計UTF-8 bytesを検査し、空record spanはNULL／count 0／stride 0とする。
全操作は作成Coreのowner threadで行う。
`io_invalidate`は明示的なauthority取消またはsession失効を全cloneへ通知する。通常のlive編集はplan snapshotを
最新内容へ置き換えず、active出力だけが公開直前に完全なdocument/editor/savepointを再検査する。
`io_validate_session`はcapture元のlive CoreについてUUIDとCore-owned保存先authorityを照合し、
Save As／文書置換／保存先変更で不一致ならsessionを失効させる。通常のdocument/editor revisionやhistory変更は比較しない。

`inkpod_core_inkscript_shared_plan_task_create`は既存PlanTaskへ共有adapterを渡す。以降のplan query／copy／take／
confirmationは既存の境界を使う。成功したRunTask作成だけがplanとconfirmationを消費し、adapterをrunへ移す。
共有経路ではhost callbackを同時指定できない。file identity、排他、temporary、保存直前fingerprintとatomic renameは
`inkpod-io`の同じbackendへ委譲する。外部assetも同managerで上限付き読込みとidentity照合を行い、既存asset ingesterへ渡す。
対応済みfilesystemでのaccess／sharingによるrename拒否はSave失敗とし、非対応のatomic operationと区別する。
いずれもguardを外す再試行や弱いrenameへのfallbackを行わない。

`INKPOD_INKSCRIPT_RUN_IMAGE_PREVIEW`は共有経路専用で、計画preview／dry-runと別の画像結果を返す。
queryは実行中のatomic progressを読み、cancelは読込み・処理中にも通知できる。全入力隔離、選択codecでの保存・再読込、
4 GiB／16,777,216 pixels上限、cleanup-before-publicationはCoreの契約をそのまま使う。
画像previewがcancel／resource／I/O／cleanup／staleで失敗した場合はterminal eventだけを返し、
reportとstaged resultを作らない。その場合の`run_task_take_report`は`INVALID_STATE`となる。

完了後の`run_task_result_count`と`run_task_take_staged_result`はCore owner threadでだけ呼ぶ。
slot indexと入力ordinalを区別し、take後も他slotを改番しない。`InkpodInkScriptStagedInfo`はkindと元sessionの固定幅DTOであり、
Core pointerをUIへ公開しない。active結果の`staged_result_apply_active`は元contextへ一Undoで適用し、両savepointとpath/viewを保持する。
引数・kind・authority検証失敗ではownerを保持し、Coreへのapply開始後はsuccess／no-op／cancel／staleともownerを消費する。
new-tab／画像previewだけが`staged_result_take_core`で新しいCore ownerへ一回移管できる。
取得後も元taskのcancel tokenを共有し、active／previewはpublicationまで取消を観測する。
new-tabの先行成功結果は後続itemの取消後も取得できる。authority／session失効では別sessionへ公開せず、
未公開ownerを保持して明示releaseできる。UIでのquery／cancelとowner threadでのreleaseは呼出側が寿命を同期する。
未取得結果はtask release、取得後の未公開結果は`staged_result_release`で破棄する。reportは結果所有権と独立したimmutable DTOのままである。
Windows側のfragment／report copyはallocation例外でも局所ownerを解放し、計画中・確認待ちでtaskを破棄するときも
残るsource／plan／confirmation／taskをowner threadでreleaseする。previewのCore取得後も、予約sessionへの最終移管と
UI cancelを同じ短いlock区間で順序づける。Core呼出し・allocation・破棄はその区間へ持ち込まない。

## 角度付き撮影 frame（現行 ABI v35）
...
製品の保存／オープン API は現行のリプレイ／カタログ契約を使い、現行でないネイティブ形式の
バージョンをすべて拒否する。

現行 ABI v35 は、Core 所有スレッド専用の永続化操作を三つ提供する。`inkpod_core_get_persistence_info` は、
...
とする別の現行形式ファイルを書き出すが、作業中のパス、リビジョン、未保存状態、保存点、ID、履歴は変更しない。
...
対話 UI は ABI v35 の分割 API を使う。`inkpod_core_history_visualization_builder_begin` は ready task と
...
## 新規 Cell creation plan（現行 ABI v35）
...
## ABI v35 の `_v3` 付き値／ID 制御 API

この節の `V3` / `_v3` は API 群とレコード名の一部である。すべての呼び出しは、全体として
ABI v35 に一致するヘッダーとライブラリの組み合わせで使用する。
...
## Batch v5 graph／staged result／二セルpair preview（ABI v35）

ABI v35 の public authoring catalog は `COLOR_REPLACE`、
...
v5; the current library nevertheless requires the exact ABI v35 header,
...
operate on the Core owner thread and reuse the existing ABI v35 records. The
...
Rust-owned snapshot lifetime. These additive exports are retained in ABI v35 and do not
...
ABI v35 の sequence import job は、共通 companion resolver で検証した最大 64 セルの complete editable target を
...
通常の前後セル切替は ABI v35 の additive な二段階 API を使う。
...
## Light Table 前後 N セル一括登録（現行 ABI v35）
...
## EditorDefaults / EditorState（現行 ABI v35）
...
ABI v35 では current selection と saved-selection mask は layer／plane tree の外にある文書状態である。
...
次の八つの Core 所有スレッド用 API と固定幅レコードは ABI v2 以降に追加され、現行 ABI v35 に保持されている。
...
ABI v35 の `InkpodSelectionInput` と `InkpodEditorSelectionOptions` は、range interpretation、
...
その入力が使用する各入れ子レコードの `struct_size` を、現行 ABI v35 ヘッダーにある完全な
...
## 正規 Genesis とアセット取り込み（現行 ABI v35）
...
| 現行形式の通常保存                                           | 不変                 | 置換成功時に文書／EditorState とも保存済み | 不変                    |
| 自動保存                                                      | 不変                 | 不変                              | 不変                              |
| 新規作成／インポート                                          | 新しい文書情報が正本 | 戻り情報が正本                    | 新しい Genesis／履歴              |
| 現行形式のオープン／復旧                                          | 実行時リビジョンを付け直す | 戻り情報が正本               | ファイルの全ジャーナル／履歴を復元 |
...
通常保存では、現行形式の必須セクション `META` / `GENS` / `ASST` / `PROC` / `EDIT`、保持対象の不透明な任意
...
変えない。通常の現行形式オープンでは、Genesis、アセット、プロシージャジャーナル、カーソル／分岐、すべての
