...
| `ABI-001`, `ABI-002` | Verified | Header/export/catalog parity for 218 ABI v6 exports; C11/C++20 layout, NULL/short/stale/wrong-thread/ownership tests; edit-target count/stride/capability/output spans; owned Cell plan and ordered render-plan spans; snapshot queue, stale, device-loss and GUI smoke | ABI v1-v5 callers must rebuild against the current header |
| `IO-001` | Verified | V11/epoch-8 stream/layout/digest/current-only tests; EDIT schema-2 multi-target and Cell/frame round-trip; checkpoint equivalence/fallback/rejection; staged save/open, recovery, failure/cancel, compaction, ABI and Windows smoke | Non-v11 files are rejected without migration; compression code 0 only |
...
| `WORKSPACE-001` | In progress | All eight modeless surfaces hosted by DockHost; dock/tab/float/hide/AutoHide policy, five presets, V5 round-trip and V4 migration, transient non-persisted Job Progress, malformed HKCU recovery, monitor/DPI clamp, pointer/accessibility and native workspace smoke | Reference Check AutoHide edge buttons are not keyboard-reachable with F6/Tab |
...
| `SAFE-001` | Verified | Corruption/mutation corpus, bounded V11/checkpoint/asset/edit-target rejection, staged publication, failure atomicity, fuzz-target declarations | Coverage-guided fuzz execution is not part of the latest local run |
...
| `DOC-002`, `DOC-003` | Experimental | Stable-ID typed topology; tree-ordered bounded multi-target EditorState; grouped duplicate/delete/visibility/editability/compatible merge/convert capability matrix; no-op/invalid/stale/Undo/replay/save-reopen; ABI spans; Windows marker, accessibility, status, menu and production smoke | x64 Release grouped main-line/color paste, one-step Undo and active-plane-only brush manual check pending |
