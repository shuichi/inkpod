...
| `IO-001` | Verified | V16/runtime epoch-13 stream/layout/digest/current-only tests; v15 rejection; explicit vector topology; EDIT schema-4 brush/selection/multi-target and Cell/frame round-trip; scoped replacement and typed separation replay; checkpoint equivalence/fallback/rejection; staged save/open, recovery, failure/cancel, compaction, ABI and Windows smoke | Non-v16 files are rejected without migration; compression code 0 only |
...
| `VIEW-005` | Experimental | Per-view typed AA/centerline/endpoint flags; explicit stable endpoint topology; exact/near/closed/connected Core contracts; v16/epoch-13 save/reopen and v15 rejection; bounded borrowed ABI records; Windows menu/shortcut/checked state, Direct2D pixel golden and device-loss smoke | x64 Release user-visible manual confirmation is pending |
