...
| ABI-001 | Verified | ABI v1 retains M0-M6 records and exposes copied, bounded M7 graph/operation records plus immutable graph/preview/report and atomic task handles | Header/export parity and direct contract-test coverage for all 155 functions; C11/C++20 layouts; post-M8 integrated ABI/application smoke; Rust invalid-size/stride, buffer, ownership, task, transaction, and round-trip tests | Nested records are validated before reference creation; preview/report strings are borrowed from their owning handle; graph creation copies every caller span |
