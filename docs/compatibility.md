...
| `ABI-001`, `ABI-002` | Verified | Header/export/catalog parity for 216 ABI v6 exports; C11/C++20 layout, NULL/short/stale/wrong-thread/ownership tests; owned Cell plan and ordered render-plan spans; snapshot queue, stale, device-loss and GUI smoke | ABI v1-v5 callers must rebuild against the current header |
| `IO-001` | Verified | V10 stream/layout/digest/current-only tests; six-frame/Cell-ID round-trip; checkpoint equivalence/fallback/rejection; staged save/open, recovery, failure/cancel, compaction, ABI and Windows smoke | Non-v10 files are rejected without migration; compression code 0 only |
...
| `SAFE-001` | Verified | Corruption/mutation corpus, bounded V10/checkpoint/asset rejection, staged publication, failure atomicity, fuzz-target compile | Coverage-guided fuzz execution is not part of the latest local run |
...
| `CELL-001` | Experimental | Public image/frame-mode table, every initial layer and RGBA8/16 save/reopen, invalid/overflow/UUID failure atomicity; owned-plan ABI negatives; one-dialog Core-plan summary; three-session Windows production smoke | x64 Release manual image/frame-mode, 16-bit three-Cell, properties comparison, and Cancel check pending |
