...
| ABI-001 | Verified | ABI v2 uses semantic public names for all 155 functions and their records, including generic raster-source and atomic-task contracts; no temporary phase-name aliases remain | Header/export parity, direct contract-test coverage for all functions, repository-wide semantic-name architecture guard, C11/C++20 layouts, integrated ABI/application smoke, and Rust invalid-size/stride, buffer, ownership, task, transaction, and round-trip tests | v1 callers must update names and rebuild; numeric layouts, ownership, thread rules, and statuses are unchanged |
...
| VIEW-003 | In progress | Same-document tabs provide independent logical views; locator sampling remains in Core/C ABI after removal of the persistent main-window locator label | Core locator/multi-view, C ABI same-revision, and Windows tab smoke | A modeless floating locator palette must expose the retained result |
