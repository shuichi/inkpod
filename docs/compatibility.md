...
| `ABI-001`, `ABI-002` | Verified | Header/export/catalog parity for 235 ABI v9 exports; C11/C++20 layout, NULL/short/stale/wrong-thread/ownership tests; immutable sequence-switch requests and borrowed recovery paths; borrowed vector-diagnostic endpoints; owned two-cell pair preview and immutable per-run graph records; scoped color-replace, brush/selection option, edit-target, Cell plan and ordered render-plan spans; snapshot queue, stale, device-loss and GUI smoke | ABI v1-v8 callers must rebuild against the current header |
| `IO-001` | Verified | V17/runtime epoch-14 stream/layout/digest/current-only tests; v16 rejection; resolved geometry procedure v2 and explicit vector topology; EDIT schema-4 brush/selection/multi-target and Cell/frame round-trip; scoped replacement and typed separation replay; checkpoint equivalence/fallback/rejection; staged save/open, recovery, failure/cancel, compaction, ABI and Windows smoke | Non-v17 files are rejected without migration; compression code 0 only |
