...
| `ABI-001`, `ABI-002` | Verified | Header/export/catalog parity for 231 ABI v9 exports; C11/C++20 layout, NULL/short/stale/wrong-thread/ownership tests; owned two-cell pair preview and immutable per-run graph records; scoped color-replace, brush/selection option, edit-target, Cell plan and ordered render-plan spans; snapshot queue, stale, device-loss and GUI smoke | ABI v1-v8 callers must rebuild against the current header |
| `IO-001` | Verified | V15/runtime epoch-12 stream/layout/digest/current-only tests; v14 rejection; EDIT schema-4 brush/selection/multi-target and Cell/frame round-trip; scoped replacement and typed separation replay; checkpoint equivalence/fallback/rejection; staged save/open, recovery, failure/cancel, compaction, ABI and Windows smoke | Non-v15 files are rejected without migration; compression code 0 only |
...
| `BATCH-004` | Experimental | Bounded multi-row seed/pair authoring; exact native-depth two-cell extraction with explicit one-to-many resolution; typed replacement/selection/main-line/color/native-file separation; immutable per-run configuration; `.inkbatch` v2 and `.inkpod` v15/epoch-12 replay; FFI ownership/negative tests and Windows production smoke | x64 Release user-visible manual confirmation is pending |
