...
| `ABI-001`, `ABI-002` | Verified | Header/export/catalog parity for 232 ABI v9 exports; C11/C++20 layout, NULL/short/stale/wrong-thread/ownership tests; borrowed vector-diagnostic endpoints; owned two-cell pair preview and immutable per-run graph records; scoped color-replace, brush/selection option, edit-target, Cell plan and ordered render-plan spans; snapshot queue, stale, device-loss and GUI smoke | ABI v1-v8 callers must rebuild against the current header |
...
| `FILTER-PREVIEW-001` | Experimental | Same-base A→B Core acceptance; existing ABI v9 begin/update/apply/cancel ownership and negatives; 120ms dialog notification debounce; one-running/one-pending latest-wins queue with cooperative cancel and issue-time target generation; shared dialog/Job Progress reporting; Windows production smoke with four preview generations, Cancel, one Undo/Redo | M10 ARM64 user-visible confirmation is pending |
