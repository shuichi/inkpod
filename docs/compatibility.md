...
| `ABI-001`, `ABI-002` | Verified | Header/export/catalog parity for 220 ABI v8 exports; C11/C++20 layout, NULL/short/stale/wrong-thread/ownership tests; scoped color-replace and brush/selection option enum/range/reserved validation; edit-target, owned Cell plan and ordered render-plan spans; snapshot queue, stale, device-loss and GUI smoke | ABI v1-v7 callers must rebuild against the current header |
| `IO-001` | Verified | V14/runtime epoch-11 stream/layout/digest/current-only tests; v13 rejection; EDIT schema-4 brush/selection/multi-target and Cell/frame round-trip; scoped replacement replay; checkpoint equivalence/fallback/rejection; staged save/open, recovery, failure/cancel, compaction, ABI and Windows smoke | Non-v14 files are rejected without migration; compression code 0 only |
...
| `COLOR-REPLACE-001` | Experimental | Exact native-depth raster and whole stable vector path/fill replacement; pen/rectangle/polyline/lasso and selection intersection; preview/Cancel, no-op/invalid/stale/overflow, Undo/Redo, replay/save-reopen; ABI negatives and Windows production gesture smoke | x64 Release manual behavior confirmation is pending |
