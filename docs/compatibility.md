...
| ABI-001 | Verified | ABI v2 uses semantic public names for all 159 functions and adds fixed-record shortcut sequence set/copy plus a pure resolver without changing the prior 155 exports | Header/export parity, direct contract-test coverage for all functions, semantic-name architecture guard, C11/C++20 layouts, integrated ABI/application smoke, and Rust invalid-size/stride, buffer, ownership, task, transaction, round-trip, and shortcut conflict tests | v1 callers must update names and rebuild; the four shortcut exports are additive to the existing v2 surface |
...
| DOC-002 | In progress | Stable-ID typed raster/vector-coloring layer/plane topology and active-node state remain connected through Core/C ABI and the Windows pane controller; persistent main-window panes were removed | Core/format topology/save-reopen, ABI node queries, and Windows model synchronization smoke | A modeless floating layer palette must re-expose selection |
| DOC-003 | In progress | Native menu commands retain create, duplicate, delete, reorder, show, edit, opacity, convert, merge, and hidden cleanup with transactional relationship preservation | Core raster/vector tree tests and Windows menu reorder smoke | Floating palette actions and direct item reordering remain to be implemented |
...
| COLOR-002 | In progress | Palette, chart, sequence subpalette, color-check models, menu commands, and number/Tab shortcuts remain connected after removal of persistent main-window color controls | Core palette/subpalette tests and Windows model/menu/shortcut/file smoke | Modeless floating palette/chart UI remains; legacy preset bytes remain `Unknown` |
...
shortcut/Canvas routing through the Windows adapter, C ABI, Core, and result display as
