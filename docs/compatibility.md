...
| VECTOR-002 | Verified | Native tools cover draw, partial/intersection/full erase, connect, four width modes, all selection modes, vector-to-new-raster-layer rasterize, and raster-to-vector conversion | Five M5 acceptance tests, bounds/all-mode/transactions, FFI conversion, and Windows RunM5 command/Canvas smoke | Rasterize preserves the source vector layer and commits one Undo unit |
