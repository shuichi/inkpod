...
mandatory, `DocumentStateDigest` is schema 12/domain 10, and the canonical
snapshot-composite is schema 5. The current image tree has one standard layer
structure: exactly one MainLine plane, exactly one Color plane, and zero or more
Raster planes. Current selection, ordered named saved-selection masks, and sparse
fill protection are document-owned data outside that tree. Retired layer-kind, selection-
plane, VanishingPoint, Adjustment, vector, Text/Annotation, primitive, object-kind,
and payload codes remain reserved tombstones and are never reused.
...
## Current Cut descriptor contract

A Cut is a separate current-only `.inkpod` descriptor identified by magic
`49 4E 4B 43 55 54 00 00` (`INKCUT\0\0`). It is not the native Cell container
described below. It owns production metadata, defaults for future Cell creation,
ordered membership, and its own metadata/defaults/membership history. Each member points to one
independently saved native Cell `.inkpod` in the descriptor's directory. Cell pixels,
layers, EditorState, document history, savepoint, and dirty state remain owned only by
that Cell file.

The Cut header is exactly 64 bytes:
...
| 24 | 8 | payload length |
| 32 | 32 | BLAKE3-256 derive-key digest of the payload, context `org.inkpod.cut-descriptor.v3` |

The little-endian payload starts with schema `u32 = 3` and Cut replay epoch
`u32 = 25`, followed in order by nonzero Cut ID, 16-byte Cut UUID, current and
...
Genesis membership, current membership, active history, and inactive history.
Savepoint zero means absent.
Metadata is five length-prefixed UTF-8 byte strings (work title, episode, scene,
Cut name, instruction) followed by positive duration frames. Defaults store sizing
mode and two dimensions, X/Y DPI in thousandths, margin, safe/maximum-close ratios,
frame anchor and exact Color-plane pixel format as fixed `u32` values. Layer kind
is not part of Cut defaults.

Each immutable member asset is `CellId u64`, 16-byte document UUID, and a
length-prefixed UTF-8 relative filename. Each ordered membership record is the same
stable identity plus a positive display number; order is the record sequence, and
neither order nor display number is identity or a filename. Core accepts exactly one `.inkpod` filename
component of at most 255 bytes; absolute paths, separators, drive prefixes,
traversal, self-reference, case-insensitive duplicate paths, duplicate persistent
identity pairs, and more than 64 members are invalid. A persistent member identity
is the exact `(CellId, document UUID)` pair: numeric Cell IDs may repeat across
independent Cell-document namespaces, but the complete pair may not repeat. Staged
open/save canonicalizes the descriptor
directory and every member, rejects symlink escape or a missing member, then opens
the member as an exact-current native Cell and compares both persistent identities.
No external path is stored in Cut history.

One history record contains nonzero Procedure, base-State, and committed-State IDs,
then before/after metadata, defaults, and ordered membership. History membership
contains only stable identity and display number; paths occur once in the immutable
member-asset table. Thus add/remove/move/renumber remains replayable while no
canonical procedure contains an external path. Active plus inactive history is bounded to
...
replacement and advances only the Cut savepoint after replacement succeeds.
Autosave/recovery uses the same format without adopting normal path authority or
savepoint. Cell files are never embedded or rewritten by Cut save. Changing Cut
defaults affects only later Cell creation; copying those defaults into a new Cell is
the explicit boundary between the two owners. Creation of several independent Cell
files and publication of the final Cut descriptor is intentionally not represented as
one false cross-file transaction: already completed Cell saves remain valid files if a
later descriptor operation fails, while no partial Cut state is published.

A sequence edit is an ordered, bounded list of insert, remove, move-before,
move-after, and range-renumber operations evaluated against one Cut base state.
Core validates the complete staged membership before publishing one Cut state and
one Cut-history record. Removing a member never deletes or renames its Cell file;
the immutable asset-table entry remains while Cut history can still restore that
identity. A frontend reference to a removed identity is reported as an orphan or
missing target and is never retargeted by order or display number. Cut save replaces
only the descriptor, so external file operations are not evidence of transaction
success.
...
- Canonical document geometry uses a signed `i64` with 16 fractional bits:
  ...
  primitives may impose tighter raster-image bounds.
...
| `0x0005_0001..0x0005_FFFF` | raster paint, fill, cleanup, filters, effects, alpha, and color transforms |
| `0x0006_0001..0x0006_FFFF` | selection and floating-selection commit |
| `0x0007_0001..0x0007_FFFF` | document transforms |
| `0x0008_0001..0x0008_FFFF` | retired built-in tombstones; never accepted or emitted |
...
`0x0003_0002/v1`, `ReplaceColorChart` is `0x0003_0003/v1`,
`ApplyRasterStroke` is `0x0005_0001/v3`, `ApplyGeometry` is
`0x0005_0003/v2`, and
`ImportRasterAsset` is `0x0009_0001/v1`. Typed invocation records use the
...
consumes a new ID.
`SelectOutputColorGuard/canonical-v1` is `0x0006_0006`, semantics revision 1,
work-formula ID `0x0006_0006`. Its fixed argument record contains the base
document revision, profile code 1 (`BT.709 conservative Y'CbCr guard`), and
selection operation 1/2/3/4 (`New`/`Add`/`Subtract`/`Intersect`). Replay scans
the committed visible straight-alpha composite in row-major order, skips alpha
zero, evaluates exact RGBA16 with the profile's rational half-up conversion,
and combines the resulting binary mask as one transaction. The profile is an
inkpod QA guard, not a standards-conformance claim or an automatic legalizer.
...
The document-owned saved-mask procedures are `SaveSelectionMask`
`0x0006_0012/v2`, `ApplySavedSelectionMask` `0x0006_0013/v2`,
`RenameSavedSelectionMask` `0x0006_0014/v2`, and `DeleteSavedSelectionMask`
`0x0006_0015/v2`. Each begins at semantics revision 1 and uses its own primitive
ID as work-formula ID. Save stores one bounded UTF-8 name and returns one newly
allocated `SavedSelectionId`; Apply stores an existing ID plus Replace/Add/
Subtract operation; Rename stores an existing ID plus its new bounded UTF-8
name; Delete stores an existing ID. The latter three name their ID as canonical
input role 1. None uses an asset or inline payload. IDs `0x0006_0010` and
`0x0006_0011`, formerly used for selection-layer conversion, remain retired and
...
| `ReplaceColorChart` | none | none | ordinal 1 = locked boolean; ordinal 2 = ordered entry sequence (`u32` count, then tagged exact-depth color and bounded UTF-8 name) | empty | 5 |
...
procedure is formed. Work formulas 1 through 5 are respectively constant 1,
`1 + palette entry count`, sample count plus every clipped dab-bounding-box
pixel tested, `1 + referenced raster logical element count`, and
`1 + Color chart entry count + total UTF-8 name bytes`. The metadata
...
Sequence, 7 Layer, 8 Plane, 9 Guide, 10 LightTableSet, 11 LightTableItem, and 12
is retired. Zero, retired codes 12 through 14, and all other unlisted codes are
...
angled shooting-frame, and saved-selection IDs from one non-reusing namespace.
...
`EditTargets/canonical-v2` uses `PrimitiveId 0x0002_0030`, semantics revision 1,
and work-formula ID `0x0002_0030`. Its canonical invocation starts with a nonzero
`u32` target count bounded to 4,096, followed in document-tree order by target
kind `u32` (1 Layer or 2 Plane), Layer ID `u64`, and for kind 2 Plane ID `u64`.
The command code is 1 Duplicate, 2 Delete, 3 SetVisibility plus canonical
boolean, 4 SetEditability plus canonical boolean, 5 ConvertPlanes plus only the
destination pixel-format code, 6 a permanently retired tombstone that is rejected,
or 7 Merge. Plane conversion preserves the plane role; layer-kind conversion no
longer exists.
Duplicate emits one output stable ID per target and Merge emits its one surviving
output; every other command emits none. Live commit, Undo/Redo, and replay use
the same executor, and invalid, stale, incompatible, overflow, or semantic no-op
input publishes no partial document state or stable ID.

`ApplyGeometry/canonical-v2` uses `PrimitiveId 0x0005_0003`, semantics
revision 1, and work-formula ID `0x0005_0003`. Its input-ID role 1 is the
nonzero target Plane ID. Its canonical bytes are, in order: that Plane ID;
primitive `u32` (1 Line, 2 Curve, 3 Rectangle, 4 Ellipse, 5 Polygon, 6
Polyline); flags `u32` (bit 0 outline, bit 1 fill, bit 2 closed); cross-section
`u32` (1 Round, 2 Square); positive Q16 outline width `i64`; tagged exact-depth
outline and fill colors; segment count `u32`; each segment as four signed-Q16
points followed by signed-Q16 start/end widths; fill-boundary count `u32`; and
that many signed-Q16 points. Segment count is bounded to 512 and fill-boundary
count to 8,192. Every other flag/code, a non-RGBA color, an invalid target,
noncontinuous or unclosed resolved geometry, and inconsistent fill data are
invalid.

The stored value is the Core-resolved geometry, not frontend clicks, device
coordinates, DPI, modifier keys, or a Windows command ID. Line and Curve are
open and reject fill; Rectangle, Ellipse, and Polygon are closed; Polyline is
closed only when requested. Raster MainLine permits outline only, raster
Color/Raster permits outline and closed fill. Raster fill samples pixel centers
with a deterministic half-open even/odd rule and is clipped by the committed
selection. A degenerate resolved segment sequence is a semantic no-op. Geometry
commits emit no output IDs. Preview bytes are never persisted. Live
...
`ReplaceColorChart/canonical-v1` has stable ID `0x0003_0003`, semantics
revision 1, and work-formula ID 5. Its arguments are a locked boolean and an
ordered bounded sequence of entries; each entry stores one tagged exact-depth
straight-alpha RGBA8/RGBA16 color followed by a bounded nonempty UTF-8 name.
It has no input/output stable IDs, assets, or inline payload. Live commit,
Undo/Redo, and replay use this same value and replace the complete document
Color chart in one transaction. The independent chart entries and lock are
part of document state; the current chart page/selected index remain
EditorState. Adding this document state, primitive, and cursor advances the
exact-current contract to top-level v19/runtime replay epoch 16, EDIT schema 5,
and `DocumentStateDigest` schema 6. v18/epoch 15 and every other noncurrent
input are rejected without migration.

Adding `SelectOutputColorGuard/canonical-v1` changes the replay result and
advances the exact-current contract to top-level v20/runtime replay epoch 17.
Version 19/epoch 16 and every other noncurrent input are rejected without
migration. The document and editor state schemas remain 6 and 5 respectively.

`CommitFloating/canonical-v3`, semantics revision 3, stores clipboard planes as one ordered collection.
Each plane records its typed plane kind, pixel format, document origin, bounded
sparse pixels, and no retired payload tail. Raster planes commit together as
one history entry; canonical bytes never retain a pointer or external path. The transform
suffix is anchor `u32` (1 TopLeft, 2 TopRight, 3 Center, 4 BottomLeft,
5 BottomRight), absolute target X/Y as signed canonical Q16, positive local X/Y
scales as signed canonical Q16, then clockwise rotation as unsigned full-turns.
Source bounds are half-open. Their five anchor points use the exact left/top/
right/bottom edges, scaling and rotation share that pivot, and the pivot is
finally placed at the absolute target. Raster replay transforms the four
half-open edges, floors minima and ceils maxima, inverse-maps destination pixel
centres, and floors the resulting source coordinate to select a half-open source
cell. This schema and
replay-result change advances the current native contract to v21 and runtime
replay epoch 18; exact v20/epoch-17 input is rejected without migration.
...
|---|---|
| `DocumentStateDigest` | `org.inkpod.digest.document-state.v10` |
| document metadata commitment | `org.inkpod.digest.document-metadata.v7` |
...
| canonical snapshot composite | `org.inkpod.digest.canonical-composite.v5` |
...
document-state frame nested within them use schema version 12. A raster-tile
...
The canonical snapshot-composite stream begins with schema `u32 = 5`, semantic
snapshot feature flags, document width/height, then the tile sequence in exact
snapshot storage order. Each tile contributes ID, origin, dimensions, stride,
byte count, and exact premultiplied BGRA8 bytes; cache/source revisions are
excluded. The stream then contains the bottom-to-top render-pass sequence. Each
pass contributes its closed kind code (1 layer-begin, 2 raster tiles, and 4
layer-end; code 3 is retired), layer ID, plane ID, opacity-milli, first-item
index, and item count. View revision, zoom, pan, flip, guides, grid, renderer
resources, and OS
...
| document metadata commitment | 1 document UUID; 2 stable Document ID; 3 paper; 4 frames/margins, including axis-aligned shooting and maximum-close; 5 base surface; 6 ordered standard layer/plane tree; 7 current-selection record; 8 ordered saved-selection frames containing stable ID, UTF-8 name, and raster commitment; 9 sparse fill-protection frame with its stable Plane ID and raster commitment; 10 palette; 11 Color chart frame (lock plus ordered exact-depth color/name entries); 12 main-line color; 13 guides; 14 grid; 15 Light Table; 16 project/cut/cell/frame/sequence identities and animation metadata, including the distinct stable Cell ID; 17 optional independent angled shooting-frame object; 18 required-extension sequence, empty in schema 12 |
...
`ScopedColorReplace/v2` has primitive ID `0x0005_0043`, semantics revision 1,
and work-formula ID `0x0005_0043`. Its canonical argument bytes are, in order:
nonzero Plane ID `u64`; mode `u32` (1 raster-color, 2 raster-main-line); exact native-depth
target and replacement colors; a canonical boolean indicating an optional
region; and, when present, one canonical selection shape restricted to trace,
...
therefore part of runtime replay epoch 13.
...
results, so the addition is part of runtime replay epoch 13 and top-level v16.
...
catalog v5 does not expose it as a command. The canonical value is a nonempty
...
schema 8, `GENS`, checkpoints, Undo/Redo, replay, snapshot revision/cache
invalidation, and `DocumentStateDigest` schema 12/domain 10.

`DocumentStateDigest` excludes document/editor revisions, history, paths, views,
transient sessions, allocation/tile-cache state, and caches. Its schema-12 root
...
- Layers and planes retain document stacking order. A layer frame orders stable
  ...
  exactly one Color, and zero or more Raster planes. A plane frame orders stable
  ID, role, pixel format, UTF-8 name, visible, editable, normalized `u16`
  opacity, and its raster. The closed editable plane-role codes are 1 MainLine,
  2 Color, and 3 Raster. Former layer-kind codes and editable Selection-plane
  code 4 are retired and invalid rather than compatibility aliases.
...
- The optional angled shooting-frame frame orders stable object ID, signed
  milli-pixel center X/Y, positive `u64` milli-pixel width/height, clockwise
  binary-turn rotation `u32`, five-point anchor code 1..5, Canvas-visible
  boolean, and instruction-export boolean. It is separate from the
  axis-aligned frame metadata used by paper fit and normal export.
- The layer/plane tree contains editable image planes only. The current
  selection record separately orders its selection Plane ID and one BinaryMask8
  raster; that ID may not occur in the editable tree. It is followed by an
  ordered saved-selection sequence. Each saved mask orders a nonzero stable
  `SavedSelectionId`, UTF-8 name, and BinaryMask8 raster. The fill-protection
  record separately orders its Plane ID and BinaryMask8 raster. Selection and
  protection bytes therefore occur exactly once and every mask change changes
  the document digest. All tree, selection, saved-selection, protection, and
  Document IDs are distinct members of the one document namespace.
- A color is tag `u8` 1 or 2 followed by RGBA8 bytes or four little-endian
  RGBA16 channels. Palette order is semantic and retained.
...
The canonical EditorState frame has schema 7 and exactly thirteen fields in
this order: 1 schema `u32 = 7`; 2 active `ToolId u32`; 3 optional last
...
optional active Layer ID; 10 optional active Plane ID; 11 optional
palette-cursor frame; 12 optional Color chart page/absolute-index cursor frame;
and 13 a required ordered edit-target sequence.
Fields 9 and 10 are either both present as nonzero IDs in the same document namespace or
...
Records are unique and ordered by the current document tree; selecting a layer
suppresses child-plane records for that layer. Decode rejects missing, deleted,
cross-layer, duplicate, out-of-order, or oversized targets rather than resolving
them to another active node.
...
FloatingTransform, 1007 LightTableMove, 1008 ColorReplace, 1009 ShootingFrame;
1011 GeometryLine, 1012 GeometryCurve, 1013 GeometryRectangle, 1014
GeometryEllipse, 1015 GeometryPolygon, 1016 GeometryPolyline; 1101
EffectGradient, 1102 EffectAirbrush, 1103 EffectBlur, 1104 EffectStamp, 1105
EffectDust, and 1106 EffectAlphaGradient. Each tool sequence contains exactly
those 24 IDs in ascending order. Tool code 1010 remains retired and is rejected.
A color entry frame
orders ToolId and an optional exact color. Color is required only for Pencil,
Brush, Fill, Selection, ColorReplace, and EffectAirbrush and is absent for every other tool. A diameter
...
Square); and booleans 12 pressure-size and 13 screen-size.
...
`org.inkpod.digest.editor-state.v2`. Editor revision, editor savepoint, and the
digest field itself are excluded.

For raster assets, canonical stride is exactly width times bytes per pixel and
payload is top-to-bottom row-major bytes without padding. Sample-stream assets
use the sequence framing above and primitive-owned scalar rules. Chunking never changes
...
Asset-kind codes 1 and 3 are CanonicalRaster and CanonicalSampleStream; retired
code 2 is invalid. Raster pixel-format codes are the five canonical codes
...
are Opaque, Straight, and CoverageMask. Sample assets omit pixel/color/
...
tail, known persistent editor/document-metadata references, and active transient
...
|---|---:|---:|---|
| `META` | 2 | critical | exactly one kind-1/v2 record |
...
21. raster companion format (`u32`: 1 PNG, 2 TIFF, 3 TGA, 4 BMP)

The format field is mandatory and rejects every unknown code. Raster import records
the decoded source format; a new blank Cell uses the application default (PNG unless
changed in Preferences). Native open, recovery, sequence switching, and explicit
compaction preserve this field. Changing the application default affects only later
blank Cells. This file metadata does not put a filesystem path into a procedure or
change replay epoch 27. The outer META section and record are version 2; the generic
length-framed field envelope remains schema 1.
...
`DocumentStateDigest`; 6 an optional 40-byte initial-raster source containing a `PlaneId`
and `AssetId`. The archive starts with base-surface code `u8` (1 SolidWhite, 2 Asset
followed by its 32-byte `AssetId`, 3 Transparent), then nested payload
...
before replay. Document metadata schema 8 stores active layer/plane IDs, current
selection and fill-protection Plane IDs, ordered standard layers and their plane
properties, guides, grid, Color chart, an exact 0/1 shooting-frame presence value,
and ordered saved-selection `(stable ID, UTF-8 name)` records. When present, the
shooting-frame descriptor stores the
stable ID, signed center, positive size, binary turns, anchor, visible/export
flags, and a zero reserved word. Plane descriptors use closed archive-kind codes
1 MainLine, 2 Color, 3 Raster, 4 CurrentSelection, 5 LightTable, 6 FillProtection,
and 7 SavedSelection. Only kinds 1 through 3 may occur in a layer. Saved-selection
descriptor IDs must match metadata records one-to-one; all mask rasters are exact-size
`BinaryMask8`. The decoder bounds object counts, validates UTF-8 and topology,
...
EditorState schema 7 contains 13 fields and uses the closed 24-tool catalog; retired
VanishingPoint tool code 1010 is rejected.
Optional ordinal 12 is the nested Color chart page `u32` and absolute selected
index `u32`; ordinal 13 is the
...
| palette entries / Color chart entries | 4,096 / 4,096 |
| one Color chart UTF-8 name | 1,024 bytes |
...
selection collections, asset graphs, event journals, and ID authorities
...
Batch settings use the separate `.inkbatch` extension. Graph version 5 with
operation schema 4 is a bounded
...
policy does not change the portable current-v5 byte schema or Core graph name.
...
Version 5 is exact-current: version 4 and every other graph version are rejected
...
enabled flag, conjunctive optional stable layer/plane IDs and semantic plane role,
...
connectivity, or legacy separation destination is representable in v5. The v5
target record removes the obsolete layer-kind field; its plane-role code is
closed to Color and Raster. Code 1 (MainLine) is a tombstone and is rejected.
...
out-of-range opacity/grid values, stable-ID collisions across document and
...
### Normal Cell save is a recoverable native/raster pair
...
explicit display/instruction exports retain their own established output contract.
