...
`.inkpod` v10 is the bounded, procedure-authoritative, little-endian native
container. Version 10 and replay epoch 7 are the only accepted native contract.
...
top-level format version 10 and replay epoch 7. It uses a hierarchical document
...
version 1. Every version other than 10 is rejected before Core state replacement;
...
schema-1 acceleration record in v10. There is no `HIST` section: history moves and
...
The four procedures below have fully specified byte schemas. V10 stores bounded
...
- An asset-reference record is `argument ordinal u32`, zero `u32`, then the
  ...
  kind authority in v10. Each sequence is strictly increasing
...
does not change native format v10 or replay epoch 7.
...
the v10 reader selects its decoder through the same catalog entry and accepts
...
- Guides retain display order and order each stable ID, axis code, and Q16
  ...
  current build contract is replay epoch 7 and top-level version 10; optional
...
the exact graph serialized by v10 `GENS` and `ASST`.
...
ownership. That detached archive remains test infrastructure; v10 now provides
...
never these digests. The same pinned implementation computes the v10 section,
root, asset-chunk, journal, document, editor, and procedure-payload commitments.

### Header, directory, and record bytes

The v10 header is exactly 128 bytes:
...
v10 GENS schema. Its fixed manifest is 200 bytes before palette, optional
...
The v10 writer emits this bounded canonical EDIT payload and the staged reader
...
Genesis and every retained journal asset are written through the same v10
...
digests. Only the exact current token can write a new v10 file whose current
...
`rust/inkpod-format/fuzz` package provides `native_v10` for the current
container, directory, CKPT removal/re-encode path and `native_core_v10` for staged Core
