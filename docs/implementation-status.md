...
| Windows frontend       | UI/Input, Core engine, and Renderer are separate long-lived threads. `Application` owns startup/message-loop/shutdown, MainWindow owns message normalization and command routing, feature controllers own typed UI state, and dialog modules use typed inputs/results without receiving the complete application state or Core. The owned floating Tools palette exposes all 37 Tools-menu leaves as text buttons, shares command state with the menu, restores an on-screen per-monitor placement, and hides instead of destroying on close. Command routing and command-state catalogs cover every production command exactly once. |
