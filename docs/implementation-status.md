...
| InkScript | M00's closed registry meta-schema, command-independent language v1 registry, empty private catalog-v1 draft, all-84-replayable-primitive owner manifest, and Batch→SCRIPT→milestone→evidence traceability are approved. M01 provides immutable UTF-8 source/line-map/diagnostic/lexer APIs; M02 provides the Core-independent lossless CST and bounded exact-current parser. M03 provides the semantic AST/canonical emitter and M04 the typed complete-file orchestration/path-intent envelope. M05A supplies generated language types, immutable typed declarations and per-run values. M05B adds scalar/list result metadata, typed step/group/result dependencies, and closed-fragment rewriting. M06 generates typed selector/assert/ID-namespace metadata and adds a crate-private initial-snapshot selector/assert/skip evaluator plus a closed test-only catalog interface for portability, work, result, asset, and editor metadata. Core-backed compilation/execution, a production catalog, authority/plan, ABI, UI and product file acceptance do not exist yet; the private draft still has zero entries and `.inkbatch` v2 remains the current Batch contract. |
...
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | M00 language/meta schemas, owner manifest, traceability and private-production gate are approved; M01 source/lexer, M02 lossless CST/bounded parser, M03 semantic AST/canonical emitter, M04 typed orchestration, M05A type/namespace/parameter, M05B typed step/result/dependency models, and M06 private selector/assert/catalog interfaces are implemented and automatically validated | M06 existing-binary regression confirmation, then M07 and later owner milestones |
| `SCRIPT-003` | Normative planning/execution/persistence contract and owner milestones are defined | No InkScript plan, runner, output, save/reopen, or report implementation exists |
...
- InkScript M00 is approved and its M01 lexical, M02 lossless-parser, M03 semantic/canonical, M04 typed-envelope, M05 declaration/step, and M06 private selector/assert/catalog Rust layers exist, but `.inkscript` remains
  unreachable from Core/FFI/Windows production, its command draft is empty, and `.inkbatch` v2 remains the
  current Batch format until the M29C/M34 gates are separately accepted.
...
The latest automatic verification is for InkScript M06 on 2026-08-15. Generated typed metadata covers
selector filters, owner/order/cardinality/missing rules, assert comparison, and persistent-ID namespace order.
Four crate-private binding/assert tests validate initial-order `one`/`first`/`all`, missing and ambiguity,
owner mismatch, strict UUID/ID/state/ID-allocation preconditions, list results, shared-graph skip propagation,
disabled-step distinction, mutation-free failure, resource bounds, and ownership. Two catalog tests validate
closed portability/work formulas and result/asset/editor metadata while rejecting query/view/session commands,
unknown paths, divide-by-zero, overflow and resource excess. One integration contract and the architecture
suite prove that Core, FFI, Windows, and the `inkpod-format` public surface do not reach the private route.
The five M05B, four M05A, four M04, six M03, thirteen M02, twelve M01, and seven M00 contracts remain green;
lexer/parser fuzz targets still type-check,
the private catalog draft still has zero entries, and `.inkscript` still has no product route. The
latest full product verification remains the strengthened WIN-001 Japanese/English
localization slice from 2026-08-14. A versioned HKCU preference records System, Japanese,
...
| Boundary | Result |
| --- | --- |
| InkScript M00 | Draft 2020-12 validation accepts the language, draft and owner registries; `inkscript_registry` passes all 7 tests: JSON/meta closure, duplicate/malformed/overflow rejection, language references, owner bijection, version/traceability drift, private-draft unreachability, and typed-model product non-reachability. File v1/catalog v1/epoch 23/native v26/ABI v14 remain aligned; the draft has zero command entries and no production parser/executor/ABI/UI route |
...
| Rust workspace | `HistoryEntryKind` replaces product-facing history labels in Core and maps exhaustively to the fixed-width ABI v14 kind. All 519 tests, including one doctest and zero ignored, passed; `fmt`, all-target/all-feature Clippy with warnings denied, workspace strict rustdoc, the seven-test InkScript registry suite, lexer/parser fuzz target check, and the resource-split route inventory passed. The inventory also proves that Japanese and English generated resources expose the same 642 resource identifiers, including the same 384 static command IDs |
...
| Windows x64 | The 2026-08-15 Debug `inkpod`, host tests, portable ZIP and unsigned MSIX rebuilt after M06 under MSVC `/W4 /WX` with verified static CRT. All 36 CTests passed on the final binary with desktop permission in 407.53 s: ABI smoke took 58.03 s, English product smoke 167.84 s and Japanese product smoke 168.35 s. M06 has no Windows/ABI route. The latest Release run remains the 2026-08-13 M22 run with all 33 then-current CTests passed in 75.57 s |
