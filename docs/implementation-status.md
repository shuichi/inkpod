...
| Build and distribution | CMake drives both the Rust static library/MSVC C++20 build and the macOS Cargo→Xcode sub-build. Rust domain crates remain OS-independent; Windows x64/ARM64 use static CRT. macOS Debug and Release fix Cargo, CMake, and Xcode to arm64; the Release gate now produces a real `Inkpod.xcarchive` and requires the Rust archive, XCTest executable, 64-session headless CoreHost, and archived app executable to contain exactly arm64. `scripts/macOS.sh verify` runs Rust, Swift/ABI/parity, UI accessibility, Metal soak, Thread Sanitizer, and archive profiles; `release` cannot skip Developer ID signing/notarization. Normal `publish` independently verifies the existing notarized DMG, requires a clean synchronized release commit, fixes the version tag to exact HEAD, adds a missing DMG to an existing public Release or creates a prerelease, and accepts tag/Release/upload races only for the same commit or byte-identical asset without clobbering different bytes. Explicit development-only `publish --force` retains all preflights but may retarget an existing prerelease tag through remote-object `force-with-lease` and clobber only its same-name macOS DMG; stable Releases are rejected, tag-race failure restores the prior local tag, and post-mutation tag/asset bytes are verified. The protected workflow imports ephemeral signing/notary credentials, and the checklist requires physical and clean-Tahoe evidence to be Pass/Fail/Blocked rather than inferred. Version 0.2.3/build 175 was signed with Developer ID team `ETD7LJJGQZ`; notarization submission `fd8ccacc-c1a2-4d30-9c5b-064ea38fc901` was Accepted with no issues, its ticket was stapled and validated, and Gatekeeper accepted the DMG as `Notarized Developer ID`. Unsigned MSIX and four-file portable ZIP packaging paths are maintained for Windows. |
...
checker work.
On 2026-08-18, a supplemental viewport/input regression first reproduced the
stale-revision race with normal Core processing paused, then verified that three
rapid sizes produce one in-flight and one latest coalesced revision, that no
document edit is issued before convergence, and that the retained stroke commits
after convergence as one Undo unit. The standard `inkpod_macos_check` target
passed, all nine product Canvas lifecycle tests passed with Metal API Validation,
CTest passed 22/22, and the original product lifecycle test passed ten
