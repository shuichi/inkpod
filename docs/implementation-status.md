...
| InkScript | Exact-current registry schema v2 and the approved command-independent language v1 remain unchanged. M07–M12 are user-confirmed and the private catalog-v1 draft remains 13/84 exact owner entries. M09 provides the crate-private compiler/binder/single-input staged runner, M10 bounded immutable asset freeze, M11 authority-bound planning, and M12 the crate-private sequential RunTask, deterministic report, current-v26 prospective-savepoint encode, and injected identity-guarded atomic-install boundary. M13 now records a measured quick/full performance proposal without changing a harness or envelope and awaits explicit approval. Production catalog ratification, Windows authority adapter, ABI, UI and product file acceptance remain absent, and `.inkbatch` v2 is still the production Batch contract. |
...
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Exact-current registry schema v2, language v1, owner manifest, traceability and private-production gate are validated; M07–M12 are user-confirmed. The private draft remains 13/84 entries, and M13 has a measured but unapproved performance proposal | M13 approval, remaining owner milestones, and production catalog/ABI/UI gates |
...
- InkScript M00 is approved and its M01–M08 language/adapter layers, M09 private compiler/binder/single-input staged runner,
  M10 asset freeze, M11 authority/plan and M12 test-adapter multi-item persistence layers exist, but `.inkscript` remains
  unreachable from Core public API/FFI/Windows production. The private command draft contains only the 13 M07/M08 owner
  entries, the real OS adapter installs no product output, and `.inkbatch` v2 remains current until M29C/M34 are accepted.
...
The latest automatic work is the InkScript M13 docs-only performance proposal on 2026-08-15; M12's private runner and
persistence slice is now user-confirmed. A temporary crate-private Release probe exhaustively measured the declared step,
item and asset candidate axes and was removed afterward. The selected quick fixture fixes 128 steps, four successful items,
a 256 KiB inline asset, six attempted items, 768 invocations, 384 Commit/no-op pairs and checksum
`0f84d2c54cfe1e2c`; its nine-process median after one discarded warm-up is 85.3722 ms. The selected full fixture fixes
1,024 steps, eight successful items, a 16 MiB inline asset, 10,240 invocations, 5,120 Commit/no-op pairs and checksum
`17c636b92b1aebf1`; its five-process median is 20.4550998 s. Proposed 64–107 ms quick and 15.3–25.6 s full envelopes are
not approved and no benchmark harness/envelope changed. The private catalog remains 13/84, and production InkScript remains
unreachable. The latest full product verification is the M12 x64 Debug regression run from 2026-08-15; the strengthened WIN-001
...
| Boundary | Result |
| --- | --- |
| InkScript registry | Exact-current registry schema v2 accepts language v1, private catalog-v1 draft and owner manifest and rejects superseded schema v1. `inkscript_registry` passes all 9 tests, including closed catalog-owned types, exact 13-entry M07/M08 owner identity/metadata, duplicate/malformed/overflow rejection, version/traceability drift, and private product non-reachability. File v1/catalog v1/epoch 23/native v26/ABI v14 remain aligned |
...
| InkScript M13 | The proposal fixes deterministic quick/full source, seed, inputs, work formulas, parser/compile/bind/runner/asset counters, checksums, timed interval, independent-process sample policy and environment-specific bounds. Every declared candidate-axis point and the selected full compound were measured with a temporary Release probe that was removed afterward. Quick/full medians are 85.3722 ms/20.4550998 s with exact checksums; the proposed envelopes remain unapproved and no harness or existing envelope changed |
| Rust workspace | `HistoryEntryKind` replaces product-facing history labels in Core and maps exhaustively to the fixed-width ABI v14 kind. All 556 tests, including one doctest and zero ignored, passed; `fmt`, all-target/all-feature Clippy with warnings denied, workspace strict rustdoc, the nine-test InkScript registry suite, every retained format contract and the resource-split route inventory passed. The inventory also proves that Japanese and English generated resources expose the same 642 resource identifiers, including the same 384 static command IDs |
...
| Windows x64 | The 2026-08-15 Debug `inkpod`, host tests, portable ZIP and unsigned MSIX rebuilt after M12 under MSVC `/W4 /WX` with verified static CRT. All 36 CTests passed on the final binary with desktop permission in 410.11 s: ABI smoke took 57.27 s, English product smoke 168.42 s and Japanese product smoke 169.34 s. M12 adds no Windows/ABI/product route. The latest Release run remains the 2026-08-13 M22 run with all 33 then-current CTests passed in 75.57 s |
