...
| Windows frontend | The Batch pane retains its editable fixed Input/Output graph and four-operation catalog. Color Replace now exposes standard checkboxes for general-raster, binary-coloring, and grayscale-coloring targets; one or more may be selected together, all selectors round-trip through `.inkbatch` v4/ABI v22, and externally loaded fixed-ID selectors remain intact until semantic selection replaces them. Preview/run/save still builds one immutable graph, and the existing singleton tab, generated Japanese/English presentation, cancellable preview, issue-time targeting, and Rust-owned staged-result routes remain. |
...
| `BATCH-001`–`BATCH-004` | Batch v4 Core/format/ABI/Windows implementation stores multiple Color Replace target selectors, resolves every matching layer, deduplicates planes, and commits one private canonical primitive/Undo unit atomically. Standard multi-select checkboxes, fixed-ID preservation, saved-set round-trip, full Rust gates, focused Windows/ABI checks, and x64 Debug English/Japanese product smokes are present; prior Batch controls and preview routes remain. | Complete physical high-contrast/screen-reader checks and v22 all-preset reruns before `Verified` |
...
| Windows Batch v4 | On 2026-08-25, x64 Debug rebuilt under MSVC 19.51 `/W4 /WX`. The Batch set-store/input-picker/color-editor, localization-catalog and ABI smoke tests passed 5/5 in 46.62 s. After final allocation-failure and DPI-font hardening, English/Japanese product smokes passed in 291.88/391.78 s and cover three simultaneous semantic target checkboxes plus `.inkbatch` v4 preservation, alongside the retained Preview/Run All/Cancel and document lifecycle routes. Other v22 presets remain pending |
| Windows x64 | Exact-current v28/ABI v22 x64 Debug executable and Batch tests rebuilt under `/W4 /WX`; focused CTest and both localized product smokes pass. The complete Debug matrix, Release build/tests, static-CRT/package checks remain to be rerun for v22; prior v21 evidence is historical |
