...
| Document-tab presentation | Initial and split editor groups apply a DPI-scaled 9-point Segoe UI ClearType font to each document-tab control. Each tab `HWND` recreates its owned font after a parent DPI transition and releases it during destruction. Every visible document tab owns a bounded, reused Common Controls close button with a system-color DPI-scaled icon and localized accessible name. The button routes the exact stable `DocumentViewId` through the existing `IDM_VIEW_CLOSE` command, preserves a still-valid previously active view when closing an inactive tab, and remains separate from label drag hit testing. |
...
| `BATCH-001`–`BATCH-004` | Batch v4 keeps multi-target Color Replace, stable plane resolution/deduplication and one private canonical primitive/Undo unit. File/folder inputs, preview copies/output reads, temporary cleanup and selected-format output now use the Rust manager through asynchronous ABI v23 jobs. Issue-time active state, new-tab results, cancellation and per-item failures remain covered by public contracts; ARM64 Debug/x64 Release English/Japanese product smokes pass. | Physical high-contrast/screen-reader checks and unrerun platform/configuration checks remain |
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Exact-current registry schema/language/file v2 and catalog/owner manifest v4 with 75 public commands are aligned to native/replay/ABI v29/25/23; private Batch v4 orchestration is excluded | Product `.inkscript` acceptance and the M36 full gate remain pending |
| `SCRIPT-003` | M12 authority/plan/run/install and the M27A/M27B private CoreHost route remain; its current native output follows v29/epoch 25/ABI 23 | Product `.inkscript` file acceptance remains absent |
...
### IO-003 migration checkpoints (2026-08-27)

These are completed checkpoints, not a claim that every current gate or
physical user flow has passed. The final Rust workspace and both recorded
Windows configurations' 41 non-GPU tests and full English/Japanese product
smokes pass. ARM64 renderer repeatability and performance acceptance remain
open; `IO-003` remains `Experimental`.
...
After the final product-smoke fixture change, both complete builds regenerated
...
The renderer fixture now shows nonoverlapping parent windows before Canvas
...
are unchanged; production renderer code is unchanged. Broadening the measured
...
flows, and non-Windows platform execution were not rerun for this migration.

#### Current Core quick timing investigation
...
They provide historical evidence and do not certify the current v29/ABI v23 I/O
implementation or replace the outstanding migration gates above.
