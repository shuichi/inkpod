...
On 2026-08-26, the Color-pane minimum-size and repaint correction passed the
x64 Release `inkpod_windows_dock_host_boundaries`,
`inkpod_windows_workspace_layout`, and `inkpod_windows_owner_model` tests. The
English and Japanese product smokes passed in 75.66 and 83.08 seconds. Both
smokes resize the actual Color content window to the descriptor minimum, require
nonempty bounds and completed `WM_PAINT` delivery for the tabs, main-line label
and swatch, picker, numeric field, and Apply button, and require no pending update
region afterward. The picker also retains the minimum geometry needed for its
ring, HSV triangle, and opacity track. The workspace test covers the 300-DIP
Color minimum at 96 and 192 DPI and proves stable `NoOp` behavior at both adjacent
splitter limits. Computer-assisted manual checking was unavailable because the
Windows helper reported an existing active request, so no new physical drag claim
...
| Windows Preferences | On 2026-08-26, ARM64 Debug rebuilt under MSVC `/W4 /WX`. The 920-by-680-DIP dialog uses current-DPI 9-point Segoe UI plus measured text-height padding on all six pages; its four keyboard rows, instruction, status, and detail controls are smoke-checked for containment and non-overlap. Page statics use the tab surface's system colors, page changes invalidate the tab and visible children without synchronous all-child redraw, and seven localized Shortcut label groups have measured-width smoke checks. The real dialog contract passed in English and Japanese through an early diagnostic product-smoke invocation. Normal complete smokes stop before that command at unrelated check `11003`; with Preferences moved ahead temporarily, they passed it and later stopped at unrelated DockHost check `11020`. Color and Layer/Plane pane rows retain the same measured-height rule. |
