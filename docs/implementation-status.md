...
| Area | Current state |
| --- | --- |
| Line correction and selection | Approved dust/background/full-component rules, permanent endpoint joining and independent raster thicken/thin/uniform-width modes are connected from localized tool/menu settings through versioned ABI and canonical Core transactions. Wand closes virtual boundaries before flooding and preserves source; trace selection captures the gesture-start coordinate frame and screen diameter. Exact native-depth pixel/mask, preview/Cancel, one-Undo, replay/save and actual Canvas/menu regressions pass; see [implementation results](line-selection-implementation.md) and the separate [historical audit](line-selection-audit.md). |
...
| Persistence | Native `.inkpod` is exact-current v34/replay epoch 29 with DocumentArchive schema 7, DOCM schema 9, document digest schema 13/domain 11 and snapshot-composite schema 5. Fully opaque common-raster imports retain exact editable RGBA MainLine pixels over a SolidWhite Genesis underlay; imports containing any non-opaque alpha retain a Transparent underlay. The Genesis asset preserves canonical decoded dimensions, RGBA8/16 depth, straight alpha and pixels, not the source container byte layout or optional metadata. DOCM is mandatory; no legacy two-plane synthesis or migration reader remains. `.inkbatch` is exact-current v5/operation v4. Pair paths and `Planned`/`Committed`/`None` remain runtime-only; the v34/epoch-29 update records the approved line-correction/background/selection semantics, new primitive and tool defaults. Normal pair save/recovery and current-only rejection retain their existing atomicity rules. |
| Shared file I/O (`IO-003`) | One application-owned `inkpod-io` manager owns editable native/raster, sequence, reference, Light Table and Batch filesystem work. Windows submits typed paths through ABI v34; Rust owns bounded enumeration, identity, codecs, temporary files and cleanup. Raster-pair candidate proof collects native and raster candidates in one directory pass, while up to 32 notification-backed complete inventories make unchanged-folder revisits enumeration-free. Every attached Sequence retains all source tiles/thumbnails and up to 64 complete inactive editable targets in a COW resident bank while discarding the same pixels' dense decode-cache ownership. Exact-provenance sidecar-less targets reuse the resident tile map and precomputed asset ID with zero read/decode/hash/retile work. Replayed and companion-validated clean sidecar targets use a configurable application-wide 0–1024 MiB LRU (1 GiB default), fixed at 64 targets maximum; exact hits COW-share Core/assets/tiles. The live cell keeps prepared pair authority without duplicating its editable Core. Selected members still require complete stamps and final namespace/TOCTOU validation. |
...
| C ABI | Exact-current ABI v34 adds a bounded 128-byte line-correction request with background, region, brush, expected revision and direct/preview task operations. Rust/C11/C++ layout, native-depth pixel results, stale/invalid/cancel atomicity and task ownership pass. Existing single-owner-thread and release contracts remain; callers of ABI v1-v33 must rebuild. |
...
| InkScript | Exact-current registry schema/language/file v2, catalog/owner manifest v7, replay epoch 29, native v34 and ABI v34 are aligned. The public catalog has 74 commands; removed document features are tombstones and private Batch v5 orchestration remains excluded. No `.inkscript` product route was added. |
| Windows frontend | Layer/Plane presents one standard layer with MainLine, Color and optional Raster rows; add-plane creates Raster only and conversion changes storage format without changing role. Selection Mask commands manage a document-owned named-mask list. File Open adds a tab; Sequence resident hits exchange complete COW editing states without a file job or loading status and start dirty autosave separately, while misses retain the common resolver fallback. Revert reconciles pair identity/shell/binding. Batch targets Color/Raster roles through `.inkbatch` v5/ABI v34. |
...
| Product surface | File-backed Sequence, raster drawing/fill/destructive effects, angled shooting-frame, document-owned current/saved selection masks, standard layer/plane editing, transform, Light Table, raster clipboard, supported raster I/O, Batch, history, recovery and compaction remain connected. Cut management, instruction-raster export, vanishing-point and non-destructive adjustment-layer features are removed from the product and data format. |
...
| --- | --- | --- |
| `ABI-001`, `ABI-002` | ABI v34 retains prior bounded APIs and adds the 128-byte line-correction request and direct/task-preview entry points. Rust layout, public FFI pixel/negative/ownership tests and C11/C++ layout checks pass. | ABI v1-v33 callers must rebuild; unrun configurations/platform/manual checks remain. |
| `IO-001` | Native v34/epoch 29, required DOCM, exact decoded-pixel Genesis assets, full journal/inactive-branch/EditorState reopen and v34-only rejection are covered together with existing native replay/save/recovery. | Compression code 0, physical/manual coverage and unrun configurations remain. Source container bytes are intentionally outside the lossless pixel contract. |
...
| `SUBPALETTE-001` | ABI v34 retains the read-only catalog/view and parallel manager jobs. The independent permanent bars, sticky range/reset causes, exact Shift+Arrow/Page pan, unmodified Arrow/Page navigation, stable auxiliary route and catalog `presentation_epoch` are implemented. Visible catalog replacement rejection preserves the prior publication; hidden replacement clears the old retained snapshot before deferral. | Exact post-scroll sampling, durable hidden-publication retry, strict active-image renderer-rejection rollback, page/thumb/resize/accessibility product evidence, interactive file-dialog and unrun platform checks remain; see [correction record](subpalette-first-load-diagnosis.md). |
| `BATCH-001`–`BATCH-004` | Batch v5 resolves Color/Raster role or fixed-ID selectors, deduplicates overlaps and commits one private primitive/Undo unit through ABI v34. | Physical accessibility and unrun platform/configuration checks remain. |
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Catalog/owner v7 with 74 public commands aligns to native34/replay29/ABI34, including canonical line correction; private Batch v5 orchestration is excluded. | Product `.inkscript` acceptance and the M36 full gate remain pending. |
| `SCRIPT-003` | The private CoreHost route emits current v34/epoch29/ABI34 output through the existing staged authority model. | Product `.inkscript` file acceptance remains absent. |
...
The historical PaintMan gap-closure program is retained in [`legacy.md`](legacy.md). User-approved simplification on 2026-09-03 retires Cut management and Cut-only membership editing. Independent Cells and ordinary file-backed Sequence retain their existing identities, recovery, navigation and thumbnail behavior.

## Known differences

- Native Cell `.inkpod` is current-only v34; non-v34 files, including v33, are rejected without a
...
- Native v34 accepts compression code 0 only; measured checkpoint behavior has not
...
### Line correction and selection implementation (`PAINT-003`, `SEL-001`, `SEL-004`, 2026-09-03)

The user approved the five semantic decisions in the [implementation plan](line-selection-implementation-plan.md).
Dust now measures entire source components, distinguishes white/transparent/native custom backgrounds,
and edits only fully contained components. Permanent joining uses inclusive empty-grid-step gap and
unambiguous facing endpoints. Raster thickening/thinning use a one-sided circular radius; uniform
width is an independent skeleton reconstruction mode. Wand closes virtual boundaries before flood.
Trace input and local corrections use the gesture-start view frame, without applying OS DPI in Core.

Core's 29 public-API synthetic contracts, image's nine native-format/boundary contracts, FFI
invalid/stale/cancel/ownership and InkScript round-trip contracts pass. The original five audit
failures now pass without weakening their expected pixels. A further real Canvas rectangle bug
at document x=11.999999 reproduced extra changes at (11,23)/(11,25); the new public regression
failed before canonical Q16 rectangle construction and passes after that fix with identical expectations.
Native v34 / runtime replay 29 / ABI 34 / catalog-owner v7 are aligned; command count is 74.
A further RGBA16 alpha=1 / one-level contrast fixture reproduced a morphology no-op against
custom background; preserving non-background ranking above zero fixes it without changing native
RGBA values or weakening the 81-pixel expected set. All 834 workspace tests pass afterward.
DocumentArchive 7, DOCM 9 and document digest 13/domain 11 remain unchanged. Catalog/editor-default
digests changed because primitive semantics and tool defaults changed, not to hide pixel failures.

Strict Clippy, all-feature workspace tests, warnings-denied rustdoc, formatting, all nine unchanged
quick Core benchmark scenarios and x64 Debug configure/build pass. English/Japanese full product
smokes pass in 346.91/351.02 seconds and again in 346.34/357.44 seconds. The complete CTest run
passes 48/49; RendererHost fails its existing Sequence first-edit readback with S_FALSE and 21
latency timeouts. After the final low-alpha correction, all Rust gates and MSVC rebuild pass again,
and isolated RendererHost/ABI pass in 67.39/45.27 seconds with unchanged assertions and waits.
The intermittent readback cause remains unisolated; its failed full-run result is retained in the
[implementation execution record](line-selection-implementation.md#実行記録).
The actual product regression checks all seven correction modes through menus/settings/Canvas,
all 1,024 source pixels and other-plane invariance, Cancel and one-step Undo/Redo, whole-plane Apply,
closed/open/virtually closed wand masks, and all 180 L-band pixels during mid-gesture view changes.

The [historical audit](line-selection-audit.md) retains its clean starting commit, five Core failures
and earlier product-smoke failures. Those historical failures are separate from implementation
bring-up failures and the final passing gates; prior pencil/selection-algebra test names alone did
not establish PAINT-003 or leakage prevention. Physical pen/touch, all physical DPI combinations,
ARM64/Windows Release and non-Windows execution are not revalidated in this implementation run.

### Cut management removal (`IO-001`, `SEQ-001`, `SHOOTING-FRAME-001`, 2026-09-03)

User-approved simplification removes New Cut, Cut save/properties/history, the File Cut-sequence submenu and its membership controls/drag/reorder/renumber routes. New Cell is a direct File command. Native and supported raster input share File > Open and its companion resolver; instruction export is removed. Normal Sequence, Canvas angled shooting frames, paper-frame operations and white-background raster composition remain. The production surface now has 304 menu commands and 312 command-state entries. CutCore/CutSession, descriptor codecs, Cut-only ABI, translations, tests and fuzz entrypoint are removed; embedded manual/file-format help is aligned; retired Cut magic remains a negative native-input fixture. User documents are not deleted.

The exact-current tuple is native v33 / replay 28 / ABI 33 / DOCM 9 / document digest 13-domain 11 / InkScript catalog-owner 6. Language/file v2, DocumentArchive 7 and snapshot-composite 5 remain. Prior native files (including standalone v32 Cells), Cut descriptors and old catalog contracts are rejected without a migration shim. ABI clients must rebuild against v33. Shortcut presets/settings naming retired commands fail the existing unknown-command validator; invalid settings keep default fallback and automatic-write protection. No retired-command migration or reassignment is added.

Rust workspace tests, strict Clippy, rustdoc with warnings denied, formatting, the generated InkScript-reference check and x64 Debug configure/build pass. Core workflow quick and full benchmarks pass all nine scenarios. Only schema-dependent document-hash goldens changed; workloads, pixel-composite goldens, counters, harnesses and timing envelopes are unchanged. The pre-existing ignored release-only InkScript performance calibration is not part of this run.

All 49 CTest cases have passing latest results across staged runs. The final English/Japanese product smokes pass in 311.67/297.06 seconds (608.75 seconds combined). The final focused 43-case run passes in 25.47 seconds, covering embedded help extraction, menu/state/shortcut/localization, ownership, Core host, InkScript, static CRT, release automation dry-runs and ZIP/MSIX payloads; the separately rerun RendererHost boundary also passes. The prior full run already passed the unchanged ABI smoke, RendererHost and Sequence performance cases (45.01/97.48/79.51 seconds). The final smoke fixes use a sidecar-less Genesis fixture with normalized paths, retain the separate-file open/close assertion after Save As, and require exact-pair duplicate Open to reuse its session/view. They do not relax pixel, history, pair-authority or resize invariants. Embedded manual/file-format HTML nesting, IDs and fragment links also pass validation. The repository website copy is aligned but is not published.

An earlier unmodified modal-dialog-position run returned 7 on this two-monitor desktop: its owner moved from the first work area `(0,0)-(1728,1026)` to `(1528,876)-(2328,1476)`, selecting the second work area `(1728,-3)-(3648,1047)`, while the test still compared against the first. Later full and focused runs passed without changes to positioning code or that test; the monitor-assumption issue is not fixed here. An initial RendererHost timeout also passed on retry and in the later full run. ARM64, Windows Release, hosted/non-Windows CI, coverage-guided fuzzing, packaging install/uninstall and physical accessibility/pen checks are not revalidated here.
...
Audit correction (2026-09-03): this historical fixture exercises ordinary
strokes and coordinates, not dust, endpoint connection or existing-line width
correction. Its PAINT-003 tag did not establish those features; see the current
[line/selection audit](line-selection-audit.md).
