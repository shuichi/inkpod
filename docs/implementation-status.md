...
| Shared file I/O (`IO-003`) | One application-owned `inkpod-io` manager owns editable native/raster, sequence, reference, Light Table and Batch filesystem work. Windows submits typed paths through ABI v30, including the common raster-pair open kind and explicit current-document Revert flag; Rust owns bounded enumeration, identity, codecs, temporary files and cleanup. Temporary-job cleanup removes only its owned child and retains the shared allocation root, so concurrent creation cannot lose that root between creation and canonicalization. Raster-pair candidate proof now collects native and raster candidates in one directory pass. Windows retains up to 32 complete inventories behind fail-closed directory-name observers, so an unchanged-folder revisit performs no `read_dir`; add/delete/rename, observer error, explicit cache clear and LRU eviction force another bounded enumeration. A decoded source capability uses same-manager, complete-stamp, generation, format, metadata and allocation proof without pinning inactive catalog payloads; an exact sidecar-less target may retain the immutable decoded allocation, while invalidation/reload/foreign allocation falls back. Exact sequence-recovery pair proof, resolver-baseline/savepoint fencing, repair-needed clean navigation, terminal authority-revoked reporting and the v29 issue-time clean-history preservation fence are retained. Selected members still require complete stamps and final TOCTOU validation; external writers retain the documented permissive sharing policy. |
| Raster pair resolution and save authority | File Open, Sequence and Revert use one same-stem resolver. A valid existing `.inkpod` wins only after staged replay and exact canonical decoded-raster comparison; an absent sidecar yields a disk-clean `Planned` pair; corrupt, non-current or mismatched sidecars fail closed. An eligible sidecar-less Sequence switch shares the catalog tile map and immutable decoded Asset payload; an ineligible source takes the same owned import. This private construction decision does not affect authority or the existing pristine re-registration of every resolver-proven pair target. Authority has exactly `None`/`Planned`/`Committed`; repair-needed is a `Committed` substate. `Committed` Save targets the installed pair, `Planned` Save materializes its first pair without Save As, and only `None` asks for a destination. Save As creates a new pair; flat raster output is Export. Exact-path/UUID Revert retains runtime sequence, inactive recovery and all live view IDs/logical states, invalidates old render caches and rebases the active binding; standalone recovery remains pathless/dirty. x64 Debug focused lifecycle tests and English/Japanese product regressions pass; unrun configurations/platform/manual checks remain under `IO-003`. |
| C ABI | Exact-current ABI v30 publishes MainLine/Color/Raster roles, role-preserving format conversion, Raster-only plane creation, document-owned saved-selection operations, common raster-pair open, exact sequence-recovery pair proof, the v28 terminal authority-revoked result flag, v29 issue-time source-recovery-required flag and explicit `INKPOD_IO_REVERT_CURRENT`. Revert is valid only for forced `OPEN_NATIVE`, requires the exact current path/UUID and retains runtime sequence plus all live view IDs/logical states while invalidating old render caches. Rust/C11/C++ numeric/layout checks fix the new kind/flag values and explicitly reject ABI v29. Removed layer-kind, Selection-plane, vanishing-point and adjustment-layer layouts/routes are not retained. Existing manager/job, snapshot, editor, persistence, Batch v5 and InkScript handles keep their ownership/thread contracts. ABI v1-v29 callers must rebuild. |
| Windows application data | Ordinary settings now have one source of truth at `%LOCALAPPDATA%\inkpod\Settings\inkpod-settings.json`. The fixed-name current-v4 UTF-8 JSON adds bounded `animation.sequenceThumbnailWidthDip` (32–96, default 64) alongside language, save/recovery, other animation policy, output guard, custom shortcut profiles and workspace layouts. `.inkshortcuts` remains exact-current v3. A valid exact-format settings object with one positive version below current is deleted after delete-handle byte revalidation and defaults are used; it is never migrated or decoded. Malformed, foreign, duplicate-marker and future files remain preserved and rejected. Settings and workspace persistence do not use HKCU, `%APPDATA%`, or old formats; current decode is staged and bounded, and saves use same-directory flush/close/replace. See [`windows-application-data.md`](windows-application-data.md). |
| InkScript | Exact-current registry schema/language/file v2, catalog/owner manifest v5, replay epoch 27, native v32 and ABI v30 are aligned. The public catalog has 73 commands; removed document features are tombstones and private Batch v5 orchestration remains excluded. No `.inkscript` product route was added. |
| Windows frontend | Layer/Plane presents one standard layer with MainLine, Color and optional Raster rows; add-plane creates Raster only and conversion changes storage format without changing role. Selection Mask commands manage a document-owned named-mask list with save, replace/add/subtract, rename, delete and reopen. File Open adds a tab, Sequence replaces the active document only after pair preparation, and applied Revert reconciles pair identity/shell/active binding without deleting inactive recovery or secondary views. Batch targets Color/Raster roles through `.inkbatch` v5/ABI v30. x64 Debug focused authority/CoreHost tests and final English/Japanese product smokes pass for the new lifecycle. |
...
| Canvas tabs and sequence presentation | The Sequence pane remains one horizontal row with horizontal scrolling and Left/Right navigation. Cached thumbnails now scale to the configured 32–96 DIP box with aspect ratio preserved; changing width retains item storage, selection, focus, top index, thumbnail generation and cache keys. A singleton Bottom Sequence measures current-DPI font/control/scrollbar/Cut metrics, rounds up to DIP with a 168-DIP floor, fixes the Bottom height and omits its splitter. Normal splitter behavior returns for shared Bottom or other zones. Initial sequence discovery retains `連番読み込み`; an individual switch has a separate `セルを読み込んでいます` progress label only after the 50 ms polling threshold, while a short cache hit shows no immediate loading status. Source recovery still reports its autosave status immediately. Document tab and sequence-switch identity/presentation fencing remains unchanged. |
| Rendering and performance | Immutable snapshots carry raster spans, the independent shooting-frame overlay, snapshot-owned pools, diagnostics and previews. Canonical composite schema 5 has no adjustment LUT or vanishing/radial record. Raster-pair proof uses three combined candidate passes instead of six independent scans on a cold/uncached resolution; an unchanged Windows folder then reuses its observed inventory and performs zero additional directory enumerations. Raster `revision-max`, payload gates, image-cache limits and approved workloads/envelopes remain unchanged; the audited callgraph lock is updated for the removed snapshot inputs. |
...
| --- | --- | --- |
| `ABI-001`, `ABI-002` | ABI v30 retains the closed standard image-role/saved-selection model, manager/job/snapshot ownership, common raster-pair open, exact recovery proof, v28 authority-revoked reporting and the v29 issue-time preservation fence; it adds explicit current-document Revert. Rust format/clippy/full tests/rustdoc/quick benchmark, header/C11/C++20 drift and explicit v29 rejection, x64 Debug build/focused tests and English/Japanese product routes pass. | ABI v1-v29 callers must rebuild; unrun configurations/platform/manual checks remain. |
...
| `IO-003`, `SESSION-001`, `SEQ-001` | Shared Rust-manager routes, recoverable pair save, one File Open/Sequence/Revert resolver, exact native-composite/raster matching, three-state authority, direct clean first Save, non-silent corrupt/mismatch handling, exact sequence-recovery authority, BIND rekey/rebase and exact-path/UUID Revert retention are implemented without a serialized path or native-version change. Native/raster companion candidates are collected together; Windows notification-backed bounded inventories make unchanged-folder revisits enumeration-free while retaining add/delete/rename and final member validation. Exact-provenance sidecar-less switches reuse decoded Asset bytes and a COW tile map; forced cache miss takes an equivalent owned import, and both keep the verified pair-target pristine registration. Initial discovery and individual-cell progress are distinct, and a switch completed before the progress poll produces no loading flash. Current x64 product regressions cover exact-pair reuse/conflict, atomic BIND publication, Committed/Planned explicit Save As, post-publication failure revocation/retry and same-path/UUID multi-view Revert. | Unrun configurations/platform/manual/physical checks remain. Backends without an equally strong namespace observer deliberately retain uncached enumeration. Two-file filesystem atomicity is not promised. |
| `PERF-001` | Core workflow workloads and `revision-max` are unchanged. `TileRaster` clones share the ordered map; first effective mutation detaches only that map and the touched tile. The sidecar-less Sequence semantic gate records managed copy/materialization as 0/0, forced-fallback work as 16 bytes/4 pixels, warm A→B→A AssetId hashing as 0, and exact native/document/Genesis/asset/digest/editor/history parity. Raster-pair resolution still reduces cold candidate enumeration from six passes to three and the Windows warm unchanged-folder contract records zero additional `read_dir` calls; add/delete/rename, clear and 32-directory LRU tests protect invalidation. The unchanged nine-workflow Core quick benchmark retains all canonical checksums without an envelope change. A current x64 Release product run completes both 64-switch scenarios with all samples foreground, zero warm reads/decodes/uploads, one snapshot per step and all 50 reversal intents committed. | Sidecar replay and recovery remain full replay routes. The native-scrollbar route must still demonstrate zero raster payload access for cache-hit line/page/thumb pan. The current product result is a single diagnostic process, not a new approved envelope or replacement for the historical five-process timing series. A full current multi-process timing series, initial/cache-miss and background latency remain outside this run. |
...
| `SUBPALETTE-001` | ABI v30 retains the read-only catalog/view and parallel manager jobs. The independent permanent bars, sticky range/reset causes, exact Shift+Arrow/Page pan, unmodified Arrow/Page navigation, stable auxiliary route and catalog `presentation_epoch` are implemented. Visible catalog replacement rejection preserves the prior publication; hidden replacement clears the old retained snapshot before deferral. | Exact post-scroll sampling, durable hidden-publication retry, strict active-image renderer-rejection rollback, page/thumb/resize/accessibility product evidence, interactive file-dialog and unrun platform checks remain; see [correction record](subpalette-first-load-diagnosis.md). |
| `BATCH-001`–`BATCH-004` | Batch v5 resolves Color/Raster role or fixed-ID selectors, deduplicates overlaps and commits one private primitive/Undo unit through ABI v30. | Physical accessibility and unrun platform/configuration checks remain. |
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Catalog/owner v5 with 73 public commands aligns to native32/replay27/ABI30; private Batch v5 orchestration is excluded. | Product `.inkscript` acceptance and the M36 full gate remain pending. |
| `SCRIPT-003` | The private CoreHost route emits current v32/epoch27/ABI30 output through the existing staged authority model. | Product `.inkscript` file acceptance remains absent. |
...
sharing. For a sidecar-less Sequence target, an exact same-manager, complete
file-stamp, decode-generation, format, metadata and allocation proof permits the
new document to retain immutable decoded pixels and the catalog tile map. Cache
clear/reload, force reload, foreign allocation or any proof mismatch fail closed
to the unchanged owned import. The proof is weak and does not pin an inactive
catalog entry; an accepted asset retains its normal decoded-budget lease.

This classification is used only to choose and measure construction work. It is
not an authority or cleanliness predicate. Pair resolution, recovery fences and
final TOCTOU validation are unchanged, and the existing resolver-proven pair
target is re-registered as pristine regardless of whether managed reuse or the
owned fallback constructed the document. Sidecar and recovery routes always use
full replay. Native v32, replay epoch 27 and ABI v30 are unchanged.

The feature-gated semantic counters prove zero dense-copy bytes and zero full
tile materialization for exact managed reuse, versus 16 bytes and four pixels
for the forced-cache-miss fallback. A warm A→B→A revisit performs zero AssetId
payload hashing. Both paths produce identical document info, Genesis, asset,
digest, editor/history state, native DTO and serialized native bytes, remain
pristine, and save identically after decoded-cache clear and manager shutdown.
These counters are absent from production `Core` storage.

Workspace format, warning-denied Clippy, all-feature Rust tests, warning-denied
Core rustdoc and all nine unchanged quick-benchmark scenarios pass. The quick
times in scenario order are 0.343, 1.658, 1.041, 1.773, 2.558, 11.178, 1.222,
26.142 and 114.052 ms; all canonical checksums/counters remain unchanged and no
envelope was edited. Clean-first x64 and ARM64 Debug builds pass with static CRT
and package generation; their non-`local-only` CTest sets pass 45/45 in 70.25
and 45.02 seconds. A current x64 Release product sequence-performance run passes
in 39.02 seconds with 64 foreground samples per scenario, zero warm reads,
decodes and uploads, one snapshot per step, and 50/50 reversal intents committed.
Its A→B→A / A→B→C→B→A handler p50 is 12.303/12.519 ms and first-Present p95 is
260.862/249.704 ms on this 60 Hz, 192-DPI desktop. This single diagnostic run
does not replace or relax the approved multi-process 120 Hz envelope.
...
### Raster-pair lifecycle and ABI v30 validation (2026-09-02)

The current source and public contract now identify ABI v30 as exact-current and
reject v1-v29 callers. ABI v30 adds `INKPOD_IO_REVERT_CURRENT`; it is valid only
...
from exact-current application-settings V4. Preferences accepts 32 through 96
