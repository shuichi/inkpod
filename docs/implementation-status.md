...
| Shared file I/O (`IO-003`) | One application-owned `inkpod-io` manager owns editable native/raster, sequence, reference, Light Table and Batch filesystem work. Windows submits typed paths through ABI v31; Rust owns bounded enumeration, identity, codecs, temporary files and cleanup. Raster-pair candidate proof collects native and raster candidates in one directory pass, while up to 32 notification-backed complete inventories make unchanged-folder revisits enumeration-free. Every attached Sequence retains all source tiles/thumbnails but discards the same pixels' dense decode-cache ownership. Exact-provenance sidecar-less switches reuse the resident tile map and precomputed asset ID with zero read/decode/hash/retile work. Replayed and companion-validated clean sidecar targets use a configurable application-wide 0–1024 MiB LRU (256 MiB default), fixed at 64 targets maximum; exact hits COW-share Core/assets/tiles. Selected members still require complete stamps and final namespace/TOCTOU validation. |
...
| C ABI | Exact-current ABI v31 retains the v30 Revert and all existing role/selection/pair/recovery contracts, and appends `inkpod_io_manager_set_validated_target_cache_bytes` plus six fixed-width validated-target cache fields to the 120-byte `InkpodIoCacheInfo`. The setter accepts 0–1 GiB and invalid input is atomic. Rust/C11/C++ numeric/layout checks reject ABI v30 and older callers. Existing manager/job, snapshot, editor, persistence, Batch v5 and InkScript handles keep their ownership/thread contracts. |
| Windows application data | Ordinary settings have one source of truth at `%LOCALAPPDATA%\inkpod\Settings\inkpod-settings.json`. Exact-current v5 adds bounded `animation.validatedSidecarCacheMiB` (0–1024, default 256, 0 disables) beside Sequence thumbnail width and the existing settings. Apply/OK persists the candidate and changes the live application-wide limit; downstream failure restores both. `.inkshortcuts` remains v3; current-only old-version deletion and invalid/future-file preservation are unchanged. |
| InkScript | Exact-current registry schema/language/file v2, catalog/owner manifest v5, replay epoch 27, native v32 and ABI v31 are aligned. The public catalog has 73 commands; removed document features are tombstones and private Batch v5 orchestration remains excluded. No `.inkscript` product route was added. |
| Windows frontend | Layer/Plane presents one standard layer with MainLine, Color and optional Raster rows; add-plane creates Raster only and conversion changes storage format without changing role. Selection Mask commands manage a document-owned named-mask list. File Open adds a tab, Sequence uses the shared application cache while replacing the active document only after pair preparation, and Revert reconciles pair identity/shell/binding. Batch targets Color/Raster roles through `.inkbatch` v5/ABI v31. |
...
| --- | --- | --- |
| `ABI-001`, `ABI-002` | ABI v31 retains the v30 Revert and existing document/I/O contracts, and adds the bounded validated-target cache setter plus a 120-byte cache-info record. Rust layout/limit/unit contracts, C11/C++ layout, ARM64 Debug build, ABI smoke and explicit previous-version rejection pass. | ABI v1-v30 callers must rebuild; unrun configurations/platform/manual checks remain. |
...
| `IO-003`, `SESSION-001`, `SEQ-001` | All Sequence source tiles/thumbnails remain resident while dense decoder-cache ownership is discarded. Exact sidecar-less target provenance now constructs Genesis/Asset from the resident tiles and precomputed ID with zero physical read, decode, dense copy, hash or full retile, even after decoded-cache clear. A shared validated-sidecar LRU caches at most 64 clean targets under a configurable 0–1024 MiB conservative weight; exact revisit tests prove zero native/raster read/decode and no replay. Existing resolver, authority, recovery, stamp and final TOCTOU fences remain unchanged. ARM64 Debug build, performance product path and both full localized GUI smokes pass. | Unrun configurations/platform/manual/physical checks remain. First visits to existing sidecars still replay/compare before admission. Backends without a strong namespace observer retain uncached enumeration. Two-file filesystem atomicity is not promised. |
| `PERF-001` | Core workflow workloads and `revision-max` are unchanged. The sidecar-less sequence contract now proves resident-tile bytes equal the manager's decoded charge with no dense duplicate, and zero read/decode/copy/hash/full-retile work on a switch after cache eviction. Validated-sidecar revisit proves zero read/decode/replay. LRU count/byte/disable/stamp invalidation and 0–1 GiB ABI bounds are covered. The ARM64 Debug product performance smoke rejects a dense-plus-tiled initial residency and passes its warm zero-read/decode/upload and one-snapshot-per-step gates. | A new multi-process timing series remains to run; first sidecar visit, edited/recovered and background latency stay separate from the warm result. |
...
| `SUBPALETTE-001` | ABI v31 retains the read-only catalog/view and parallel manager jobs. The independent permanent bars, sticky range/reset causes, exact Shift+Arrow/Page pan, unmodified Arrow/Page navigation, stable auxiliary route and catalog `presentation_epoch` are implemented. Visible catalog replacement rejection preserves the prior publication; hidden replacement clears the old retained snapshot before deferral. | Exact post-scroll sampling, durable hidden-publication retry, strict active-image renderer-rejection rollback, page/thumb/resize/accessibility product evidence, interactive file-dialog and unrun platform checks remain; see [correction record](subpalette-first-load-diagnosis.md). |
| `BATCH-001`–`BATCH-004` | Batch v5 resolves Color/Raster role or fixed-ID selectors, deduplicates overlaps and commits one private primitive/Undo unit through ABI v31. | Physical accessibility and unrun platform/configuration checks remain. |
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Catalog/owner v5 with 73 public commands aligns to native32/replay27/ABI31; private Batch v5 orchestration is excluded. | Product `.inkscript` acceptance and the M36 full gate remain pending. |
| `SCRIPT-003` | The private CoreHost route emits current v32/epoch27/ABI31 output through the existing staged authority model. | Product `.inkscript` file acceptance remains absent. |
...
complete stamps, a configurable 0–1024 MiB conservative weight (256 MiB default),
