...
| --- | --- | --- |
| `ABI-001`, `ABI-002` | ABI v23 manager/job lifecycle, path-only requests, bounded progress/result spans and owner-thread publication are unchanged. All 60 Rust ABI tests pass; earlier header/platform-smoke evidence remains a baseline. | ARM64 Debug/x64 Release product/statusbar and ABI gates pass; older ABI callers must rebuild, and x64 Debug/ARM64 Release were not rerun |
| `IO-001`, `IO-003` | Native v29/META v2 companion metadata, shared Rust-manager routes, cached asynchronous progress, bounded cache, ordinary pair save and recovery remain. Fresh raster sequence cells start clean without acquiring save-path authority; no-op/bind and recovery preserve their existing state. All 246 public Core contracts and 60 ABI tests pass. | Standalone renderer repeats fail on both selected configurations; physical file-management gaps remain; no cross-file atomicity promise |
...
### Current canvas, sequence and job-progress checkpoints (2026-08-28)
...
#### Current Core quick timing checks
