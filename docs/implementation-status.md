...
| InkScript | Exact-current registry schema v2 and the approved command-independent language v1 remain unchanged. M07–M14 are user-confirmed. M15 expands the private catalog-v1 draft from 13/84 to 26/84 exact owner entries and connects all paper/frame and document-tree create/duplicate/delete/reorder/merge/delete-hidden/edit-target commands to the single canonical executor. Scalar create results and split typed layer/plane list results resolve in later steps without active-document fallback. Production catalog ratification, Windows authority adapter, ABI, UI and product file acceptance remain absent; the full performance runner remains reserved for M36 and `.inkbatch` v2 is still the production Batch contract. |
...
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Exact-current registry schema v2, language v1, owner manifest, traceability and private-production gate are validated; M07–M14 are user-confirmed. M15 adds the first 13 document-tree owner entries and typed runtime result references, bringing the private draft to 26/84 | M15 user confirmation, remaining owner milestones, and production catalog/ABI/UI gates |
| `SCRIPT-003` | M12 completes the crate-private M09–M12 path through authority-bound multi-item planning, staged execution, deterministic report, current-v26 prospective-savepoint encode/reopen, and injected identity-guarded atomic install. Success/no-op/invalid/cancel/stale/overflow/failure/resource/atomicity plus cache-free replay, Undo/Redo, ID and savepoint parity are covered | Real Windows authority/install adapter, public ABI, product UI, and `.inkscript` file acceptance remain assigned to later milestones |
...
- InkScript M00 is approved and its M01–M14 private language/compiler/runner/performance layers exist. M15 adds the
  document-tree owner family, but `.inkscript` remains unreachable from Core public API/FFI/Windows production. The private
  command draft contains 26/84 M07/M08/M15 owner entries, the real OS adapter installs no product output, and `.inkbatch` v2
...
The latest automatic work is InkScript M15 on 2026-08-15. Its Core-private document-tree adapter and compiler/runner route
cover all 13 owned paper/frame and layer/plane primitives, scalar create results, split typed mixed-target result lists,
strict initial bindings and runtime references, no-op/missing-result/cancel/stale/overflow/resource atomicity, Undo/Redo,
ID high-watermark, current-v26 encode, full-replay reopen and document/editor savepoints. The private draft is now 26/84;
production InkScript remains unreachable. The approved quick gate retained every counter and checksum
`0f84d2c54cfe1e2c` at 89.4783 ms, inside the 64–107 ms envelope, and the existing ten-scenario `core_workflows` quick
checksums remain unchanged. The full fixture remains reserved for M36. The latest full product verification is the M15 x64
Debug regression run from 2026-08-15; the strengthened WIN-001
...
| Boundary | Result |
| --- | --- |
| InkScript registry | Exact-current registry schema v2 accepts language v1, private catalog-v1 draft and owner manifest and rejects superseded schema v1. `inkscript_registry` passes all 10 tests, including closed catalog-owned types, exact 26-entry M07/M08/M15 owner identity/metadata, duplicate/malformed/overflow rejection, version/traceability drift, and private product non-reachability. File v1/catalog v1/epoch 23/native v26/ABI v14 remain aligned |
...
| InkScript M14 | The user-approved quick Release runner retains its 128-step/six-item semantic contract, checksum `0f84d2c54cfe1e2c`, independent-process policy and 64–107 ms x64 envelope without changing the existing benchmark harness. The current M15 run measured 89.4783 ms with every counter exact |
| InkScript M15 | Seven new contracts cover the exact 13-entry owner slice, all bidirectional codecs and direct canonical executor equivalence, typed scalar and mixed layer/plane list results with later-step references, strict/missing target handling, no-op/cancel/stale/overflow/resource atomicity, ownership/thread suitability, Undo/Redo, ID high-watermark, current-v26 encode/full replay, and both savepoints. The private draft is 26/84 and no public/ABI/Windows/product route exists |
| Rust workspace | `HistoryEntryKind` replaces product-facing history labels in Core and maps exhaustively to the fixed-width ABI v14 kind. All 563 tests passed with the Release-only quick gate as the sole ignored test and one doctest included; `fmt`, all-target/all-feature Clippy with warnings denied, workspace strict rustdoc, the ten-test InkScript registry suite, every retained format contract and the resource-split route inventory passed. The inventory also proves that Japanese and English generated resources expose the same 642 resource identifiers, including the same 384 static command IDs |
...
| Windows x64 | The 2026-08-15 Debug `inkpod`, host tests, portable ZIP and unsigned MSIX rebuilt after M15 under MSVC `/W4 /WX` with verified static CRT. All 36 CTests passed on the final binary with desktop permission in 424.47 s: ABI smoke took 60.89 s, English product smoke 174.81 s and Japanese product smoke 175.88 s. M15 adds no Windows/ABI/product route. The latest Release run remains the 2026-08-13 M22 run with all 33 then-current CTests passed in 75.57 s |
