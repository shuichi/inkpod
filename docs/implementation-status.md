...
| Windows frontend       | UI/Input, Core engine, and Renderer are separate long-lived threads. `Application` owns startup/message-loop/shutdown, MainWindow owns message normalization and command routing, feature controllers own typed UI state, and pane/dialog modules use typed inputs/results without receiving the complete application state or Core. The default workspace fixes a compact 20-tool dock on the left, active-tool options across the top, color/palette/chart above layer/plane inspectors on the right, and Canvas in the center. Splitters, visibility, DPI-independent widths/ratios, session restore, explicit save/restore/reset, and left/right mirroring are connected. Layer and plane tiles expose selection, visibility/editability, actions, thumbnails/metadata, and drag reordering while sharing command state with menus. Command routing and command-state catalogs cover every production command exactly once. |
...
| `COLOR-002`          | The fixed right inspector exposes color, palette, and chart tabs with RGBA editing, swatches, group navigation, selection, and retained file/menu/shortcut models | Palette registration/clear/file affordances, subpalette presentation, and optional floating transition |
...
| 2026-07-29 | Windows 11 ARM64, MSVC 19.51, stable Rust            | Rust format, zero-warning clippy, all-feature workspace tests; fresh ARM64 Debug/Release configure/build and MSIX assembly; both default CTest presets; whitespace check | 142 Rust tests passed; strict Debug/Release builds and MSIX assembly passed; Debug and Release CTest each passed 10/10. Pure layout coverage includes 96/144 DPI, narrow fallback, visibility, and mirroring; native smoke covers child-pane geometry, tool dispatch/check state, diameter limits and pencil read-only state, Canvas area recovery, workspace commands, and stable-ID layer/plane synchronization. The administrator-only MSIX install/uninstall smoke remains opt-in and was not run. |
