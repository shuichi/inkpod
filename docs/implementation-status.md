...
The 2026-09-01 CI correction changes only the smoke oracle, not the product
layout rule. The former oracle always required Reference to join the selected
Color/Layer tab. A 96-DPI join needs 748 content pixels, so a 768-pixel hosted
desktop correctly selected SPEC 88's Reference-only fallback tab and was
misreported as `11202`. The probe now computes the settled right-content height,
clones `RightToolTabsModel`, and runs the same public `AddPaneToSelected` input
before issuing the real command. A fitting desktop retains the three-pane
shrink/paint checks. A constrained desktop instead requires a selected singleton
Reference tab, unchanged hidden Color/Layer membership and list state, stable
content `HWND`s, one advanced non-reused tab-ID high-watermark, and exact model,
geometry, focus and visibility restoration after removal. Both paths retain
sentinel clearance, zero list reset, empty relevant update regions and rejected-
callback rollback. Failed add/remove diagnostics now include DPI, work/client and
right heights, required height, model IDs, visibility, geometry, focus, paints,
resets, list state, update regions and sentinel state. The model boundary matrix
now covers both Locator and Reference at 96/120/144/192 DPI for exact fit and one
pixel short.

MSVC x64 Debug `/W4 /WX` and the four focused workspace-layout,
DockHost-boundary, pane-dialog-layout and owner-model tests pass. A temporary
700-pixel outer-height diagnostic run completed the English product smoke in
115.12 seconds; the Japanese run passed this add/remove/rollback probe and later
stopped at the separately tracked visible locator-sampling check `859`. With the
temporary cap removed, the final Japanese product smoke passes in 119.06 seconds.
Two final English runs pass the same-tab probe and then reproduce only the later
visible locator-sampling check `859` on the current desktop.
...
| Windows right-pane resize transaction | On 2026-08-31, the former Color-only target-row/per-control correction was replaced by the shared fixed-capacity pane transaction and the DockHost outer deferral/bounded-union transaction. On 2026-09-01, its product-smoke oracle was corrected to predict the SPEC 88 same-tab/singleton outcome from the settled right-content height and DPI instead of always requiring Color/Layer/Reference to fit together. Both paths compare the complete expected tab model, identity, focus, list/reset, paint, update-region and add/remove/rollback state; Reference exact-fit/one-pixel-short model coverage now spans 96/120/144/192 DPI. MSVC x64 Debug `/W4 /WX` and the focused DockHost-boundary, workspace-layout, pane-dialog-layout and owner-model tests pass. A forced 700-pixel diagnostic completes the English product smoke in 115.12 seconds; its Japanese counterpart passes the corrected probe and then reaches the separately tracked visible locator check `859`. With the cap removed, the final Japanese product smoke passes in 119.06 seconds; two English runs pass the corrected same-tab probe and reproduce only that later `859` on the current desktop. Prior unrestricted English/Japanese product-smoke evidence remains 110.55/113.32 seconds. Outer DockHost OS-level placement/rollback fault injection, physical high-DPI repaint, non-x64-Debug configurations and sequence performance remain pending. |
