...
| --- | --- | --- |
| `BATCH-004` | Multi-row seed/pair authoring, exact two-cell extraction and explicit ambiguity resolution, five typed separation destinations, immutable per-run configuration, current-only persistence, ABI and Windows production routes | x64 Release user-visible behavior confirmation |
...
- Native `.inkpod` is current-only v15; non-v15 files are rejected without a
...
- V15 accepts compression code 0 only; measured checkpoint behavior has not
...
The latest complete M07 automatic verification is dated 2026-08-10. The
recommended exact two-cell extraction, explicit ambiguity, typed separation, and
immutable per-run configuration semantics are fixed in `SPEC.md`; x64 Release
manual behavior confirmation is pending.

| Boundary | Result |
| --- | --- |
| Rust workspace | 379 tests including one doctest, zero ignored; `fmt`, all-target/all-feature Clippy with warnings denied, and strict rustdoc passed |
| Native format | V15/runtime replay epoch 12 and `.inkbatch` v2 current-only round-trip/noncurrent rejection; scoped replacement and typed separation procedure/replay; selection-destination save/reopen Undo/Redo; EDIT schema-4 persistence; bounded streaming, checkpoint match/fallback/rejection, staged open, failed replacement, recovery, and compaction contracts passed |
| Windows ARM64 | Not rerun for M07; ARM64 remains optional and does not substitute for the completed x64 Release gate |
| Windows x64 | 2026-08-10 Release configure/build completed with warnings denied and static CRT; `ctest --preset windows-x64-release` passed all 30 tests, including loaded per-run preset cloning, two-cell extraction, typed separation, owned ABI results, production Batch/UI/Core/renderer smoke, portable ZIP, and MSIX payload routes; manual behavior confirmation remains pending |
| Performance | All nine quick and full checksum, revision, history, reuse/rebuild, payload-access, and failure-atomicity gates passed with the unchanged workload, harness, envelopes, and `revision-max` formula. After discarded warm-ups, five-process medians were quick/full: batch preview 0.5295/2.1454 ms, canonical replay 0.8435/0.8042 ms, checkpoint open 10.9187/33.6897 ms, dirty rebuild 1.2710/6.0401 ms, and pan/zoom 0.6430/9.0939 ms. This x64 host does not match the approved ARM64 Parallels envelope, so wall-clock is diagnostic and no envelope was changed |
| Fuzzing | `native_v15` and `native_core_v15` target declarations are current. Fuzz binary build and coverage-guided execution were not run for M07 because the optional `cargo fuzz` subcommand is outside the required gate |
