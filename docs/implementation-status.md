...
| 2026-07-29 | Windows 11 ARM64, MSVC 19.51, stable Rust            | Rust format, zero-warning clippy, all-feature workspace tests; ARM64 Debug build; fresh ARM64 Release configure/build and MSIX assembly; both default CTest presets; whitespace check | 142 Rust tests passed; strict Debug/Release builds and MSIX assembly passed; Debug and Release CTest each passed 10/10. Pure layout coverage includes 96/144 DPI, narrow fallback, visibility, and mirroring; native smoke covers child-pane geometry, tool dispatch/check state, Canvas area recovery, workspace commands, and stable-ID layer/plane synchronization. The administrator-only MSIX install/uninstall smoke remains opt-in and was not run. |
