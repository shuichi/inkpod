...
On 2026-08-26, an application-wide tab/label audit enumerated the document-tab,
DockHost header-tab, Color-page, and Preferences-page families. Document and
DockHost tabs are header strips and have no overlaid page labels. Color and
Preferences now share one helper that reproduces the live themed tab client at
each overlaid label's actual coordinates. Preferences applies it to every
unframed `STATIC` across all six pages; framed value fields retain their separate
system-window background. The x64 Release build passed under `/W4 /WX`, followed
by `inkpod_windows_dock_host_boundaries`, `inkpod_windows_workspace_layout`, and
`inkpod_windows_owner_model`. English and Japanese product smokes passed in 65.14
and 68.64 seconds. They compare corresponding tab/label background pixels and
also retain the prior minimum-size and completed-paint Color checks. The Windows
automation helper could launch the product but did not expose a targetable
window; that inspection-owned untitled process was identified by its exact x64
Release path and terminated before the successful rebuild, so no new manual GUI
claim is recorded.
...
| Windows Preferences | On 2026-08-26, x64 Release rebuilt under MSVC `/W4 /WX`. The 920-by-680-DIP dialog uses current-DPI 9-point Segoe UI plus measured text-height padding on all six pages; its four keyboard rows, instruction, status, and detail controls are smoke-checked for containment and non-overlap. Every unframed page `STATIC` paints the corresponding live themed tab pixels with transparent text, while client-edge values retain the independent system-window surface. Page changes, resizes, theme changes, and system-color changes invalidate the tab and visible children. Seven localized Shortcut label groups retain measured-width checks. Complete English/Japanese product smokes passed in 65.14/68.64 seconds and traverse every page while requiring exact representative tab/label background-pixel equality. The prior complete ARM64 Debug baseline remains applicable to unchanged behavior outside this x64 presentation correction. Color and Layer/Plane pane rows retain the same measured-height rule. |
