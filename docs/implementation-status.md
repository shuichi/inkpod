...
| `IO-003`, `SESSION-001`, `SEQ-001` | Shared Rust-manager routes, recoverable pair save, one File Open/Sequence/Revert resolver, exact native-composite/raster matching, three-state authority, direct clean first Save, non-silent corrupt/mismatch handling, exact sequence-recovery authority, BIND rekey/rebase and exact-path/UUID Revert retention are implemented without a serialized path or native-version change. Native/raster companion candidates are collected together; Windows notification-backed bounded inventories make unchanged-folder revisits enumeration-free while retaining add/delete/rename and final member validation. Initial discovery and individual-cell progress are distinct, and a switch completed before the progress poll produces no loading flash. Current x64 product regressions cover exact-pair reuse/conflict, atomic BIND publication, Committed/Planned explicit Save As, post-publication failure revocation/retry and same-path/UUID multi-view Revert. | Unrun configurations/platform/manual/physical checks remain. Backends without an equally strong namespace observer deliberately retain uncached enumeration. Two-file filesystem atomicity is not promised. |
| `PERF-001` | Core workflow workloads and `revision-max` are unchanged. Raster-pair resolution reduces cold candidate enumeration from six passes to three and the Windows warm unchanged-folder contract records zero additional `read_dir` calls; add/delete/rename, clear and 32-directory LRU tests protect invalidation. The current v32/catalog-v5 InkScript quick semantic gate passes with all non-byte counters unchanged, checksum `1e41e17e8bda22e3` and 88.9261 ms inside the existing 64–107 ms envelope. | Sidecar replay and sidecar-less Genesis/Asset materialization remain unchanged; sharing those pixels safely requires manager/stamp provenance and cache-free save/replay parity. The native-scrollbar route must still demonstrate zero raster payload access for cache-hit line/page/thumb pan. The current Windows sequence/renderer retries cannot acquire foreground and do not produce an accepted timing sample. A full current multi-process timing series, initial/cache-miss and background latency remain outside this run; prior approved measurements remain historical evidence. |
...
checks. The remaining sequence-performance test stops at its desktop gate before
measurement: both the ordinary run and an explicit foreground-request retry
reported `foreground=0` and Canvas `visible=0`. ARM64 Debug builds successfully;
its DockHost/status/localization/ABI/static-CRT focused set passes 5/5.
...
after package regeneration. Of the non-product tests, only sequence performance
cannot run its presentation gate: even with foreground requested, Windows reports
`foreground=0` and the Canvas remains `visible=0`. This is recorded under
`PERF-001`, not as a raster-pair protocol failure.
