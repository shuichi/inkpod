...
| `WORKSPACE-001` | Windows retains its eight DockHost surfaces, presets, V7 bounded layout, monitor/DPI recovery, AutoHide pointer access, and accessible names. macOS now has maximum-two nonrecursive EditorGroups, an adaptive three-state glass Tool surface, one five-tab inspector with follow/pin, five presets, and bounded named layout-v2 records with deterministic v1 migration, malformed recovery, and display clamp | Windows Reference Check AutoHide edge buttons are not reachable by F6/Tab/Shift+Tab; physical multiple-display macOS interaction is unverified |
...
95 strict Swift 6 unit/integration tests plus the separate 64-session headless
executable, and validated the value-only CoreHost boundary. V-MacUI passed all
17 selected tests: eight product Canvas lifecycle tests and nine launched-product
XCUITests. The new coverage exercises one five-tab inspector, toolbar/menu/
shortcut routes, product accessibility audit, 640/800/1200-point resize and
reversible adaptation, stale inspector reopening, unchanged document/history/
dirty state, half-open backing-pixel bounds, and 200 resize/snapshot ownership
...
All 613 required String Catalog keys have complete English and Japanese values,
...
| macOS arm64 | CMake→Cargo→Xcode M12 automatic gate passed on macOS 26.6.1 with Xcode 26.6/Tahoe SDK. All 95 strict Swift tests, the 64-session headless executable, eight product Canvas lifecycle tests, nine launched-product XCUITests including accessibility audit, 384-command parity with all rows implemented, 613-key en/ja localization, CoreHost and release source boundaries, fault/no-op/cancel/stale/failure coverage, 200-cycle Metal ownership/reuse metrics, prior M2–M11 regressions, and CTest 22/22 passed. V-MacUI passed 17/17 and V-Metal reported zero validation errors |
