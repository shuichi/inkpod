...
| InkScript | M00's closed registry meta-schema, command-independent language v1 registry, empty private catalog-v1 draft, all-84-replayable-primitive owner manifest, and Batch→SCRIPT→milestone→evidence traceability are approved. M01 provides immutable UTF-8 source/line-map/diagnostic/lexer APIs; M02 provides the Core-independent lossless CST and bounded exact-current parser. M03 provides the semantic AST/canonical emitter and M04 the typed complete-file orchestration/path-intent envelope. M05A supplies generated language types, immutable typed declarations and per-run values. M05B extends the bounded test `SchemaView` with scalar/list result metadata, integrates typed step/group/result and all-reference dependency edges into the same declaration model, and closes selected step/group fragments without importing outside mutations: an outside stable result must become an explicit strict binding or fail, while symbol/group collision rewriting is deterministic and atomic. Selector/assert execution, catalog implementation, Core execution, authority/plan, ABI, UI and product file acceptance do not exist yet; the private draft still has zero entries and `.inkbatch` v2 remains the current Batch contract. |
...
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | M00 language/meta schemas, owner manifest, traceability and private-production gate are approved; M01 source/lexer, M02 lossless CST/bounded parser, M03 semantic AST/canonical emitter, M04 typed orchestration, M05A type/namespace/parameter, and M05B typed step/result/dependency models are implemented and automatically validated | M05B binary regression confirmation, then M06 selector/assert/catalog interface and later owner milestones |
...
- InkScript M00 is approved and its M01 lexical, M02 lossless-parser, M03 semantic/canonical, and M04 typed-envelope Rust APIs exist, but `.inkscript` remains
...
The latest automatic verification is for InkScript M05B on 2026-08-15. Five public contracts validate
bounded test-schema scalar/list results, typed steps/groups, every parameter/binding/result/asset edge,
source-bound field/index/cardinality/availability/overflow/resource diagnostics, external-producer
rejection or explicit strict binding, asset deduplication, deterministic alpha-renaming, canonical
emit/reparse and failure atomicity. The architecture suite proves that Core, FFI and Windows production
do not reach the new model. The four M05A type, four M04 envelope, six M03 semantic, thirteen M02 parser,
twelve M01 lexer, and seven M00 registry contracts remain green; lexer/parser fuzz targets still type-check,
...
| Rust workspace | `HistoryEntryKind` replaces product-facing history labels in Core and maps exhaustively to the fixed-width ABI v14 kind. All 512 tests, including one doctest and zero ignored, passed; `fmt`, all-target/all-feature Clippy with warnings denied, workspace strict rustdoc, the seven-test InkScript registry suite, lexer/parser fuzz target check, and the resource-split route inventory passed. The inventory also proves that Japanese and English generated resources expose the same 642 resource identifiers, including the same 384 static command IDs |
...
| Windows x64 | The 2026-08-15 Debug `inkpod`, host tests, portable ZIP and unsigned MSIX rebuilt after M05B under MSVC `/W4 /WX` with verified static CRT. All 36 CTests passed on the final binary with desktop permission in 419.29 s: ABI smoke took 58.38 s, English product smoke 170.71 s and Japanese product smoke 177.70 s. M05B has no Windows/ABI route. The latest Release run remains the 2026-08-13 M22 run with all 33 then-current CTests passed in 75.57 s |
