...
| `COLOR-002`          | The fixed right inspector exposes color, palette, and chart tabs with RGBA editing, swatches, group navigation, selection, and retained file/menu/shortcut models | Palette registration/clear/file affordances and subpalette presentation |
...
| 2026-07-29 | Windows 11 ARM64, MSVC 19.51, stable Rust            | Rust format, zero-warning clippy, all-feature workspace tests; fresh ARM64 Debug/Release configure/build and MSIX assembly; both default CTest presets; whitespace check | 142 Rust tests passed; strict Debug/Release builds and MSIX assembly passed; Debug and Release CTest each passed 10/10. Pure layout coverage includes the 80-DIP tool dock at 96/144 DPI, narrow fallback, visibility, and mirroring; native smoke covers readable single-word labels for all 20 tools, child-pane geometry, tool dispatch/check state, diameter limits, pencil read-only state, Canvas-focus commit, Canvas area recovery, workspace commands, and stable-ID layer/plane synchronization. The administrator-only MSIX install/uninstall smoke remains opt-in and was not run. |
