...
| Windows frontend | UI/Input, Core engine, and Renderer are separate owner threads connected by bounded value/ownership queues. Multiple windows, sessions, views, split groups, single-instance activation, recovery, workspace presets, target-aware panes, and device-loss reconstruction are connected without a process-global active-document pointer. |
...
| `WIN-001` | Native Windows shell, offline Help/About, DPI-aware layout, keyboard routes, MSAA/UIA names, theme and accessibility hooks | High contrast, 200% DPI, complete screen-reader, and Japanese IME validation/fixes; the current display resources are Japanese-only |
| `WORKSPACE-001` | Dock/float/hide/restore, five presets, persisted bounded layout, monitor/DPI recovery, AutoHide pointer access and accessible names | Reference Check AutoHide edge buttons are not reachable by F6/Tab/Shift+Tab |
