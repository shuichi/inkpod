...
| --- | --- | --- |
| `PAINT-004` | Round/square pressure-aware brush, fixed-point smoothing, exact immutable native-depth start-color mode, canonical Core/ABI/Windows route, v13/epoch-10 persistence, goldens and smoke | x64 Release narrow two-color repair, shape/smoothing comparison, Cancel and Undo manual verification |
...
- Native `.inkpod` is current-only v13; non-v13 files are rejected without a
...
- V13 accepts compression code 0 only; measured checkpoint behavior has not
...
The latest complete M05 automatic verification is dated 2026-08-10. The M04
raster-selection option manual check was confirmed before M05 began.

| Boundary | Result |
| --- | --- |
| Rust workspace | 364 tests including one doctest, zero ignored; `fmt`, all-target/all-feature Clippy with warnings denied, and strict rustdoc passed |
| Native format | V13/runtime replay epoch 10 round-trip, v12 rejection, EDIT schema-4 brush/selection/target persistence, bounded streaming, checkpoint match/fallback/rejection, inactive-branch asset retention, staged open, failed replacement, recovery, and exact-confirmation compaction contracts passed |
| Windows ARM64 | Not rerun for M05; ARM64 remains optional and does not substitute for the completed x64 Release gate |
...
| Performance | All nine quick and full checksum, revision, history, reuse/rebuild, and payload-access gates passed with the unchanged workload, harness, envelopes, and `revision-max` formula. Quick medians were 1.2683 ms dirty rebuild and 0.7605 ms pan/zoom; full medians were 6.0177 ms and 9.4844 ms. Native warm-up plus five x64 processes retained 512 wheel events/Presents and 16 strokes/544 samples/16 final Presents; medians were 8,534,237,600 ns wheel and 266,889,200 ns drawing. This Ryzen 9 9950X3D/RTX 5090 x64 host at 59 Hz is not the approved ARM64 Parallels/120 Hz envelope, so wall-clock is diagnostic and no envelope was changed |
| Fuzzing | `native_v13` and `native_core_v13` target declarations are current. Fuzz binary build and coverage-guided execution were not run for M05 because the optional `cargo fuzz` subcommand is outside the required gate |
