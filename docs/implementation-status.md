...
| InkScript | Exact-current registry schema v2 and the approved command-independent language v1 remain unchanged. M07–M09 are user-confirmed and the private catalog-v1 draft remains 13/84 exact owner entries. M09 provides the crate-private compiler/binder/single-input staged runner. M10 adds crate-private bounded decode of inline Base64 and caller-authorized streams, approved descriptor/resource validation, two-phase immutable freeze into the existing Rust-owned asset store, exact resource counters, and typed asset-role/portability summaries. External path authority and ScriptExecutionPlan connection remain M11 work; production catalog ratification, multi-input/output install, ABI, UI and product file acceptance remain absent, and `.inkbatch` v2 is still the production Batch contract. |
...
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | Exact-current registry schema v2, language v1, owner manifest, traceability and private-production gate are validated; M07–M09 are user-confirmed. M10 adds approved asset descriptor/resource enforcement, bounded canonical ingestion, immutable freeze and typed role/portability metadata without changing the private 13-entry catalog | M10 existing-binary non-regression/non-reachability confirmation, then the remaining owner milestones and production catalog/ABI/UI gates |
| `SCRIPT-003` | One captured in-memory or exact-current native-byte input can be checked, bound and executed on a staged Core. Inline Base64 and caller-authorized asset streams can be atomically validated and frozen into the existing immutable Core asset store with exact cancellation/stale/resource accounting | External path authority and ScriptExecutionPlan connection, multi-input tasks, output persistence/install, ABI and product report UI remain unimplemented |
...
- InkScript M00 is approved and its M01–M08 language/adapter layers, M09 private compiler/binder/single-input staged runner,
  and M10 private asset ingestion/freeze layer exist, but `.inkscript` remains unreachable from Core public API/FFI/Windows
  production, its private command draft contains only the 13 M07/M08 owner entries, no output is installed, and `.inkbatch`
  v2 remains the current Batch format until the M29C/M34 gates are separately accepted.
...
The latest automatic verification is for InkScript M10 on 2026-08-15. The approved language-v1 asset descriptor now
enforces exactly one of inline `data` and external `data_file`. The private Core ingestion layer accepts either inline
Base64 or a bounded caller-authorized stream carrying an opaque verified identity; it does not interpret or retain a
path, OS authority token, or handle. It validates descriptor semantics, declared canonical `AssetId`, logical length,
duplicates, per-asset and aggregate limits, cancellation, stream identity before and after reads, truncation, extra
bytes, read failure, and malformed read counts. All records are built in a local existing `AssetStore` and exposed only
after complete success as Rust-owned immutable assets. Exact declaration/unique/logical/read/decode/copy counters and a
typed asset-role/portability-summary foundation are available privately. Four Core contracts and one format contract
cover inline/authorized parity, no-op, invalid, stale, cancellation, resource and ownership/thread behavior; partial
asset, commit, and output publication remain impossible because no production plan or command is connected. M09's
compiler/staged runner is now user-confirmed; the private catalog remains 13/84, and M10 awaits the same existing-binary
Batch non-regression and InkScript UI non-reachability confirmation. The latest full product verification is the M10
x64 Debug regression run from 2026-08-15; the strengthened WIN-001
...
| InkScript M10 | Four Core-private asset contracts and one format public-model contract cover exact `data`/`data_file`, inline/authorized canonical identity and bytes, no-op, descriptor/digest/length/duplicate validation, truncation/extra data, individual and aggregate limits, cancellation, stale identity, reader failures, exact read/copy counters, typed role policy, immutable Rust ownership and product non-reachability. The route is crate-private and awaits existing-binary non-regression/non-reachability confirmation |
| Rust workspace | `HistoryEntryKind` replaces product-facing history labels in Core and maps exhaustively to the fixed-width ABI v14 kind. All 547 tests, including one doctest and zero ignored, passed; `fmt`, all-target/all-feature Clippy with warnings denied, workspace strict rustdoc, the nine-test InkScript registry suite, every format fuzz target check, and the resource-split route inventory passed. The inventory also proves that Japanese and English generated resources expose the same 642 resource identifiers, including the same 384 static command IDs |
...
| Windows x64 | The 2026-08-15 Debug `inkpod`, host tests, portable ZIP and unsigned MSIX rebuilt after M10 under MSVC `/W4 /WX` with verified static CRT. All 36 CTests passed on the final binary with desktop permission in 405.02 s: ABI smoke took 57.50 s, English product smoke 167.21 s and Japanese product smoke 167.48 s. M10 adds no Windows/ABI/product route. The latest Release run remains the 2026-08-13 M22 run with all 33 then-current CTests passed in 75.57 s |
