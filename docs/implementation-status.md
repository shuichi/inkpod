...
| InkScript | M00's closed registry meta-schema, command-independent language v1 registry, empty private catalog-v1 draft, all-84-replayable-primitive owner manifest, and Batch→SCRIPT→milestone→evidence traceability are approved. M01 provides immutable UTF-8 source/line-map/diagnostic/lexer APIs; M02 provides the Core-independent lossless CST and bounded exact-current parser. M03 adds a trivia-free semantic AST, build-generated language-schema projection, bounded private `SchemaView`, canonical file/fragment emitter and deterministic generated-name allocator. Canonical output uses schema field order and normalized values while the lossless writer remains byte-perfect for BOM, CRLF, comments and literal spelling. No typed orchestration, Core executor, ABI, UI, or product file acceptance exists, and `.inkbatch` v2 remains the current Batch contract. |
...
| `SCRIPT-001`, `SCRIPT-002`, `SCRIPT-005` | M00 language/meta schemas, owner manifest, traceability and private-production gate are approved; M01 source/lexer, M02 lossless CST/bounded parser, and M03 semantic AST/schema-driven canonical emitter/generated-name basis are implemented and automatically validated | M03 binary regression confirmation, then M04 typed orchestration and later owner milestones |
| `SCRIPT-003`, `SCRIPT-004` | Normative planning/execution/persistence/export/rebind/clipboard contracts and owner milestones are defined | No implementation exists yet |
...
- InkScript M00 is approved and its M01 lexical, M02 lossless-parser, and M03 semantic/canonical Rust APIs exist, but `.inkscript` remains
  unreachable from production, its command draft is empty, and `.inkbatch` v2 remains the
...
The latest automatic verification is for InkScript M03 on 2026-08-15. Six public semantic/canonical
tests validate file/fragment AST round-trip, registry-derived section/field order, all literal classes,
default/`none` omission, deterministic declaration order and generated names, invalid CST isolation,
closed bounded schema composition, unknown-command non-fallback, and lossless/canonical writer separation.
The thirteen M02 parser, twelve M01 lexer, and six M00 registry tests remain green; lexer/parser fuzz
targets still type-check, and `.inkscript` still has no product route. The
...
| Boundary | Result |
| --- | --- |
| InkScript M00 | Draft 2020-12 validation accepts the language, draft and owner registries; `inkscript_registry` passes all 6 tests: JSON/meta closure, duplicate/malformed/overflow rejection, language references, owner bijection, version/traceability drift, and private-draft unreachability. File v1/catalog v1/epoch 23/native v26/ABI v14 remain aligned; the draft has zero command entries and no production parser/executor/ABI/UI route |
...
| Rust workspace | `HistoryEntryKind` replaces product-facing history labels in Core and maps exhaustively to the fixed-width ABI v14 kind. All 498 tests, including one doctest and zero ignored, passed; `fmt`, all-target/all-feature Clippy with warnings denied, workspace strict rustdoc, the six InkScript registry tests, and the resource-split route inventory passed. The inventory also proves that Japanese and English generated resources expose the same 642 resource identifiers, including the same 384 static command IDs |
...
| Windows x64 | The 2026-08-15 Debug `inkpod`, host tests, portable ZIP and unsigned MSIX rebuilt after M03 under MSVC `/W4 /WX` with verified static CRT. All 36 CTests passed in 428.32 s; the final incremental build's ABI smoke passed again in 57.78 s, while English product smoke passed in 174.13 s and Japanese product smoke in 182.49 s. M03 has no Windows/ABI route. The latest Release run remains the 2026-08-13 M22 run with all 33 then-current CTests passed in 75.57 s |
