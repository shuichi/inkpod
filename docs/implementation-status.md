...
| Windows frontend | Right-side Splitter geometry updates now preserve existing Common Controls tab items and pane data, skip unchanged child placements, and repaint only the resized Color picker instead of erasing every child; Color and Layer child clipping prevents sibling overdraw during live resize. Width-dependent Layer/Plane owner-draw rows paint synchronously once after their list geometry changes, so right-aligned status cells cannot leave their old frame visible between pointer samples. Structural dock changes retain the full refresh path. The Batch pane retains its editable fixed Input/Output graph and four-operation catalog. Color Replace exposes standard checkboxes for general-raster, binary-coloring, and grayscale-coloring targets; one or more may be selected together, all selectors round-trip through `.inkbatch` v4/ABI v22, and externally loaded fixed-ID selectors remain intact until semantic selection replaces them. Preview/run/save still builds one immutable graph, and the existing singleton tab, generated Japanese/English presentation, cancellable preview, issue-time targeting, and Rust-owned staged-result routes remain. |
| Preferences and shortcuts | `編集 > 環境設定` opens one resizable, owner-centered six-tab dialog for all persisted application/workspace preferences. Its Keyboard Shortcut tab projects the 329-command catalog into grouped/filterable command and physical-key views, supports two slots, action/context/matching metadata, editable copies of the complete built-in preset, conflict repair, layout selection, and current-only `.inkshortcuts` import/export. Exact conflicts remain editable but disable Apply/OK; prefix conflicts are rejected. Runtime input resolves Global/Canvas/Timeline/Pane plus logical/physical strokes, restores temporary Hold tools on key-up/deactivation, suppresses Toggle key-repeat, and installs no global hook. The former language and shortcut-reset menu commands are no longer separate product routes. |
...
| `WORKSPACE-001` | Persistent and auxiliary panes retain DockHost and Tool Options remains an owned flyout. The right side uses dynamic stable-ID tabs with nonempty unique membership, deterministic add/remove/move/reorder, accessible descriptions and Workspace V9 persistence/migration from V2–V8. Live Splitter geometry updates do not rebuild right-side tabs or Color/Layer list contents, while structural dock changes still refresh their presentation. The transient Job Progress pane and transient narrow-width suppression are not persisted. | Reference Check AutoHide edge buttons are not reachable by F6/Tab/Shift+Tab |
...
On 2026-08-25, the right-side Splitter live-resize correction rebuilt the x64
...
99.93-second English product smoke
...
Layer owner-draw list has no deferred paint left behind. A separate x64 Release
GUI check dragged the live Splitter left and right and showed no stale Layer/Plane
status-cell frames. Rust Core, C ABI, native/replay, workspace persistence, and
the approved benchmark contract are unchanged.
...
| Windows Preferences | On 2026-08-25, ARM64 Debug rebuilt `inkpod`, `inkpod_windows_command_state_tests`, and `inkpod_windows_shortcut_profile_tests` under MSVC `/W4 /WX`. All 43 CTest entries pass on the final binary: the 36 non-product-smoke tests completed in 72.30 s, English/Japanese product smokes in 285.13/288.13 s, and release-automation/static-CRT/MSIX/portable-ZIP checks passed after regenerating the ZIP from the final executable. Coverage includes command routes/state ownership, modal positioning, localization/catalog, default-profile completeness/conflict freedom, context/physical resolution, action metadata, exact/prefix conflict policy, and current-only preset-codec rejection. |
| Windows x64 | Exact-current v28/ABI v22 x64 Debug executable and packaging outputs rebuilt under `/W4 /WX`; focused Batch/localization CTest, static CRT verification, and both localized product smokes pass. The complete Debug matrix and Release build/tests remain to be rerun for v22; prior v21 evidence is historical |
| Windows ARM64 | Exact-current v28/ABI v22 ARM64 Debug has a complete 43-test pass for the final Preferences binary, including ABI, localized product smokes, static CRT, MSIX, and portable ZIP. ARM64 Release remains at the prior v21 evidence and still requires a v22 rerun |
