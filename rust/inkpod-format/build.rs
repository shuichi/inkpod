...
fn generate() -> Result<(), String> {
    ...
    if string(member(&catalog_root, "kind")?)? != "inkpod.inkscript.catalog"
        ...
        || command_count != 75
        || catalog_fingerprint != FROZEN_CATALOG_V8_FNV1A64
    {
        return Err(format!(
            "production catalog v8 identity/freeze mismatch: version={catalog_version}, commands={command_count}, fingerprint={catalog_fingerprint:016x}"
        ));
    }
