...
fn result_field_index_availability_and_cardinality_fail_with_source_ranges() {
    ...
    let valid_list = analyze(
        b"inkscript_fragment 1; requires { procedure_catalog = 1; replay_epoch = 23; } program { step \"Create\" as made { enabled = true; invoke create_test {}; } step \"Use\" { enabled = true; invoke use_path_list_test { paths = $made.paths; }; } }",
    )
    .unwrap();
...
fn result_schema_and_unknown_commands_fail_closed_without_debug_name_fallback() {
    ...
    let input = source(
        b"inkscript_fragment 1; requires { procedure_catalog = 1; replay_epoch = 23; } program { step \"Unknown\" { enabled = true; invoke rust_debug_variant {}; } }",
    );
