...
requires { replay_epoch = 28; procedure_catalog = 6; }
"#,
    );
...
fn exact_current_versions_and_complete_file_boundary_fail_closed() {
    ...
    assert_eq!(INKSCRIPT_PROCEDURE_CATALOG_VERSION, 6);
    assert_eq!(INKSCRIPT_REQUIRED_REPLAY_EPOCH, 28);

    let noncurrent_file = source(
        b"inkscript 1; requires { procedure_catalog = 6; replay_epoch = 28; } inputs {} program {} output { policy = explicit_overwrite; format = inkpod; } execution { failure = stop; wait_ms = 0; preview_before_save = true; }",
    );
    assert!(!parse_inkscript(&noncurrent_file).is_valid());

    for (requires, expected) in [
        (
            "procedure_catalog = 1; replay_epoch = 28;",
            InkScriptEnvelopeErrorCode::NonCurrentProcedureCatalog,
        ),
        (
            "procedure_catalog = 6; replay_epoch = 23;",
...
fn metadata_ranges_and_execution_bounds_reject_invalid_values_atomically() {
    ...
    for (meta, expected) in [
        ...
        let text = format!(
            "inkscript 2; requires {{ procedure_catalog = 6; replay_epoch = 28; }} {meta} inputs {{ file \"a.inkpod\"; }} program {{}} output {{ {base_output} }} execution {{ {base_execution} }}"
        );
    ...
    for (inputs, expected) in [
        ...
        let text = complete_file(
            "procedure_catalog = 6; replay_epoch = 28;",
            inputs,
            base_output,
            base_execution,
        );
...
fn output_variants_are_closed_and_explicit_overwrite_intents_are_typed() {
    let common_requires = "procedure_catalog = 6; replay_epoch = 28;";
