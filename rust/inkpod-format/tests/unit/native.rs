...
#[test]
fn io_001_v20_directory_digest_and_opaque_sections_round_trip() {
...
#[test]
fn io_001_v20_accepts_checkpoint_and_rejects_v19_missing_duplicate_overlap_and_bad_digest() {
    let file = procedure_file_fixture();
    let encoded = encode_procedure_file(&file).unwrap();

    let mut v19 = encoded.clone();
    v19[8..12].copy_from_slice(&19_u32.to_le_bytes());
    assert!(matches!(
        decode_procedure_file(&v19),
        Err(FormatError::Unsupported("format version is not supported"))
    ));
...
#[test]
fn io_001_v20_streaming_cancel_keeps_existing_destination_and_removes_temp() {
    let directory = std::env::temp_dir().join(format!(
        "inkpod-v20-cancel-test-{}-{}",
        std::process::id(),
...
#[test]
fn io_001_v20_atomic_save_replaces_an_existing_container() {
    let directory = std::env::temp_dir().join(format!(
        "inkpod-v20-replace-test-{}-{}",
        std::process::id(),
        TEMP_SEQUENCE.fetch_add(1, Ordering::Relaxed)
    ));
    fs::create_dir(&directory).unwrap();
