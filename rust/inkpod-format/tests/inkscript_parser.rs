...
fn public_parser_rejects_noncurrent_versions_without_a_compatibility_reader() {
    for bytes in [
        ...
        &b"inkscript 4; requires {} inputs {} program {} output {} execution {}"[..],
...
"#,
    ...
    assert_eq!(
        diagnostic_codes(b"inkscript_fragment 3; requires {}"),
...
fn parser_truncation_corpus_is_deterministic_and_always_lossless() {
    let valid = b"inkscript_fragment 3; requires { x = [1, 2]; } program { assert ready {}; }";
...
fn parser_diagnostic_recovery_stops_at_the_caller_lowered_limit() {
    ...
    let source =
        source(b"inkscript_fragment 3; requires {} requires {} requires {} program {} program {}");
...
fn parser_enforces_section_and_declaration_limits_without_truncation() {
    let cases = [
        ...
        (
            ...
            b"inkscript 3; requires {} inputs { current_document; current_sequence; } program {} output {} execution {}".as_slice(),
            InkScriptDiagnosticCode::InputLimitExceeded,
        ),
        (
            InkScriptParserLimits::exact_current().with_parameter_limit(1),
            b"inkscript_fragment 3; requires {} parameters { param a: u32 = 1; param b: u32 = 2; } program {}".as_slice(),
            InkScriptDiagnosticCode::ParameterLimitExceeded,
        ),
        (
            InkScriptParserLimits::exact_current().with_binding_limit(1),
            b"inkscript_fragment 3; requires {} bindings { let a = select layer {}; let b = select layer {}; } program {}".as_slice(),
            InkScriptDiagnosticCode::BindingLimitExceeded,
        ),
        (
            InkScriptParserLimits::exact_current().with_program_statement_limit(1),
            b"inkscript_fragment 3; requires {} program { assert a {}; assert b {}; }".as_slice(),
...
fn editor_groups_are_nonempty_and_must_form_one_contiguous_run() {
    assert_eq!(
        diagnostic_codes(
            br#"inkscript_fragment 3; requires {} program {
