    ...
    fn from_code(value: u32) -> Result<Self, FormatError> {
        match value {
            ...
            _ => Err(FormatError::Unsupported(
                "unknown M4 light-table display mode",
            )),
        }
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct FileM4Metadata {
    pub active_set_id: u64,
    pub sets: Vec<FileLightTableSet>,
}

pub(super) fn encode_m4_metadata(metadata: &FileM4Metadata) -> Result<Vec<u8>, FormatError> {
    validate_m4_metadata(metadata, None)?;
...
pub(super) fn decode_m4_metadata(bytes: &[u8]) -> Result<FileM4Metadata, FormatError> {
    let mut reader = Reader::new(bytes);
    if reader.take(4)? != b"M4WF" || reader.u32()? != 1 {
        return Err(FormatError::Unsupported(
            "M4 metadata version is not supported",
        ));
    }
    let active_set_id = reader.u64()?;
    let set_count = reader.u32()? as usize;
    if reader.u32()? != 0 || set_count == 0 || set_count > MAX_LIGHT_TABLE_SETS {
        return Err(FormatError::Invalid("M4 set count is outside bounds"));
    }
    ...
    for _ in 0..set_count {
        ...
        if reader.u32()? != 0 {
            return Err(FormatError::Unsupported(
                "M4 set reserved field is not zero",
            ));
        }
        total_items = total_items
            .checked_add(item_count)
            .ok_or(FormatError::Invalid("M4 item count overflows"))?;
        if item_count > MAX_PLANES || total_items > MAX_LIGHT_TABLE_ITEMS {
            return Err(FormatError::Invalid("M4 item count is outside bounds"));
        }
        ...
        for _ in 0..item_count {
            ...
            let source_document_uuid: [u8; 16] = reader
                .take(16)?
                .try_into()
                .map_err(|_| FormatError::Invalid("M4 source UUID is truncated"))?;
            ...
            if flags & !1 != 0 {
                return Err(FormatError::Unsupported("unknown M4 item flags"));
            }
    ...
    if reader.position != bytes.len() {
        return Err(FormatError::Invalid("M4 metadata has trailing bytes"));
    }
    let metadata = FileM4Metadata {
        active_set_id,
        sets,
    };
    validate_m4_metadata(&metadata, None)?;
...
fn read_name(reader: &mut Reader<'_>, length: usize) -> Result<String, FormatError> {
    if length == 0 || length > MAX_NODE_NAME_BYTES {
        return Err(FormatError::Invalid("M4 name length is outside bounds"));
    }
    let text = std::str::from_utf8(reader.take(length)?)
        .map_err(|_| FormatError::Invalid("M4 name is not valid UTF-8"))?;
    if text.chars().any(char::is_control) {
        return Err(FormatError::Invalid("M4 name contains control characters"));
    }
    Ok(text.to_owned())
}

pub(super) fn validate_m4_metadata(
    metadata: &FileM4Metadata,
    ...
    for set in &metadata.sets {
        ...
        if set.id == 0 || !ids.insert(set.id) || set.global_opacity_milli > 1_000 {
            return Err(FormatError::Invalid("M4 set properties are invalid"));
        }
        ...
        if item_count > MAX_LIGHT_TABLE_ITEMS {
            return Err(FormatError::Invalid("M4 item count exceeds its bound"));
        }
        for item in &set.items {
            ...
            {
                return Err(FormatError::Invalid("M4 item properties are invalid"));
            }
        }
    }
    if !active_found {
        return Err(FormatError::Invalid("M4 active set ID is invalid"));
    }
    if source_plane_ids.is_some_and(|planes| planes != &referenced_planes) {
        return Err(FormatError::Invalid(
            "M4 item and light-table plane IDs differ",
        ));
    }
    Ok(())
}
...
fn validate_name(name: &str) -> Result<(), FormatError> {
    if name.is_empty() || name.len() > MAX_NODE_NAME_BYTES || name.chars().any(char::is_control) {
        Err(FormatError::Invalid("M4 name is invalid"))
    } else {
        Ok(())
    }
}
