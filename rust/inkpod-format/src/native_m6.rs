...
pub struct FileM6Metadata {
    pub adjustments: Vec<FileAdjustmentLayer>,
}

pub(super) fn encode_m6_metadata(metadata: &FileM6Metadata) -> Result<Vec<u8>, FormatError> {
    validate_m6_metadata(metadata, None)?;
...
pub(super) fn decode_m6_metadata(bytes: &[u8]) -> Result<FileM6Metadata, FormatError> {
    let mut reader = Reader::new(bytes);
    if reader.take(4)? != b"M6AD" || reader.u32()? != 1 {
        return Err(FormatError::Unsupported(
            "M6 metadata version is not supported",
        ));
    }
    ...
    if count > MAX_ADJUSTMENT_LAYERS {
        return Err(FormatError::Invalid(
            "M6 adjustment count exceeds its bound",
        ));
    }
    ...
    for _ in 0..count {
        ...
        if point_count > MAX_CURVE_POINTS {
            return Err(FormatError::Invalid(
                "M6 curve point count exceeds its bound",
            ));
        }
        let parameters = [
            reader.i32()?,
        ...
        for _ in 0..point_count {
            ...
            points.push(CurvePoint {
                input: input
                    .try_into()
                    .map_err(|_| FormatError::Invalid("M6 curve input exceeds 16-bit"))?,
                output: output
                    .try_into()
                    .map_err(|_| FormatError::Invalid("M6 curve output exceeds 16-bit"))?,
            });
        }
    ...
    if reader.position != bytes.len() {
        return Err(FormatError::Invalid("M6 metadata has trailing bytes"));
    }
    let metadata = FileM6Metadata { adjustments };
    validate_m6_metadata(&metadata, None)?;
    Ok(metadata)
}

pub(super) fn validate_m6_metadata(
    metadata: &FileM6Metadata,
    adjustment_layer_ids: Option<&BTreeSet<u64>>,
) -> Result<(), FormatError> {
    if metadata.adjustments.is_empty() || metadata.adjustments.len() > MAX_ADJUSTMENT_LAYERS {
        return Err(FormatError::Invalid(
            "M6 adjustment count is outside bounds",
        ));
    }
    ...
    for layer in &metadata.adjustments {
        ...
        {
            return Err(FormatError::Invalid("M6 adjustment properties are invalid"));
        }
    }
    if adjustment_layer_ids.is_some_and(|expected| expected != &ids) {
        return Err(FormatError::Invalid("M6 adjustment layers are incomplete"));
    }
    Ok(())
}
...
fn parse_channel(value: u32) -> Result<Channel, FormatError> {
    match value {
        ...
        3 => Ok(Channel::Green),
        4 => Ok(Channel::Blue),
        _ => Err(FormatError::Unsupported("M6 channel is unknown")),
    }
}
...
fn parse_interpolation(value: u32) -> Result<CurveInterpolation, FormatError> {
    match value {
        ...
        _ => Err(FormatError::Unsupported(
            "M6 curve interpolation is unknown",
        )),
    }
}
...
fn bounded_u16(value: i32) -> Result<u16, FormatError> {
    value
        .try_into()
        .map_err(|_| FormatError::Invalid("M6 level value exceeds 16-bit"))
}
