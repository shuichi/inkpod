...
const DIGEST_CONTEXT: &str = "org.inkpod.cut-descriptor.v1";
static TEMP_SEQUENCE: AtomicU64 = AtomicU64::new(1);

/// Runtime replay epoch for the current Cut canonical procedure semantics.
pub const CUT_DESCRIPTOR_REPLAY_EPOCH: u32 = 19;
...
/// One ordered reference to an independently saved Cell `.inkpod` in the same directory.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct FileCutMember {
    pub cell_id: u64,
...
/// One canonical metadata/defaults edit. Member paths never occur in procedures.
#[derive(Clone, Debug, Eq, PartialEq)]
...
pub fn encode_cut_descriptor(descriptor: &FileCutDescriptor) -> Result<Vec<u8>, FormatError> {
    ...
    for member in &descriptor.members {
        push_u64(&mut payload, member.cell_id);
        payload.extend_from_slice(&member.document_uuid);
        push_u32(&mut payload, member.display_number);
        push_text(&mut payload, &member.relative_path, MAX_PATH_BYTES)?;
    }
...
fn validate_descriptor(descriptor: &FileCutDescriptor) -> Result<(), FormatError> {
    ...
    for member in &descriptor.members {
        if member.cell_id == 0
            ...
            || member.relative_path.len() > MAX_PATH_BYTES
            || !identities.insert((member.cell_id, member.document_uuid))
            || !paths.insert(member.relative_path.as_str())
        {
            return Err(FormatError::Invalid(
                "Cut member identity is invalid or duplicated",
            ));
        }
    }
...
fn decode_history(reader: &mut Reader<'_>) -> Result<FileCutHistoryEntry, FormatError> {
    Ok(FileCutHistoryEntry {
        procedure_id: reader.u64()?,
        base_state_id: reader.u64()?,
        committed_state_id: reader.u64()?,
        before_metadata: decode_metadata(reader)?,
        before_defaults: decode_defaults(reader)?,
        after_metadata: decode_metadata(reader)?,
        after_defaults: decode_defaults(reader)?,
    })
}
