...
fn vector_line(
    ...
    let dx = (end.0 - start.0) / 3.0;
    let dy = (end.1 - start.1) / 3.0;
    VectorPathInput {
...
fn vector_rectangle_at(
    ...
    for pair in points.windows(2) {
        segments.extend(vector_line(pair[0], pair[1], 1.0, 1.0, color).segments);
    }
    VectorPathInput {
        segments,
        color,
...
#[test]
fn color_replace_001_vector_line_uses_whole_stable_path_coverage_and_fill_is_explicit() {
    let mut core = Core::new();
    ...
    let (_, layer_id) = core
        ...
        .unwrap();
    let (main_id, trace_id, fill_plane_id) = core.vector_layer_planes(layer_id).unwrap();
    let old = PixelValue::Rgba([10, 20, 30, 255]);
    let new = PixelValue::Rgba([90, 80, 70, 200]);
    let (_, touched_id) = core
        .vector_add_path(
            trace_id,
            vector_line((2.0, 5.0), (28.0, 5.0), 4.0, 8.0, old),
        )
        .unwrap();
    let (_, untouched_id) = core
        .vector_add_path(
            trace_id,
            vector_line((2.0, 20.0), (28.0, 20.0), 2.0, 2.0, old),
        )
        .unwrap();
    let (_, boundary_id) = core
        .vector_add_path(trace_id, vector_rectangle_at(10.8, 10.0, 18.0, 18.0, old))
        .unwrap();
    let (_, fill_id) = core
        .vector_add_fill(fill_plane_id, &[boundary_id], old)
        .unwrap();
    ...
    let preview = core.preview_scoped_color_replace(&line).unwrap();
    assert_eq!(
        preview.matched_objects, 1,
        "variable-width coverage touches the region"
    );
    core.apply_scoped_color_replace(line).unwrap();
    let paths = core.vector_paths().unwrap();
    assert_eq!(
        paths
            .iter()
            .find(|path| path.id == touched_id)
            .unwrap()
            .color,
        new
    );
    assert_eq!(
        paths
            .iter()
            .find(|path| path.id == untouched_id)
            .unwrap()
            .color,
        old
    );
    ...
    assert_eq!(
        core.preview_scoped_color_replace(&fill)
            .unwrap()
            .matched_objects,
        1,
        "fill interior entering a region cell counts even when its center is outside"
    );
    core.apply_scoped_color_replace(fill).unwrap();
    assert_eq!(
        core.vector_fills()
            .unwrap()
            .into_iter()
    ...
    assert!(matches!(
        core.apply_scoped_color_replace(request(
            &core,
            main_id,
            ScopedColorReplaceMode::VectorColorLine,
            old,
