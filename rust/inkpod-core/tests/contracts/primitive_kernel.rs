...
fn canonical_execution_and_fresh_replay_are_bit_exact_at_each_primitive_boundary() {
    ...
    assert_eq!(main_line_procedure.replay_epoch().get(), 14);
    ...
    assert_eq!(palette_procedure.replay_epoch().get(), 14);
    ...
    assert_eq!(stroke_procedure.replay_epoch().get(), 14);
    ...
    let composite = runtime
        ...
        .unwrap()
        .as_bytes();
    assert_eq!(contract.replay_epoch().get(), 14);
    assert_eq!(contract.procedure_format_version(), 17);
    assert_eq!(contract.canonical_numeric_version(), 1);
    assert_eq!(contract.primitive_count(), 79);
    assert_eq!(
        *contract.primitive_catalog_digest(),
        [
            172, 119, 7, 20, 57, 54, 43, 108, 229, 44, 205, 223, 3, 192, 219, 14, 124, 31, 237,
            115, 201, 190, 61, 157, 22, 171, 1, 133, 109, 11, 98, 200
        ]
    );
    assert_eq!(
        boundary_digests,
