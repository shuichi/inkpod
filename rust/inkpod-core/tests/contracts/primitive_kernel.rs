...
fn canonical_execution_and_fresh_replay_are_bit_exact_at_each_primitive_boundary() {
    ...
    assert_eq!(main_line_procedure.replay_epoch().get(), 12);
    ...
    assert_eq!(palette_procedure.replay_epoch().get(), 12);
    ...
    assert_eq!(stroke_procedure.replay_epoch().get(), 12);
    ...
    let composite = runtime
        ...
        .unwrap()
        .as_bytes();
    assert_eq!(contract.replay_epoch().get(), 12);
    ...
    assert_eq!(
        *contract.primitive_catalog_digest(),
        [
            252, 158, 33, 101, 215, 189, 11, 123, 138, 80, 0, 185, 50, 60, 104, 166, 198, 241, 78,
            57, 133, 138, 29, 49, 146, 95, 129, 12, 105, 131, 11, 226
        ]
    );
    assert_eq!(
        boundary_digests,
