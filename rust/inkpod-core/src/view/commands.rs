...
impl Core {
    ...
    pub(super) fn apply_view_for_document_size(
        ...
        let (view, clear_render_cache) = apply_view_state(self.view, command, document_size)?;
        self.view = view;
...
/// Shared OS-neutral view math for editable and reference-only image views.
pub(crate) fn apply_view_state(
    view: ViewState,
    command: ViewCommand,
    document_size: DocumentSizeU32,
) -> Result<(ViewState, bool), CoreError> {
    let command_viewport = match command {
        ...
        | ViewCommand::ViewportResized {
            ...
            DeviceSizeF64::new(viewport_width, viewport_height)
                .map_err(|_| CoreError::InvalidArgument("view command contains invalid values"))?,
        ),
        ViewCommand::BoxZoom {
            viewport_width,
            viewport_height,
    ...
    if matches!(
        ...
        return Ok((toggle_candidate, clear_render_cache));
    }
    ...
    let (next_zoom, next_pan, next_mode) = match command {
        ViewCommand::PanBy {
            ...
            let pan = DeviceOffsetF64::new(view.pan.x + device_dx, view.pan.y + device_dy)
                .map_err(|_| result_outside_range())?;
            (view.zoom, pan, ViewMode::Manual)
        }
        ViewCommand::ZoomAt {
            factor,
            device_x,
            device_y,
        } if factor.is_finite() && factor > 0.0 && device_x.is_finite() && device_y.is_finite() => {
            let device_point = DevicePointF64::new(device_x, device_y)
                .map_err(|_| CoreError::InvalidArgument("view command contains invalid values"))?;
            let document_point = device_to_document(view, document_size, device_point);
            let zoom = ZoomFactor::clamped(view.zoom.get() * factor)
                .map_err(|_| result_outside_range())?;
        ...
        ViewCommand::Fit {
            ...
            .map_err(|_| result_outside_range())?;
            let pan =
                centered_pan(document_size, viewport, zoom).map_err(|_| result_outside_range())?;
        ...
        ViewCommand::ViewportResized {
            ...
            ViewMode::Manual => (view.zoom, view.pan, ViewMode::Manual),
            ViewMode::Fit => {
        ...
        ViewCommand::BoxZoom {
            ...
            .map_err(|_| result_outside_range())?;
            let pan =
                box_zoom_pan(document_rect, viewport, zoom).map_err(|_| result_outside_range())?;
            (zoom, pan, ViewMode::Manual)
        }
        _ => {
            return Err(CoreError::InvalidArgument(
    ...
    if next_zoom != view.zoom || next_pan != view.pan || next_mode != view.mode {
