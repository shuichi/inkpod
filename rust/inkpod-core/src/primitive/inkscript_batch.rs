...
mod tests {
    ...
    fn unknown_field_enum_and_nonexact_filter_variant_are_rejected() {
        let prefix = format!(
            "inkscript_fragment 3; requires {{ procedure_catalog = 8; replay_epoch = 29; }} bindings {{ let target_plane = select plane {{ source_document_uuid = uuid\"{ADAPTER_SOURCE_UUID}\"; persistent_id = 12; }}; }} "
        );
