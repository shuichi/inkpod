            ...
            match edge.consumer().kind() {
                ...
                InkScriptDependencyNodeKind::Step => {
                    if let Some(index) = edge.consumer().step_index() {
                        if model.steps()[index as usize].command() == "apply_batch_operations"
                            && !super::batch::uses_runtime_dependency(
                                &frozen_arguments[index as usize],
...
"#,
        ...
        let ambiguous = model(
            r#"inkscript_fragment 3; requires { procedure_catalog = 8; replay_epoch = 29; }
...
program {{
    ...
    assert object {{ target = $target; name = "A"; visible = true; editable = true; }};
...
program {}"#,
        ...
        let empty = model(
            "inkscript_fragment 3; requires { procedure_catalog = 8; replay_epoch = 29; } program {}",
        );
        assert_eq!(
