use super::m7::{inkpod_batch_task_create, inkpod_batch_task_query, inkpod_batch_task_release};
use super::*;
...
fn ffi_contract_vector_filter_and_task_state_machines() {
    ...
    unsafe {
        ...
        assert_eq!(task_info.state, INKPOD_M6_TASK_READY);
...
fn ffi_contract_public_surface_matches_header_and_every_function_has_a_test_reference() {
    ...
    let library = read(&repository.join("rust/inkpod-ffi/src/lib.rs"));
    let m7 = read(&repository.join("rust/inkpod-ffi/src/m7.rs"));
    let contract_tests = read(&repository.join("rust/inkpod-ffi/src/ffi_contract_tests.rs"));
    let cpp_tests = read(&repository.join("tests/abi_smoke.cpp"));

    let header_names = names_followed_by_parenthesis(&header);
    let mut implementation_names = exported_function_names(&library);
    implementation_names.extend(exported_function_names(&m7));
    assert_eq!(
        implementation_names, header_names,
        "public header declarations and no_mangle Rust exports drifted"
    );

    let inline_tests = library
        .find("#[cfg(test)]\nmod tests")
        .map_or("", |start| &library[start..]);
    let m7_inline_tests = m7
        .find("#[cfg(test)]\nmod tests")
        .map_or("", |start| &m7[start..]);
    let mut referenced = names_followed_by_parenthesis(inline_tests);
    referenced.extend(names_followed_by_parenthesis(m7_inline_tests));
