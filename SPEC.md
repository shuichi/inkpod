...
- exact-current 契約は `.inkpod` top-level format v28、runtime replay epoch 25、C ABI v21、`.inkbatch` v3、InkScript registry schema／language／file v2、production catalog／owner manifest v4 とする。native v27／epoch 24、ABI v20以前、`.inkbatch` v2、catalog／owner manifest v3、および削除済み Batch authoring operation は migration や shim を設けず拒否する。今回の更新は native format freeze 宣言ではない。
