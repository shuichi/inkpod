...
- 通常保存、自動保存、recovery、export は別 status とし、初回の連番探索と個別セルの pair 解決／読み込みも別 status とする。短時間で完了した cache-hit cell switch を連番全体の再読み込みとして表示しない。自動保存成功だけで通常 savepoint、document path authority、dirty 表示を進めない。
...
- exact-current 契約は `.inkpod` top-level format v32、runtime replay epoch 27、C ABI v30、`DocumentArchive` schema 7、必須 `DOCM` schema 8、`DocumentStateDigest` schema 12/domain 10、snapshot-composite schema 5、Cut payload schema 3／Cut replay epoch 25、`.inkbatch` v5／operation schema 4、InkScript registry schema／language／file v2、73-command production catalog／owner manifest v5 とする。native v31以前、epoch 26以前、ABI v29以前、Cut payload schema 2以前、`.inkbatch` v4以前、catalog／owner manifest v4以前、および削除済み layer-kind／Selection-plane／VanishingPoint／Adjustment-layer contract は migration や shim を設けず拒否する。ABI v26 は共通 raster-pair open の public I/O kind、ABI v27 は sequence recovery が committed／planned／repair-needed／authority-none の exact runtime pair proof を伝達する固定幅 kind、ABI v28 は pair 保存の公開開始後に runtime 保存 authority を失効した terminal failure を frontend へ伝える固定幅 result flag、ABI v29 は clean savepoint でも journal／Redo branch／EditorState generation が最後に表現されたartifactより新しければsource recoveryを必須にするissue-time sequence request flagを追加した。ABI v30 は `INKPOD_IO_REVERT_CURRENT` を追加し、`INKPOD_IO_OPEN_NATIVE` と force reload の組合せだけで受理する。apply 時に発行対象と同じ current native path／document UUID を要求し、成功時は runtime sequence を保持する。pair path/authority、runtime-only sequence preservation baseline と Revert flag は native 文書へ永続化しないため native format v32 と replay epoch 27 は変更しない。今回の更新は native format freeze 宣言ではない。
...
- 大画像は sparse tile、遅延割当、copy-on-write を基本とし、Undo、snapshot、Light Table、checkpoint のために画像全体を無条件に複製しない。snapshot は変更 tile だけを再合成し、pan、zoom、flip、viewport 変更では既存の合成済み tile と renderer resource を再利用する。sidecar のない Sequence pair target は、最終 decoded raster と catalog source が同じ manager、complete stamp、generation、format、metadata、allocation であると runtime に証明できる場合だけ、immutable dense payload と tile map を COW 共有して Genesis/Asset を構築する。証明できない場合は同じ意味結果を返す owned import へ戻す。この最適化分類は pair authority と resolver-proven target の pristine 再登録を変更しない。
...
- `SEQ-001`: cut/cell sequence、共通 companion resolution、`BIND` rekey／binding rebase、target 固有 pair authority、Revert 中の runtime catalog／inactive recovery 保持、自然順の前後セル、欠番、thumbnail preview
