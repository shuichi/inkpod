...
- ユーザーがフォーマットフリーズを宣言するまで、`.inkpod`、`.inkbatch`、`.inkshortcuts`、`inkpod-settings.json`、session record、native preset 等の application 固有の永続化形式は現在 version だけを読み書きし、下位互換 reader/writer、migration、互換 shim を持たない。現在の要件に対して最も頑健で効率的な schema を選ぶ。
