...
- exact-current 契約は `.inkpod` top-level format v32、runtime replay epoch 27、C ABI v31、`DocumentArchive` schema 7、必須 `DOCM` schema 8、`DocumentStateDigest` schema 12/domain 10、snapshot-composite schema 5、Cut payload schema 3／Cut replay epoch 25、`.inkbatch` v5／operation schema 4、InkScript registry schema／language／file v2、73-command production catalog／owner manifest v5 とする。native v31以前、epoch 26以前、ABI v30以前、Cut payload schema 2以前、`.inkbatch` v4以前、catalog／owner manifest v4以前、および削除済み layer-kind／Selection-plane／VanishingPoint／Adjustment-layer contract は migration や shim を設けず拒否する。ABI v26 は共通 raster-pair open の public I/O kind、ABI v27 は sequence recovery が committed／planned／repair-needed／authority-none の exact runtime pair proof を伝達する固定幅 kind、ABI v28 は pair 保存の公開開始後に runtime 保存 authority を失効した terminal failure を frontend へ伝える固定幅 result flag、ABI v29 は clean savepoint でも journal／Redo branch／EditorState generation が最後に表現されたartifactより新しければsource recoveryを必須にするissue-time sequence request flagを追加した。ABI v30 は `INKPOD_IO_REVERT_CURRENT` を追加した。ABI v31 は application-wide validated sidecar target cache の 0～1 GiB setter と fixed-width usage counters を追加する。pair path/authority、runtime-only sequence preservation baseline、Revert flag、cache entry／limit は native 文書へ永続化しないため native format v32 と replay epoch 27 は変更しない。今回の更新は native format freeze 宣言ではない。
