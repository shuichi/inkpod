...
本文の機能節が操作の詳細契約を定め、メニュー節と要件 ID はその入口・要約とする。詳細契約を変更するときは要約と参照先も照合する。本文、要約、専門文書、既存 test が食い違う場合は、`AGENTS.md` の判断規則に従って文書の誤記、実装不具合、未確定の製品判断を区別し、優先順位だけを理由に期待値や製品挙動を変えない。現在有効な専門契約と公開 gate は参照先の指定範囲に限って適用し、過去の工程記録や作業 prompt の参照を作業再開・承認とみなさない。
...
- `保存`: 文書を通常の native/raster pair へ保存する。保存先の決定、競合・busy・書き出し省略の条件は [第4節](#4-起動終了読み込みセル切替保存)に従う。
- `名前を付けて保存`: 新しい native/raster pair へ保存する。対象確認と保存後の状態は [第4節](#4-起動終了読み込みセル切替保存)に従う。平坦化出力は `書き出し > ラスター` で行う。
- `復帰`: 現在文書を最後に通常保存した文書・履歴へ戻す。対象確認、維持する view／Sequence と失敗時の状態は [第4節](#4-起動終了読み込みセル切替保存)に従う。
...
- `色領域外を選択`: [出力色安全ガード](#12-彩色チェックとモーションチェック)の条件外の色を選択 mask にする。正式な放送規格への適合判定は行わない。
...
- 表示切替は `ツールパレット`、`ツールオプション` flyout、`カラー`、`レイヤー／プレーン`、`ロケーター`、`シーケンス`、`ライトテーブル`、`サブパレット／参照ビュー`、`バッチ` の九つを Window menu 直下の直接 checked toggle とする。menu、shortcut、pane control は同じ command ID と checked state を使い、checked state は dock／floating／AutoHide を含む実可視性、`ツールオプション` は flyout の可視性を表す。Color の文書固定／追従 command は Window menu に重複配置せず、pane の target control から操作する。Batch は [第19節](#19-バッチ処理)に従って job 発行時の target を固定し、pane の固定／追従 control は持たない。
...
- 自動保存済みセルは sequence entry の document UUID と source generation に関連付け、resident hit の直接交換には recovery replay を挟まない。recovery artifact から戻す経路では exact native state を staged Core で検証・replay してから active Core を交換する。flattened preview source から history、layer/plane tree、selection、editor state を再構成しない。通常 pair authority を持つ source の recovery metadata は capture 時の両 member の complete stamp または normalized missing-path identity を exact proof として保持する。戻る時は同じ共通 companion resolver が現在の pair を再解決し、UUID、canonical Genesis、raster source identity、capture 時 proof がすべて一致するときだけ、その target 固有の `Committed`（companion 欠落の repair-needed を含む）または `Planned` runtime authority を recovery の exact history／EditorStateへ再採用する。coherent な外部 pair 保存を含む差替え、欠落 member の出現、stamp／missing identity 不一致は原子的な conflict とする。authority `None` の untitled／明示 raster import／standalone recovery は proof `None` のまま pathless に復元し、`名前を付けて保存`を要求する。
- 別の `(document UUID, source generation)` へ実際に切り替えたときは、旧セルの通常保存先、source path、recovery association、file identity を新セルへ流用しない。同じ UUID でも source generation が異なれば実切替とする。target は共通 companion resolver が返した target 固有の `Committed`、`Planned`、または明示 conflict result を採用する。valid sidecar があれば exact history／EditorState を staged replayし、sidecar がなければ clean Genesis と planned pair を採用する。Core は no-op、現在の画像への初回の連番関連付け、実際の文書置換を判別し、`NOOP`／`BIND` だけなら文書と旧保存 authority を維持する。Core replacement の commit 前は、target 解決、identity 予約、source の保存／recovery 判断に成功するまで旧セルとその authority を保持し、取消、失敗、stale、queue rejection では target state を公開しない。resident hit の切替後 autosave と、commit 後の snapshot／presentation failure は [横断的な状態遷移契約](#横断的な状態遷移契約)の別境界に従う。
...
- 通常 raster-pair destination state は `Planned`、`Committed`、`None` の三つだけとする。`Committed` を持つ文書の `保存` は同じ native/raster pair を使う。`Planned` を持つ文書の `保存` は destination dialog を開かず、open 時に固定した両 target の identity／missing-path proof を再検証して最初の pair を materialize する。`None` の文書の `保存` だけが通常保存先を得るため destination dialog へ進む。完全な clean `Committed` pair は、両 member の identity／complete stamp と外部競合を再検査した後に限り、実装が物理的な再書き出しを論理的に省略してよい。省略は必須ではなく、通常の pair transaction を検証して両 member を再書き出してもよい。
...
- 保存は temp file 完成後の置換とし、元 file を先に truncate しない。通常 native/raster pair の部分 install・復旧・authority の失効は [共有 I/O](#41-共有ファイル-io-と編集参照の分離)と [原子性の確定単位](#横断的な状態遷移契約)に従う。起動時は全 recovery 候補を列挙し、一件ずつ復元/破棄/保留を選べるようにして、silent に捨てない。通常の前回文書復元は layout と crash recovery から分離した既定 off の明示設定とする。document session を閉じる時も、まだ通常保存で退役していない inactive sequence cell の append-only recovery を自動削除しない。live sequence association は終了するが、artifact と metadata は起動時の standalone recovery 候補として残し、authority `None` の pathless 文書として exact history／EditorState を復元できるようにする。対象 cell の通常保存成功または利用者がその候補を明示破棄した場合だけ exact proof 付きで削除する。
- `名前を付けて保存` は任意の編集文書から明示実行でき、成功時に pair logical identity registry、title、recent files、recovery metadata を一つの transaction として更新する。保存先 pair のいずれかの identity または missing path が別の open/planned session と競合する場合は上書きや silent merge をせず、明示的な解決を求める。保存先は新しい `.inkpod` と同一 stem の raster companion の組とする。単体 raster や別形式への平坦化出力は `書き出し > ラスター` の責務であり、この command の保存先にはしない。
- `復帰` は現在の `Committed` pair の native path を対象に、`INKPOD_IO_OPEN_NATIVE` に force reload と current-document Revert の両 flag を付ける専用要求とし、通常の強制 open を Revert と推測しない。Core は発行時の current path と document UUID の完全一致を apply 時にも再検証し、成功時は native に保存された document/history/editor/savepoint を採用する一方、runtime-only の sequence catalog、active index、全 live view の stable ID／論理 state と次の view ID を保持する。旧 document revision に属する render cache は破棄して再構築する。frontend は Core apply 後に pair logical identity、shell path、active `SequenceFileBinding` と sequence pane projection を新しい owner generationへ一回で再公開し、inactive recovery association を破棄しない。Core apply 後の snapshot／presentation failure は適用済み Revert を再実行せず、authority と UI を適用済み状態へ reconciliation してから error を報告する。別 path、別 UUID、pair conflict、取消、stale は現在文書と frontend authority を変更せず失敗する。
...
- IO は bounded worker/queue で非同期・並列に実行し、同じ物理 file と replacement destination の read/write は排他する。file alias と置換後の file identity 変更を扱う。live Core は owner thread に保持し、staged decode/replay/encode 結果だけを受け取る。Core apply 前の stale/cancel/failure は別 active target へ fallback せず、文書と既存表示を保持する。Core apply 後の snapshot／presentation failure は [原子性の確定単位](#横断的な状態遷移契約)に従い、確定済み文書を巻き戻さない。
...
- 基準 frame は用紙左上を原点とした、選択 anchor に対応する基準点の X/Y と、左上/右上/中央/左下/右下の簡易配置を持つ。drag と数値のどちらでも移動できる。
...
InkScript file v3 の入出力は明示 `inputs.profile = batch | canonical`（省略は canonical）を持ち、[入力契約](INKSCRIPT.md#73-inputs)に従って順序、range、重複、open snapshot の意味を保持する。共通codecの `.inkpod`／PNG／TIFF／TGA／BMP、非再帰folder、発行時active入力を共有Rust I/O managerで扱う。[出力契約](INKSCRIPT.md#79-output)のclosed native naming／folder／active_document／new_tabsを使い、activeは単一Batch transactionと両savepoint・path authorityを保持し、新規tabは新identity／pathless／dirtyとする。[画像preview](INKSCRIPT.md#821-画像-preview-と-staged-publication)は全入力を隔離後に保存・再読込し、4 GiB／16,777,216 pixelsの上限とcleanup-before-publication、元targetの固定を守る。ABI v35とWindows private engineは同じ共有Rust I/Oとstaged publicationを使用し、発行時context・owner thread・take/releaseを固定する。製品Batch切替やfile filter／pane／公開commandへの接続はM15の明示cutoverまで行わない。
...
次表を要求する exact-current 値の一覧とする。宣言・registry は実装との照合先であり、値の不一致を仕様変更として自動採用しない。版を更新するときは対応する宣言・registry、旧版拒否 test、[形式文書](docs/file-format.md)、[FFI](docs/ffi.md)、[InkScript 契約](INKSCRIPT.md)と生成 reference の該当箇所を照合する。歴史的な検証記録の版を現行値へ書き換えない。

| 契約 | 現行値 | 宣言・registry の照合先 |
| --- | --- | --- |
| `.inkpod` top-level format | 34 | [format](rust/inkpod-format/src/procedure.rs) の `FORMAT_VERSION`、[Core model](rust/inkpod-core/src/primitive/model.rs) の `PROCEDURE_FORMAT_VERSION` |
| runtime replay epoch | 29 | [Core model](rust/inkpod-core/src/primitive/model.rs) の `ReplayEpoch::CURRENT` |
| C ABI | 35 | [公開 header](include/inkpod/core_ffi.h) の `INKPOD_ABI_VERSION` |
| `DocumentArchive`／必須 `DOCM` schema | 7／9 | [native model](rust/inkpod-format/src/native/model.rs) の `DOCUMENT_ARCHIVE_VERSION`／`DOCUMENT_METADATA_VERSION` |
| `DocumentStateDigest` schema／domain | 13／11 | [digest](rust/inkpod-core/src/primitive/digest.rs) の `DOCUMENT_STATE_SCHEMA_VERSION`／`DOCUMENT_STATE_CONTEXT` |
...
| `ApplySelection` semantics | 4 | 同 catalog の `APPLY_SELECTION` |

- 非 current version と、廃止した Cut descriptor・指示画像書き出しの契約は migration や shim を設けず拒否する。旧 ABI の呼び出し元は現行 header での再 build を必須とする。Cut handle／records／functions、Cut 判別と指示画像の I/O flags、撮影 frame の指示 export flag は存在せず、入力・永続化・canonical procedure に指示 export flag を含めない。
- 通常 Sequence、pair authority、resident bank、render preparation は runtime-only とする。native format は未フリーズとする。線補正 request・task API と2種類の raster line tool を提供し、背景判定・成分全体判定・inclusive gap・探索前仮想境界・線幅 mode を上記の canonical replay semantics へ固定する。
...
- フォーマットフリーズまでは、serialized schema を変更するたびに対象形式の最上位 format version を必ずインクリメントする。section/record version だけの変更で代用せず、旧 version は明示的に拒否する。フリーズはユーザーの明示宣言を条件とし、コードの整理や文書更新だけで成立しない。
...
版更新の適用関係は次のとおりとする。複数に該当する変更では必要な更新を併用し、現行値の表記訂正や意味を変えない文書整理だけでは版を上げない。

| 変更 | 必須の版更新・確認 |
| --- | --- |
| application 固有形式の serialized schema | 対象形式の最上位 format version。該当 section／record schema の更新だけで代用しない |
| 同じ canonical invocation から得る replay 結果 | replay epoch と `.inkpod` top-level format。該当 primitive の schema／semantics と catalog への影響も確認する |
| 公開 C ABI の symbol／record／呼出契約 | C ABI version、公開 header、FFI 契約と非 current caller の拒否を同時更新する |
| InkScript grammar／command／selector／binding | [InkScript の版更新表](INKSCRIPT.md#1-文書の位置付け)に従い file／catalog／replay の影響を判定し、未解除の公開 gate を保持する |
| runtime-only authority／cache の意味を保つ変更 | serialized schema／replay への影響がなければ永続化版は更新しない。性能の変更承認条件は別に適用する |
...
- 本文書が数値や内部表現を一意に定めない場合は、対応する専門契約（数値は [determinism.md](docs/determinism.md)）と既存の公開契約 test を確認する。その契約内の内部実装は安全性、頑健性、効率、決定性に基づいて決める。画素結果、選択集合、Undo 単位、保存・replay 結果、公開 API の意味を新たに定める必要がある場合は、`AGENTS.md` の判断規則に従って提案する。確定した数値契約は式、rounding、fixture と観測可能な期待結果を対応する正本へ記録する。
- 永続化schemaに影響する決定は、前節のフォーマットフリーズ前version規則に従う。

#### 未決事項と変更の境界

以下は文書の参照・整合修正だけでは製品判断を確定しない。判断対象に依存しない既存契約の修正・検証は進められる。

| 対象 | 維持する境界 | 製品判断が必要な内容・解除条件 |
| --- | --- | --- |
| Sequence の resident miss | resident hit の即時交換、未公開 target の検証、未保存編集と identity／authority の保護。既存の非同期 recovery 経路は [architecture.md](docs/architecture.md) を参照し、この未決事項を理由に追加・削除しない | 即時拒否と非同期復元の適用条件、待機中の表示・入力。hit／miss／proof 不一致、取消・失敗時の観測結果を示して方針を確定する |
| pair conflict 時の明示 raster import | 通常 open は conflict を拒否し、sidecar を silent に無視しない。明示 import は `None` の pathless 文書とし、既存 pair を保存先に採用しない | 独立 command、Open 内の明示操作、Core-only のどの境界に置くか。現行 menu には独立 command を設けず、利用者向け導線の確定後に menu と I/O 契約を同時更新する |
| `revision-max` の既知制約の是正 | 現在の式、payload 非走査、metadata invalidation、workload／counter／承認済み envelope と、後述する正しさ保証の制限 | 是正する範囲、再現ケースと期待画像、意味 counter、同条件の before/after と全 sample を示す。式・harness・envelope 等の変更は明示承認後に行う |
...
- no-op は document revision、`StateId`、history、journal、persistent ID、dirty、render content を変えない。invalid、cancel、stale revision、overflow、失敗は、対象となる確定単位の commit 前に document、history、journal、ID、dirty、revision、確定 snapshot を部分公開しない。file installation と commit 後の失敗は次表の境界に従い、確定済みの別 transaction や出力まで巻き戻すという意味ではない。
...
- 長時間処理は base revision、cancel、target generation を確定し、対象となる確定単位の全計算と検証が成功した場合だけ結果を公開する。

操作の所有者・route 分類は [architecture.md](docs/architecture.md#primitive-route-and-journal-target-contract) に従う。原子性の確定単位と、失敗時に保持する状態は次のとおりとする。

| 操作・確定単位 | 確定境界 | 取消・失敗時の状態 |
| --- | --- | --- |
| 一つの document edit | canonical executor の明示 commit | commit 前は元 document、history、journal、ID、dirty、revision を保持する。成功した一 edit は一回の Undo 単位とする |
| history control／EditorState／view-only／preview | 各 route 固有の apply、または preview の確定・取消 | document edit と混同せず、各 route が所有する状態だけを変更する。EditorState の persisted revision／dirty は [形式契約](#20-形式白透過一般画像入出力)に従う |
| 新規 Cell の複数作成 | 全件 staging 成功後の一括公開 | 一件でも失敗すれば session／tab を含め全件未公開とする |
| Batch folder 出力の一 item | 完成済み temporary output の atomic replace | 失敗・取消 item に部分 output を残さず、それ以前に成功した別 item の出力は保持する。job 全体の all-or-none は保証しない |
| 通常 native/raster pair 保存 | 両 member の install 成功後の path／authority／両 savepoint 公開 | 中間状態を保存成功にしない。部分 install は復元を試み、未解決状態は復旧対象として保持する。失敗時の authority stamp 修復／path・savepoint 失効と dirty 化は [FFI の pair authority 契約](docs/ffi.md)に従い、保存成功による savepoint 前進と区別する。commit fence 後の cleanup failure は確定済み保存を失敗へ戻さない。二 file 同時の filesystem 原子性は保証しない（[共有 I/O](#41-共有ファイル-io-と編集参照の分離)） |
| open／Revert／Sequence の文書置換 | target 検証後の Core replacement | commit 前は元文書と authority を保持する。commit 後の snapshot／presentation failure は確定文書を巻き戻さず、適用済み状態へ UI を整合させて報告する |
| resident hit による切替後の autosave | 切替と独立した recovery publication | 失敗しても outgoing resident の未保存編集を保持し、表示セルを巻き戻さず status へ報告する |
...
- 数値最大値は衝突のない source 記述ではない。高い Light Table revision が後続の低い raster revision を mask する場合、同じ最大値を持つ source の一方を削除しても値が変わらない場合、独立 revision namespace が同値になる場合、表示 mode の異なる view が cache を共有する場合がある。また透明結果は negative cache を持たない。これらは `revision-max` を性能上の正本とする際の既知制約であり、暗黙に別方式へ変更しない。衝突時に古い合成を再利用し得るため、この cache の正しさ保証には当該制約がある。意味状態・永続化・cache-free replay の決定性は別の契約として維持し、性能 gate の合格だけでこの制約の解消を主張しない。是正の判断条件は [第21節](#未決事項と変更の境界)に従う。
...
- 未指定事項と仕様の食い違いの判断は [第21節](#21-未指定事項の決定)と `AGENTS.md` に従う。
